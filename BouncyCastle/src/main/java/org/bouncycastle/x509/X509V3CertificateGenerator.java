...
public class X509V3CertificateGenerator {
	...
	/**
	 * Set the issuer distinguished name - the issuer is the entity whose private
	 * key is used to sign the certificate.
	 */
	...
	/**
	 * Set the subject distinguished name. The subject describes the entity
	...
	/**
	 * Set the signature algorithm. This can be either a name or an OID, names are
	...
	/**
	 * Set the subject unique ID - note: it is very rare that it is correct to do
	 * this.
	 */
	...
	/**
	 * Set the issuer unique ID - note: it is very rare that it is correct to do
	 * this.
	 */
	...
	/**
	 * add a given extension field for the standard extensions tag (tag 3) The value
	 * parameter becomes the contents of the octet string associated with the
	 * extension.
	 */
	...
	/**
	 * generate an X509 certificate, based on the current issuer and subject using
	 * the default provider "BC".
	 * 
	...
	/**
	 * generate an X509 certificate, based on the current issuer and subject using
	 * the default provider "BC", and the passed in source of randomness (if
	 * required).
	 * 
	...
	/**
	 * generate an X509 certificate, based on the current issuer and subject, using
	 * the passed in provider for the signing.
	 * 
	...
	/**
	 * generate an X509 certificate, based on the current issuer and subject, using
	 * the passed in provider for the signing and the supplied source of randomness,
	 * if required.
	 * 
	 * @deprecated use generate()
	 */
	 ...
	 * <p>
	 * <b>Note:</b> this differs from the deprecated method in that the default
	 * provider is used - not "BC".
	 * </p>
	 */
	...
	/**
	 * generate an X509 certificate, based on the current issuer and subject using
	 * the default provider, and the passed in source of randomness (if required).
	 * <p>
	 * <b>Note:</b> this differs from the deprecated method in that the default
	 * provider is used - not "BC".
	 * </p>
	 */
	public X509Certificate generate(PrivateKey key, SecureRandom random) throws CertificateEncodingException,
			IllegalStateException, NoSuchAlgorithmException, SignatureException, InvalidKeyException {
	...
	private X509Certificate generateJcaObject(TBSCertificate tbsCert, byte[] signature) throws Exception {
		...
		return (X509Certificate) certificateFactory
				.engineGenerateCertificate(new ByteArrayInputStream(new DERSequence(v).getEncoded(ASN1Encoding.DER)));
	}
