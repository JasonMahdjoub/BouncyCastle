...
public class ExtendedPKIXBuilderParameters extends ExtendedPKIXParameters {
	 ...
	 * certification path. If the <code>Set</code> is <code>null</code> an empty set
	 * is assumed.
	 * <p>
	...
	/**
	 * Creates an instance of <code>PKIXBuilderParameters</code> with the specified
	 ...
	 * certification path. The PKIX <code>CertPathBuilder</code> must not build
	 * paths longer then this length.
	 * <p>
	 * A value of 0 implies that the path can only contain a single certificate. A
	 * value of -1 does not limit the length. The default length is 5.
	 * 
	 * <p>
	 * 
	 * The basic constraints extension of a CA certificate overrides this value if
	 * smaller.
	 * 
	 * @param maxPathLength
	 *            the maximum number of non-self-issued intermediate certificates in
	 *            the certification path
	 * @throws InvalidParameterException
	 *             if <code>maxPathLength</code> is set to a value less than -1
	 * 
	 * @see org.bouncycastle.jce.provider.PKIXCertPathBuilderSpi
	 ...
	 * 
	 * @return the maximum number of non-self-issued intermediate certificates in
	 *         the certification path, or -1 if no limit exists.
	 * 
	...
	/**
	 * Makes a copy of this <code>PKIXParameters</code> object. Changes to the copy
	 ...
	 * <code>PKIXBuilderParameters</code>, <code>PKIXParameters</code>, and
	 * <code>ExtendedPKIXParameters</code> instances.
	 * 
	 * @param pkixParams
	...
	public static ExtendedPKIXParameters getInstance(PKIXParameters pkixParams) {
		...
		try {
			params = new ExtendedPKIXBuilderParameters(pkixParams.getTrustAnchors(),
					X509CertStoreSelector.getInstance((X509CertSelector) pkixParams.getTargetCertConstraints()));
		} catch (Exception e) {
			// cannot happen
			throw new RuntimeException(e.getMessage());
		}
