...
public class AttributeCertificateHolder implements CertSelector, Selector {
	...
	public AttributeCertificateHolder(X509Principal issuerName, BigInteger serialNumber) {
		holder = new org.bouncycastle.asn1.x509.Holder(new IssuerSerial(
				GeneralNames.getInstance(new DERSequence(new GeneralName(issuerName))), new ASN1Integer(serialNumber)));
	}
	...
	/**
	 * Constructs a holder for v2 attribute certificates with a hash value for some
	 * type of object.
	 * <p>
	 * <code>digestedObjectType</code> can be one of the following:
	 * <ul>
	 * <li>0 - publicKey - A hash of the public key of the holder must be passed.
	 * <li>1 - publicKeyCert - A hash of the public key certificate of the holder
	 * must be passed.
	 * <li>2 - otherObjectDigest - A hash of some other object type must be passed.
	 * <code>otherObjectTypeID</code> must not be empty.
	 * </ul>
	 * <p>
	 ...
	 *            The object type ID if <code>digestedObjectType</code> is
	...
	public AttributeCertificateHolder(int digestedObjectType, String digestAlgorithm, String otherObjectTypeID,
			byte[] objectDigest) {
		holder = new Holder(new ObjectDigestInfo(digestedObjectType, new ASN1ObjectIdentifier(otherObjectTypeID),
				new AlgorithmIdentifier(new ASN1ObjectIdentifier(digestAlgorithm)), Arrays.clone(objectDigest)));
	}
	 ...
	 * 
	 * @return The other object type ID or <code>null</code> if no object digest
	 *         info is set.
	 */
	 ...
	 * 
	 * @return The digest algorithm ID or <code>null</code> if no object digest info
	 *         is set.
	 */
	...
	private boolean matchesDN(X509Principal subject, GeneralNames targets) {
		...
		for (int i = 0; i != names.length; i++) {
			...
			if (gn.getTagNo() == GeneralName.directoryName) {
				try {
					if (new X509Principal(((ASN1Encodable) gn.getName()).toASN1Primitive().getEncoded())
							.equals(subject)) {
						return true;
					}
				} catch (IOException e) {
	...
	/**
	 * Return any principal objects inside the attribute certificate holder entity
	 * names field.
	 * 
		...
		try {
			if (holder.getBaseCertificateID() != null) {
				return holder.getBaseCertificateID().getSerial().getValue().equals(x509Cert.getSerialNumber())
						&& matchesDN(PrincipalUtil.getIssuerX509Principal(x509Cert),
