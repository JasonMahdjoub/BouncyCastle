...
public abstract class CipherSpiExt extends CipherSpi {
	...
	/**
	 * Initialize this cipher object with a proper key and some random seed. Before
	 * a cipher object is ready for data processing, it has to be initialized
	 * according to the desired cryptographic operation, which is specified by the
	 * <tt>opMode</tt> parameter.
	 * <p>
	 * If this cipher (including its underlying mode or padding scheme) requires any
	 * random bytes, it will obtain them from <tt>random</tt>.
	 * </p>
	 * <p>
	 * Note: If the mode needs an initialization vector, a blank array is used in
	 * this case.
	 * </p>
	 * 
	 * @param opMode
	 ...
	 */
	protected final void engineInit(int opMode, java.security.Key key, java.security.SecureRandom random)
			throws java.security.InvalidKeyException {

		try {
			engineInit(opMode, key, (java.security.spec.AlgorithmParameterSpec) null, random);
	 ...
	 * source of randomness. The cipher is initialized for encryption or decryption,
	 * depending on the value of <tt>opMode</tt>.
	 * <p>
	 * If this cipher (including its underlying mode or padding scheme) requires any
	 * random bytes, it will obtain them from <tt>random</tt>. Note that when a
	 * cipher object is initialized, it loses all previously-acquired state. In
	 * other words, initializing a Cipher is equivalent to creating a new instance
	 * of that Cipher and initializing it.
	 * </p>
	 * <p>
	 * Note: If the mode needs an initialization vector, a try to retrieve it from
	 * the AlgorithmParametersSpec is made.
	 * </p>
	 * 
	 * @param opMode
	 *            the operation mode ({@link #ENCRYPT_MODE} or
	 ...
	 *            the random seed
	 * @throws java.security.InvalidKeyException
	 *             if the key is inappropriate for initializing this block cipher.
	 * @throws java.security.InvalidAlgorithmParameterException
	 *             if the parameters are inappropriate for initializing this block
	 *             cipher.
	 */
	protected final void engineInit(int opMode, java.security.Key key, java.security.AlgorithmParameters algParams,
	...
	/**
	 * Initialize this cipher with a key, a set of algorithm parameters, and a
	 * source of randomness. The cipher is initialized for one of the following four
	 * operations: encryption, decryption, key wrapping or key unwrapping, depending
	 * on the value of opMode. If this cipher (including its underlying feedback or
	 * padding scheme) requires any random bytes (e.g., for parameter generation),
	 * it will get them from random. Note that when a Cipher object is initialized,
	 * it loses all previously-acquired state. In other words, initializing a Cipher
	 * is equivalent to creating a new instance of that Cipher and initializing it.
	 *
	 * @param opMode
	 *            the operation mode ({@link #ENCRYPT_MODE} or
	 *            {@link #DECRYPT_MODE})
	 ...
	 *             cipher, or if this cipher is being initialized for decryption and
	 *             requires algorithm parameters and the parameters are null.
	 */
	protected void engineInit(int opMode, java.security.Key key, java.security.spec.AlgorithmParameterSpec params,
	 ...
	 * given input data and any remaining buffered data. The data to be processed is
	 * given in an input byte array. Beginning at inputOffset, only the first
	 * inputLen bytes are en-/decrypted, including any buffered bytes of a previous
	 * update operation. If necessary, padding is performed. The result is returned
	 * as a output byte array.
	 *
	 * @param input
	 ...
	 *             if the ciphertext length is not a multiple of the blocklength.
	 ...
	 * any remaining buffered data. The data to be processed is given in an input
	 * byte array. Beginning at inputOffset, only the first inputLen bytes are
	 * en-/decrypted, including any buffered bytes of a previous update operation.
	 * If necessary, padding is performed. The result is stored in the given output
	 * byte array, beginning at outputOffset. The number of bytes stored in this
	 ...
	 *            array to which the en/decrypted data is written
	...
	/**
	 * @return the block size (in bytes), or 0 if the underlying algorithm is not a
	 *         block cipher
	 */
	 ...
	 *
	 * @return the initialization vector in a new buffer, or <tt>null</tt> if the
	...
	/**
	 * Return the length in bytes that an output buffer would need to be in order to
	 * hold the result of the next update or doFinal operation, given the input
	 * length inputLen (in bytes).
	 * <p>
	 * This call takes into account any unprocessed (buffered) data from a previous
	 * update call, and padding.
	 * </p>
	 * <p>
	 * The actual output length of the next update or doFinal call may be smaller
	 ...
	 * generated parameters used by the underlying cipher implementation (provided
	 * that the underlying cipher implementation uses a default set of parameters or
	 * creates new parameters if it needs parameters but was not initialized with
	 * any).
	 * </p>
	 * 
	 * @return the parameters used with this cipher, or null if this cipher does not
	 *         use any parameters.
	 */
	 ...
	 *             if neither the mode with the given name nor the default mode can
	 *             be found
	 */
	 ...
	 * inputOffset, only the first inputLen bytes are en-/decrypted. The result is
	...
	/**
	 * Perform the next step of a multi-step en-/decryption operation. The data to
	 * be processed is given in an input byte array. Beginning at inputOffset, only
	 * the first inputLen bytes are en-/decrypted. The result is stored in the given
	 * output byte array, beginning at outputOffset. The number of bytes stored in
	 * this output byte array are returned.
	 *
	 * @param input
	 *            the byte array holding the data to be processed
	 * @param inOff
	 ...
	 *             if the output buffer is too short to hold the output.
	 */
	protected final int engineUpdate(final byte[] input, final int inOff, final int inLen, byte[] output,
			final int outOff) throws javax.crypto.ShortBufferException {
	 ...
	 * </p>
	 * <p>
	 * If this cipher requires any algorithm parameters and paramSpec is null, the
	 ...
	 * InvalidAlgorithmParameterException if it is being initialized for decryption.
	 ...
	 *             if the given key is inappropriate for initializing this block
	 *             cipher.
	 ...
	 * underlying cipher implementation is supposed to generate the required
	 * parameters itself (using provider-specific default or random values) if it is
	 * being initialized for encryption, and throw an
	 ...
	 * to hold the result of the next update or doFinal operation, given the input
	 * length inputLen (in bytes).
	 * </p>
	 * <p>
	 * This call takes into account any unprocessed (buffered) data from a previous
	 ...
	 *             if the requested padding scheme cannot be found.
	 */
	protected abstract void setPadding(String padding) throws NoSuchPaddingException;

	/**
	 * Continue a multiple-part encryption or decryption operation (depending on how
	...
	/**
	 * Finish a multiple-part encryption or decryption operation (depending on how
	 * this cipher was initialized).
	 *
	 * @return a new buffer with the result
	 * @throws IllegalBlockSizeException
	 *             if this cipher is a block cipher and the total input length is
	 *             not a multiple of the block size (for encryption when no padding
	 *             is used or for decryption).
