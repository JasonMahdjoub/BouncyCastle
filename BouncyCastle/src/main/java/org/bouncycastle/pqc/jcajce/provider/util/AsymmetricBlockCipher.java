...
/**
 * The AsymmetricBlockCipher class extends CipherSpiExt. NOTE: Some Ciphers are
 * using Padding. OneAndZeroesPadding is used as default padding. However
 * padding can still be specified, but mode is not supported; if you try to
 * instantiate the cipher with something else than "NONE" as mode
...
public abstract class AsymmetricBlockCipher extends CipherSpiExt {
	...
	/**
	 * Return the block size (in bytes). Note: although the ciphers extending this
	 * class are not block ciphers, the method was adopted to return the maximal
	 * plaintext and ciphertext sizes for non hybrid ciphers. If the cipher is
	 * hybrid, it returns 0.
	 *
	 * @return if the cipher is not a hybrid one the max plain/cipher text size is
	 ...
	 * length <tt>inLen</tt> (in bytes). This call takes into account any
	 * unprocessed (buffered) data from a previous update call, and padding. The
	 * actual output length of the next update() or doFinal() call may be smaller
	 * than the length returned by this method.
	 * <p>
	 * If the input length plus the length of the buffered data exceeds the maximum
	 * length, <tt>0</tt> is returned.
	 * </p>
	 * 
	 * @param inLen
	 *            the length of the input
	 * @return the length of the ciphertext or <tt>0</tt> if the input is too long.
	 */
	...
	/**
	 * Initializes the cipher for encryption by forwarding it to initEncrypt(Key,
	 * FlexiSecureRandom).
	 * <p>
	 * If this cipher requires any algorithm parameters that cannot be derived from
	 * the given key, the underlying cipher implementation is supposed to generate
	 * the required parameters itself (using provider-specific default or random
	 * values) if it is being initialized for encryption, and raise an
	 * InvalidKeyException if it is being initialized for decryption. The generated
	 * parameters can be retrieved using engineGetParameters or engineGetIV (if the
	 * parameter is an IV).
	 * </p>
	 * 
	 * @param key
	...
	/**
	 * Initialize this cipher for encryption by forwarding it to initEncrypt(Key,
	 ...
	 *             if the given algortihm parameters are inappropriate for this
	...
	/**
	 * This method initializes the AsymmetricBlockCipher with a certain key for data
	 * encryption.
	 * <p>
	...
	/**
	 * Continue a multiple-part encryption or decryption operation. This method just
	 * writes the input into an internal buffer.
	 *
	 * @param input
	 ...
	 * en/decrypted. This method uses the specific values {@link #maxPlainTextSize}
	 * and {@link #cipherTextSize} which are set by the implementations. If the
	 * input length plus the length of the internal buffer is greater than
	 * {@link #maxPlainTextSize} for encryption or not equal to
	 * {@link #cipherTextSize} for decryption, an {@link IllegalBlockSizeException}
	 * will be thrown.
	 *
	 * @param inLen
	...
	protected void checkLength(int inLen) throws IllegalBlockSizeException {
		...
		if (opMode == ENCRYPT_MODE) {
			if (inLength > maxPlainTextSize) {
				throw new IllegalBlockSizeException("The length of the plaintext (" + inLength
						+ " bytes) is not supported by " + "the cipher (max. " + maxPlainTextSize + " bytes).");
			}
	 ...
	 *             if the given key is inappropriate for initializing this cipher.
	 * @throws InvalidAlgorithmParameterException
	 *             if the given parameters are inappropriate for initializing this
	 *             cipher.
	 */
	 ...
	 *
	 * @param input
	 *            the message to be encrypted (usually the message length is less
	 *            than or equal to maxPlainTextSize)
	 ...
	 *
	 * @param input
	 *            the ciphertext to be decrypted (the ciphertext length is less than
	 *            or equal to maxCipherTextSize)
