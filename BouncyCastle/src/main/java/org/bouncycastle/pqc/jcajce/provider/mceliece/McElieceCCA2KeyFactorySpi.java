...
public class McElieceCCA2KeyFactorySpi extends KeyFactorySpi {
	 ...
	 * {@link BCMcElieceCCA2PublicKey}. Currently, the following key specifications
	 * are supported: {@link X509EncodedKeySpec}.
	 *
	 * @param keySpec
		...
		if (keySpec instanceof X509EncodedKeySpec) {
			...
			try {
				if (PQCObjectIdentifiers.mcElieceCca2.equals(pki.getAlgorithm().getAlgorithm())) {
					...
					return new BCMcElieceCCA2PublicKey(new McElieceCCA2PublicKeyParameters(key.getN(), key.getT(),
							key.getG(), Utils.getDigest(key.getDigest()).getAlgorithmName()));
				} else {
	...
	/**
	 * Converts, if possible, a key specification into a
	 * {@link BCMcElieceCCA2PrivateKey}. Currently, the following key specifications
	 * are supported: {@link PKCS8EncodedKeySpec}.
	 *
	 * @param keySpec
	 *            the key specification
		...
		if (keySpec instanceof PKCS8EncodedKeySpec) {
			...
			try {
				if (PQCObjectIdentifiers.mcElieceCca2.equals(pki.getPrivateKeyAlgorithm().getAlgorithm())) {
					...
					return new BCMcElieceCCA2PrivateKey(new McElieceCCA2PrivateKeyParameters(key.getN(), key.getK(),
							key.getField(), key.getGoppaPoly(), key.getP(),
							Utils.getDigest(key.getDigest()).getAlgorithmName()));
				} else {
	...
	/**
	 * Converts, if possible, a given key into a key specification. Currently, the
	 ...
	 * @throws InvalidKeySpecException
	 *             if the key type or the key specification is not supported.
	 * @see BCMcElieceCCA2PrivateKey
	 * @see BCMcElieceCCA2PublicKey
	 */
	...
	/**
	 * Translates a key into a form known by the FlexiProvider. Currently, only the
	 * following "source" keys are supported: {@link BCMcElieceCCA2PrivateKey},
	...
	public PublicKey generatePublic(SubjectPublicKeyInfo pki) throws InvalidKeySpecException {
		...
		try {
			...
			return new BCMcElieceCCA2PublicKey(new McElieceCCA2PublicKeyParameters(key.getN(), key.getT(), key.getG(),
					Utils.getDigest(key.getDigest()).getAlgorithmName()));
		} catch (IOException cce) {
	...
	public PrivateKey generatePrivate(PrivateKeyInfo pki) throws InvalidKeySpecException {
		...
		try {
			...
			return new BCMcElieceCCA2PrivateKey(new McElieceCCA2PrivateKeyParameters(key.getN(), key.getK(),
					key.getField(), key.getGoppaPoly(), key.getP(), null));
		} catch (IOException cce) {
			throw new InvalidKeySpecException("Unable to decode PKCS8EncodedKeySpec");
		}
	}
