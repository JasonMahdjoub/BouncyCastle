...
public class McElieceKeyFactorySpi extends KeyFactorySpi {
		...
		if (keySpec instanceof PKCS8EncodedKeySpec) {
			...
			try {
				if (PQCObjectIdentifiers.mcEliece.equals(pki.getPrivateKeyAlgorithm().getAlgorithm())) {
					...
					return new BCMcEliecePrivateKey(new McEliecePrivateKeyParameters(key.getN(), key.getK(),
							key.getField(), key.getGoppaPoly(), key.getP1(), key.getP2(), key.getSInv()));
				} else {
	 ...
	 * following "source" keys are supported: {@link BCMcEliecePrivateKey},
	...
	public PrivateKey generatePrivate(PrivateKeyInfo pki) throws InvalidKeySpecException {
		...
		try {
			...
			return new BCMcEliecePrivateKey(new McEliecePrivateKeyParameters(key.getN(), key.getK(), key.getField(),
					key.getGoppaPoly(), key.getP1(), key.getP2(), key.getSInv()));
		} catch (IOException cce) {
			throw new InvalidKeySpecException("Unable to decode PKCS8EncodedKeySpec");
		}
	}
