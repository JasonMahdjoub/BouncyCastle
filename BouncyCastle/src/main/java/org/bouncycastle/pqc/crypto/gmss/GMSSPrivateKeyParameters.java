...
public class GMSSPrivateKeyParameters extends GMSSKeyParameters {
	 ...
	 *            subtrees
	 * @param nextNextSeed
	 *            seed for the generation of private OTS keys for the next subtrees
	...
	public GMSSPrivateKeyParameters(byte[][] currentSeed, byte[][] nextNextSeed, byte[][][] currentAuthPath,
			byte[][][] nextAuthPath, Treehash[][] currentTreehash, Treehash[][] nextTreehash, Vector[] currentStack,
			Vector[] nextStack, Vector[][] currentRetain, Vector[][] nextRetain, byte[][] nextRoot,
			byte[][] currentRootSig, GMSSParameters gmssParameterset, GMSSDigestProvider digestProvider) {
		this(null, currentSeed, nextNextSeed, currentAuthPath, nextAuthPath, null, currentTreehash, nextTreehash,
				currentStack, nextStack, currentRetain, nextRetain, null, null, null, null, nextRoot, null,
				currentRootSig, null, gmssParameterset, digestProvider);
	}
	 ...
	 *            the GMSS Parameterset
	 */
	public GMSSPrivateKeyParameters(int[] index, byte[][] currentSeeds, byte[][] nextNextSeeds,
			...
			GMSSRootSig[] nextRootSig, GMSSParameters gmssParameterset, GMSSDigestProvider digestProvider) {
		...
		if (layer > 0) {
			...
			if (!lastTree) {
				...
				for (int i = 0; i < heightOfTrees[layer]; i++) {
					...
					System.arraycopy(nextNextRoot[layer - 1].getAuthPath()[i], 0, nextAuthPaths[layer - 1][i], 0,
							mdLength);
				}
	...
	/**
	 * This method computes the authpath (AUTH) for the current tree, Additionally
	 * the root signature for the next tree (SIG+), the authpath (AUTH++) and root
	 * (ROOT++) for the tree after next in layer <code>layer</code>, and the
	 * LEAF++^1 for the next next tree in the layer above are updated This method is
	 * used by nextKey()
	 *
	 * @param layer
	 */
	private void updateKey(int layer) {
		...
		if (layer > 0) {
			...
			int t = (int) Math.floor((double) (this.getNumLeafs(layer) * 2)
			...
			if (index[layer] % t == 1) {
				...
				if (this.minTreehash[layer - 1] >= 0) {
					...
					this.upperTreehashLeaf[layer - 1] = new GMSSLeaf(this.digestProvider.get(),
							this.otsIndex[layer - 1], t, seed);
	...
	/**
	 * Computes the upcoming currentAuthpath of layer <code>layer</code> using the
	 * revisited authentication path computation of Dahmen/Schneider 2008
	 *
	 * @param layer
	 *            the actual layer
	 */
			...
			for (int i = 0; i < Tau; i++) {
				...
				if (i < H - K) {
					if (currentTreehash[layer][i].wasFinished()) {
						System.arraycopy(currentTreehash[layer][i].getFirstNode(), 0, currentAuthPaths[layer][i], 0,
								mdLength);
						currentTreehash[layer][i].destroy();
					} else {
						System.err.println(
								"Treehash (" + layer + "," + i + ") not finished when needed in AuthPathComputation");
					}
				}
				...
				if (i < H - 1 && i >= H - K) {
					if (currentRetain[layer][i - (H - K)].size() > 0) {
						...
						System.arraycopy(currentRetain[layer][i - (H - K)].lastElement(), 0, currentAuthPaths[layer][i],
								0, mdLength);
			...
			for (int tmp = 1; tmp <= (H - K) / 2; tmp++) {
				...
				if (minTreehash >= 0) {
					try {
						...
						System.arraycopy(this.currentTreehash[layer][minTreehash].getSeedActive(), 0, seed, 0,
								mdLength);
						byte[] seed2 = gmssRandom.nextSeed(seed);
						WinternitzOTSignature ots = new WinternitzOTSignature(seed2, this.digestProvider.get(),
								this.otsIndex[layer]);
	...
	/**
	 * Updates the authentication path and root calculation for the tree after next
	 * (AUTH++, ROOT++) in layer <code>layer</code>
	 *
	 * @param layer
	 */
