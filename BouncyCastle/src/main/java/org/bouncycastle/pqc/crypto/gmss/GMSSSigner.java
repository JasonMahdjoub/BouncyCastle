...
public class GMSSSigner implements MessageSigner {
	...
	/**
	 * Array of strings containing the name of the hash function used by the OTS and
	 * the corresponding provider name
	 */
	...
	private void initSign() {
		...
		this.ots = new WinternitzOTSignature(OTSSeed, digestProvider.get(),
				gmssPS.getWinternitzParameter()[numLayer - 1]);
	...
	public byte[] generateSignature(byte[] message) {
		...
		for (int i = numLayer - 1 - 1; i >= 0; i--) {
			...
			gmssSigNextPart = new byte[helpGmssSig.length + indexBytes.length + subtreeRootSig[i].length
			...
			System.arraycopy(subtreeRootSig[i], 0, gmssSigNextPart, (helpGmssSig.length + indexBytes.length),
					subtreeRootSig[i].length);
			System.arraycopy(authPathBytes, 0, gmssSigNextPart,
					(helpGmssSig.length + indexBytes.length + subtreeRootSig[i].length), authPathBytes.length);
	...
	/**
	 * This function verifies the signature of the message that has been updated,
