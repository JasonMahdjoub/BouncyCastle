...
class Horst {
	...
	static final int HORST_SIGBYTES = (64 * SPHINCS256Config.HASH_BYTES
			+ (((HORST_LOGT - 6) * SPHINCS256Config.HASH_BYTES) + HORST_SKBYTES) * HORST_K);

	static final int N_MASKS = (2 * (Horst.HORST_LOGT)); /*
															 * has to be the max of (2*(SUBTREE_HEIGHT+WOTS_LOGL)) and
															 * (WOTS_W-1) and 2*HORST_LOGT
															 */
	...
	static int horst_sign(HashFunctions hs, byte[] sig, int sigOff, byte[] pk, byte[] seed, byte[] masks,
			byte[] m_hash) {
		...
		byte[] tree = new byte[(2 * HORST_T - 1)
				* SPHINCS256Config.HASH_BYTES]; /* replace by something more memory-efficient? */
		...
		for (i = 0; i < HORST_LOGT; i++) {
			...
			for (j = 0; j < (1 << (HORST_LOGT - i - 1)); j++) {
				hs.hash_2n_n_mask(tree, (int) ((offset_out + j) * SPHINCS256Config.HASH_BYTES), tree,
						(int) ((offset_in + 2 * j) * SPHINCS256Config.HASH_BYTES), masks,
						2 * i * SPHINCS256Config.HASH_BYTES);
			}
		}
		...
		// Signature consists of HORST_K parts; each part of secret key and HORST_LOGT-4
		// auth-path hashes
		for (i = 0; i < HORST_K; i++) {
	...
	static int horst_verify(HashFunctions hs, byte[] pk, byte[] sig, int sigOff, byte[] masks, byte[] m_hash) {
			...
			for (j = 1; j < HORST_LOGT - 6; j++) {
				...
				if ((idx & 1) == 0) {
					...
					hs.hash_2n_n_mask(buffer, SPHINCS256Config.HASH_BYTES, buffer, 0, masks,
							2 * (j - 1) * SPHINCS256Config.HASH_BYTES);
					for (k = 0; k < SPHINCS256Config.HASH_BYTES; k++)
		...
		for (j = 0; j < 32; j++) {
			hs.hash_2n_n_mask(buffer, j * SPHINCS256Config.HASH_BYTES, sig,
					sigOff + 2 * j * SPHINCS256Config.HASH_BYTES, masks,
					2 * (HORST_LOGT - 6) * SPHINCS256Config.HASH_BYTES);
		}

		// Hash from level 11 to 12
		for (j = 0; j < 16; j++) {
			hs.hash_2n_n_mask(buffer, j * SPHINCS256Config.HASH_BYTES, buffer, 2 * j * SPHINCS256Config.HASH_BYTES,
					masks, 2 * (HORST_LOGT - 5) * SPHINCS256Config.HASH_BYTES);
		}

		// Hash from level 12 to 13
		for (j = 0; j < 8; j++) {
			hs.hash_2n_n_mask(buffer, j * SPHINCS256Config.HASH_BYTES, buffer, 2 * j * SPHINCS256Config.HASH_BYTES,
					masks, 2 * (HORST_LOGT - 4) * SPHINCS256Config.HASH_BYTES);
		}

		// Hash from level 13 to 14
		for (j = 0; j < 4; j++) {
			hs.hash_2n_n_mask(buffer, j * SPHINCS256Config.HASH_BYTES, buffer, 2 * j * SPHINCS256Config.HASH_BYTES,
					masks, 2 * (HORST_LOGT - 3) * SPHINCS256Config.HASH_BYTES);
		}

		// Hash from level 14 to 15
		for (j = 0; j < 2; j++) {
			hs.hash_2n_n_mask(buffer, j * SPHINCS256Config.HASH_BYTES, buffer, 2 * j * SPHINCS256Config.HASH_BYTES,
					masks, 2 * (HORST_LOGT - 2) * SPHINCS256Config.HASH_BYTES);
		}
