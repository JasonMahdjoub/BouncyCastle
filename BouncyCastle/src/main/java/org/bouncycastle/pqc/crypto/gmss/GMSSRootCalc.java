...
public class GMSSRootCalc {
	...
	/**
	 * Integer that stores the index of the next seed that has to be omitted to the
	 * treehashs
	 */
	private int indexForNextSeed;

	/**
	 * temporary integer that stores the height of the next treehash instance that
	 * gets initialized with a seed
	 */
	 ...
	 *            function and PRNG and the digest of the corresponding provider
	 * @param statByte
	 *            status bytes
	 * @param statInt
	 *            status ints
	...
	public GMSSRootCalc(Digest digest, byte[][] statByte, int[] statInt, Treehash[] treeH, Vector[] ret) {
		...
		for (int i = 0; i < tailLength; i++) {
			this.heightOfNodes.addElement(Integers.valueOf(statInt[8 + heightOfTree + i]));
		}
	 ...
	 *            an array of strings, containing the name of the used hash function
	 *            and PRNG and the name of the corresponding provider
	 */
	...
	/**
	 * updates the root with one leaf and stores needed values in retain, treehash
	 * or authpath. Additionally counts the seeds used. This method is used when
	 * performing the updates for TREE++.
	 *
	 * @param seed
	...
	public void update(byte[] leaf) {
		...
		if (index[0] == 0) {
			...
			while (tailStack.size() > 0 && helpHeight == ((Integer) heightOfNodes.lastElement()).intValue()) {
