...
class Tree {
	...
	static void l_tree(HashFunctions hs, byte[] leaf, int leafOff, byte[] wots_pk, int pkOff, byte[] masks,
			int masksOff) {
		int l = Wots.WOTS_L;
		int i, j = 0;
		for (i = 0; i < Wots.WOTS_LOG_L; i++) {
			for (j = 0; j < (l >>> 1); j++) {
				hs.hash_2n_n_mask(wots_pk, pkOff + j * SPHINCS256Config.HASH_BYTES, wots_pk,
						pkOff + j * 2 * SPHINCS256Config.HASH_BYTES, masks,
						masksOff + i * 2 * SPHINCS256Config.HASH_BYTES);
			}

			if ((l & 1) != 0) {
				System.arraycopy(wots_pk, pkOff + (l - 1) * SPHINCS256Config.HASH_BYTES, wots_pk,
						pkOff + (l >>> 1) * SPHINCS256Config.HASH_BYTES, SPHINCS256Config.HASH_BYTES);
				l = (l >>> 1) + 1;
			} else {
				l = (l >>> 1);
			}
	...
	static void treehash(HashFunctions hs, byte[] node, int nodeOff, int height, byte[] sk, leafaddr leaf, byte[] masks,
			int masksOff) {
		...
		for (; a.subleaf < lastnode; a.subleaf++) {
			...
			while (stackoffset > 1 && stacklevels[stackoffset - 1] == stacklevels[stackoffset - 2]) {
				...
				hs.hash_2n_n_mask(stack, (stackoffset - 2) * SPHINCS256Config.HASH_BYTES, stack,
						(stackoffset - 2) * SPHINCS256Config.HASH_BYTES, masks, masksOff + maskoffset);
	...
	static void gen_leaf_wots(HashFunctions hs, byte[] leaf, int leafOff, byte[] masks, int masksOff, byte[] sk,
			leafaddr a) {
