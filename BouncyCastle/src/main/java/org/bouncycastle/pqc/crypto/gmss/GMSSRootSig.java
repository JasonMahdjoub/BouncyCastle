...
public class GMSSRootSig {
	...
	public GMSSRootSig(Digest digest, byte[][] statByte, int[] statInt) {
		...
		this.test8 = ((statByte[4][0] & 0xff)) | ((long) (statByte[4][1] & 0xff) << 8)
				| ((long) (statByte[4][2] & 0xff) << 16) | ((long) (statByte[4][3] & 0xff)) << 24
				| ((long) (statByte[4][4] & 0xff)) << 32 | ((long) (statByte[4][5] & 0xff)) << 40
				| ((long) (statByte[4][6] & 0xff)) << 48 | ((long) (statByte[4][7] & 0xff)) << 56;

		this.big8 = ((statByte[4][8] & 0xff)) | ((long) (statByte[4][9] & 0xff) << 8)
				| ((long) (statByte[4][10] & 0xff) << 16) | ((long) (statByte[4][11] & 0xff)) << 24
				| ((long) (statByte[4][12] & 0xff)) << 32 | ((long) (statByte[4][13] & 0xff)) << 40
				| ((long) (statByte[4][14] & 0xff)) << 48 | ((long) (statByte[4][15] & 0xff)) << 56;
	}
	...
	/**
	 * This method initializes the distributed sigature calculation. Variables are
	 * reseted and necessary steps are calculated
	 *
	 * @param seed0
	 *            the initial OTSseed
	 * @param message
	...
	public boolean updateSign() {
		...
		for (int s = 0; s < steps; s++) { // do 'step' times

			if (counter < keysize) { // generate the private key or perform
										// the next hash
				oneStep();
			}
			if (counter == keysize) {// finish
				return true;
			}
		}
	...
	private void oneStep() {
		...
		if (8 % w == 0) {
			if (test == 0) {
				...
				if (ii < mdsize) { // for main message part
					test = hash[ii] & k;
					hash[ii] = (byte) (hash[ii] >>> w);
				} else { // for checksum part
					test = checksum & k;
					checksum >>>= w;
				}
			} else if (test > 0) { // hash the private Key 'test' times (on
									// time each step)
			...
			if (test == 0) { // if all hashes done copy result to siganture
								// array
				System.arraycopy(privateKeyOTS, 0, sign, counter * mdsize, mdsize);
				counter++;

				if (counter % (8 / w) == 0) { // raise array index for main
												// massage part
					ii++;
				}
			}
			...
			if (test == 0) {
				if (counter % 8 == 0 && ii < mdsize) { // after every 8th "add
														// to signature"-step
					big8 = 0;
					if (counter < ((mdsize / w) << 3)) {// main massage
														// (generate w*8 Bits
														// every time) part
						for (int j = 0; j < w; j++) {
					...
					} else { // rest of massage part (once)
				...
				if (counter == messagesize) { // checksum part (once)
					big8 = checksum;
				}
			...
			if (test8 == 0) {
				...
				// --- message part---
				if (s < mdsize) {
					if (r <= ((mdsize << 3) - w)) { // first message part
						r += w;
						f = (r + 7) >>> 3;
					} else { // rest of message part (once)
						f = mdsize;
						r += w;
					}
			...
			} else if (test8 > 0) { // hash the private Key 'test' times (on
									// time each step)
				messDigestOTS.update(privateKeyOTS, 0, privateKeyOTS.length);
				privateKeyOTS = new byte[messDigestOTS.getDigestSize()];
				messDigestOTS.doFinal(privateKeyOTS, 0);
			...
			if (test8 == 0) { // if all hashes done copy result to siganture
								// array
				System.arraycopy(privateKeyOTS, 0, sign, counter * mdsize, mdsize);
				counter++;
			}
	 ...
	 * @param intValue
	 *            an integer
	 * @return The least integer greater or equal to the logarithm to the base 2 of
	 *         <code>intValue</code>
	 */
