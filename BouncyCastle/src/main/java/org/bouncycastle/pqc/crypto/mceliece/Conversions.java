...
final class Conversions {
	 ...
	 * vector of length n with weight t. The number is given as a byte array. Only
	 * the first s bits are used, where s = floor[log(n|t)].
	 *
	 * @param n
	 *            integer
	 * @param t
	...
	/**
	 * Decode a binary vector of length n and weight t into a number between 0 and
	 * (n|t) (binomial coefficient). The result is given as a byte array of length
	 * floor[(s+7)/8], where s = floor[log(n|t)].
	 *
	 * @param n
	 *            integer
	 * @param t
	 ...
	 * <tt>n</tt> bit and of hamming weight <tt>t</tt>. The result is a byte array
	 * of length <tt>(s+7)/8</tt>, where <tt>s = floor[log(n|t)]</tt>.
	 *
	 * @param n
	 *            integer
	 * @param t
		...
		for (int i = 0; i < n; i++) {
			...
			if (e != 0) {
				...
				if (nn == tt) {
					...
					bc = (bc.multiply(new BigInteger(Integer.toString(tt + 1))))
