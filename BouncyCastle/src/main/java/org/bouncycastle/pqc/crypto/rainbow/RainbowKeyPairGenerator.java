...
/**
 * This class implements AsymmetricCipherKeyPairGenerator. It is used as a
 * generator for the private and public key of the Rainbow Signature Scheme.
 * <p>
...
public class RainbowKeyPairGenerator implements AsymmetricCipherKeyPairGenerator {
	...
	private void initializeDefault() {
		RainbowKeyGenerationParameters rbKGParams = new RainbowKeyGenerationParameters(new SecureRandom(),
				new RainbowParameters());
		initialize(rbKGParams);
	}

	/**
	 * This function calls the functions for the random generation of the
	 * coefficients and the matrices needed for the private key and the method for
	 ...
	 * <p>
	 * The translation part b1, is stored in a separate array. The inverse of the
	 * matrix-part of L1 A1inv is also computed here.
	 * </p>
	 * <p>
	 ...
	 * <p>
	 * The translation part b2, is stored in a separate array. The inverse of the
	 * matrix-part of L2 A2inv is also computed here.
	 * </p>
	 * <p>
	...
	/**
	 * This function generates the private map F, which consists of u-1 layers. Each
	 * layer consists of oi polynomials where oi = vi[i+1]-vi[i].
	 * <p>
	 * The methods for the generation of the coefficients of these polynomials are
	 * called here.
	 * </p>
	 */
	 ...
	 * composition's result. The singular and scalar values constitute to the public
	 * key as is, the quadratic terms are compacted in <tt>compactPublicKey()</tt>
	 * </p>
	 */
			...
			for (int p = 0; p < oils; p++) {
				...
				for (int x1 = 0; x1 < oils; x1++) {
					for (int x2 = 0; x2 < vins; x2++) {
						...
						coeff_quadratic_3dim[crnt_row + p] = c.addSquareMatrix(coeff_quadratic_3dim[crnt_row + p],
								c.multVects(vect_tmp, this.A2[x2]));
						...
						this.pub_scalar[crnt_row + p] = GF2Field.addElem(this.pub_scalar[crnt_row + p],
								GF2Field.multElem(sclr_tmp, this.b2[x2]));
					}
				}
				// multiply betas
				...
				for (int n = 0; n < vins + oils; n++) {
					...
					this.pub_scalar[crnt_row + p] = GF2Field.addElem(this.pub_scalar[crnt_row + p],
							GF2Field.multElem(coeff_gamma[p][n], this.b2[n]));
				}
				// add eta
	...
	/**
	 * The quadratic (or mixed) terms of the public key are compacted from a n x n
	 * matrix per polynomial to an upper diagonal matrix stored in one integer array
	 * of n (n + 1) / 2 elements per polynomial. The ordering of elements is
	 * lexicographic and the result is updating <tt>this.pub_quadratic</tt>, which
	 * stores the quadratic elements of the public key.
	 *
	 * @param coeff_quadratic_to_compact
	 *            3-dimensional array containing a n x n Matrix for each of the n -
	 *            v1 polynomials
	 */
			...
			for (int x = 0; x < n; x++) {
				for (int y = x; y < n; y++) {
					if (y == x) {
						...
						this.pub_quadratic[p][offset] = GF2Field.addElem(coeff_quadratic_to_compact[p][x][y],
