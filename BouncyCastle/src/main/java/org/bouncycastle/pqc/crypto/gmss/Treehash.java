...
public class Treehash {
	...
	/**
	 * Calculates one update of the treehash instance, i.e. creates a new leaf and
	 * hashes if possible
	 *
	 * @param gmssRandom
	...
	public void update(GMSSRandom gmssRandom, byte[] leaf) {
		...
		if (this.firstNode == null) {
			...
			while (this.tailLength > 0 && helpHeight == ((Integer) heightOfNodes.lastElement()).intValue()) {
				...
				System.arraycopy(help, 0, toBeHashed, this.messDigestTree.getDigestSize(),
			...
			if (((Integer) heightOfNodes.lastElement()).intValue() == this.firstNodeHeight) {
				...
				System.arraycopy(this.tailStack.lastElement(), 0, toBeHashed, this.messDigestTree.getDigestSize(),
						this.messDigestTree.getDigestSize());
				this.tailStack.removeElementAt(this.tailStack.size() - 1);
				this.heightOfNodes.removeElementAt(this.heightOfNodes.size() - 1);
	...
	/**
	 * Destroys a treehash instance after the top node was taken for authentication
	 * path.
	 */
	public void destroy() {
		this.isInitialized = false;
	 ...
	 * this cases are considered in the computeAuthPaths method of JDKGMSSPrivateKey
	 *
	...
	public int getLowestNodeHeight() {
		if (this.firstNode == null) {
			...
			return Math.min(this.firstNodeHeight, ((Integer) heightOfNodes.lastElement()).intValue());
		}
	}
