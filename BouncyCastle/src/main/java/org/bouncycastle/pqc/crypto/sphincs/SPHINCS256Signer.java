 ...
 * SPHINCS-256 signer.
 * <p>
 * This implementation is heavily based on the reference implementation in
 * SUPERCOP, the main difference being the digests used for message hashing and
 * tree construction are now configurable (within limits...) and that the
 * implementation produces detached signatures.
 * </p>
 */
public class SPHINCS256Signer implements MessageSigner {
	 ...
	 *
	 * @param nDigest
	 *            the "n-digest" must produce 32 bytes of output - used for tree
	 *            construction.
	 * @param twoNDigest
	 *            the "2n-digest" must produce 64 bytes of output - used for initial
	 *            message/key/seed hashing.
	 */
	...
	static void validate_authpath(HashFunctions hs, byte[] root, byte[] leaf, int leafidx, byte[] authpath, int auOff,
		...
		for (i = 0; i < height - 1; i++) {
			leafidx >>>= 1;
			if ((leafidx & 1) != 0) {
				hs.hash_2n_n_mask(buffer, SPHINCS256Config.HASH_BYTES, buffer, 0, masks,
						2 * (Wots.WOTS_LOG_L + i) * SPHINCS256Config.HASH_BYTES);
				for (j = 0; j < SPHINCS256Config.HASH_BYTES; j++) {
	...
	static void compute_authpath_wots(HashFunctions hs, byte[] root, byte[] authpath, int authOff, Tree.leafaddr a,
			byte[] sk, byte[] masks, int height) {
		int i, idx, j;
		...
		for (ta.subleaf = 0; ta.subleaf < (1 << SPHINCS256Config.SUBTREE_HEIGHT); ta.subleaf++) {
			w.wots_pkgen(hs, pk, (int) (ta.subleaf * Wots.WOTS_L * SPHINCS256Config.HASH_BYTES), seed,
					(int) (ta.subleaf * SPHINCS256Config.SEED_BYTES), masks, 0);
		}

		for (ta.subleaf = 0; ta.subleaf < (1 << SPHINCS256Config.SUBTREE_HEIGHT); ta.subleaf++) {
			Tree.l_tree(hs, tree,
					(int) ((1 << SPHINCS256Config.SUBTREE_HEIGHT) * SPHINCS256Config.HASH_BYTES
							+ ta.subleaf * SPHINCS256Config.HASH_BYTES),
		...
		for (i = (1 << SPHINCS256Config.SUBTREE_HEIGHT); i > 0; i >>>= 1) {
			for (j = 0; j < i; j += 2) {
				hs.hash_2n_n_mask(tree,
						(i >>> 1) * SPHINCS256Config.HASH_BYTES + (j >>> 1) * SPHINCS256Config.HASH_BYTES, tree,
						i * SPHINCS256Config.HASH_BYTES + j * SPHINCS256Config.HASH_BYTES, masks,
						2 * (Wots.WOTS_LOG_L + level) * SPHINCS256Config.HASH_BYTES);
			}
		...
		// copy authpath
		for (i = 0; i < height; i++) {
			System.arraycopy(tree,
					((1 << SPHINCS256Config.SUBTREE_HEIGHT) >>> i) * SPHINCS256Config.HASH_BYTES
							+ ((idx >>> i) ^ 1) * SPHINCS256Config.HASH_BYTES,
					authpath, authOff + i * SPHINCS256Config.HASH_BYTES, SPHINCS256Config.HASH_BYTES);
		}
	...
	byte[] crypto_sign(HashFunctions hs, byte[] m, byte[] sk) {
		...
		// create leafidx deterministically
		{
			...
			System.arraycopy(tsk, SPHINCS256Config.CRYPTO_SECRETKEYBYTES - SPHINCS256Config.SK_RAND_SEED_BYTES, sm,
					scratch, SPHINCS256Config.SK_RAND_SEED_BYTES);
			...
			// prepare msg_hash
			scratch = SPHINCS256Config.CRYPTO_BYTES - SPHINCS256Config.MESSAGE_HASH_SEED_BYTES
					- SPHINCS256Config.CRYPTO_PUBLICKEYBYTES;
			...
			Tree.treehash(hs, sm, pk + (Horst.N_MASKS * SPHINCS256Config.HASH_BYTES), SPHINCS256Config.SUBTREE_HEIGHT,
					tsk, b, sm, pk);
	...
	boolean verify(HashFunctions hs, byte[] m, byte[] sm, byte[] pk) {
		...
		for (i = 0; i < SPHINCS256Config.N_LEVELS; i++) {
			...
			validate_authpath(hs, root, pkhash, (int) (leafidx & 0x1f), sig, sigp, tpk,
					SPHINCS256Config.SUBTREE_HEIGHT);
			leafidx >>= 5;
