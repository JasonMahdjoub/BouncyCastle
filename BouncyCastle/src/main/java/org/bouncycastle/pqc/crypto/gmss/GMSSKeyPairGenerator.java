...
public class GMSSKeyPairGenerator implements AsymmetricCipherKeyPairGenerator {
	...
	/**
	 * The standard constructor tries to generate the GMSS algorithm identifier with
	...
	private AsymmetricCipherKeyPair genKeyPair() {
		...
		GMSSPrivateKeyParameters privateKey = new GMSSPrivateKeyParameters(currentSeeds, nextNextSeeds,
				currentAuthPaths, nextAuthPaths, currentTreehash, nextTreehash, currentStack, nextStack, currentRetain,
				nextRetain, nextRoots, currentRootSigs, gmssPS, digestProvider);
	 ...
	 *
	 * @param keySize
	 *            Assigns the parameters used for the GMSS signatures. There are 3
	 *            choices:<br>
	...
	public void initialize(KeyGenerationParameters param) {
		...
		this.gmssPS = new GMSSParameters(gmssParams.getParameters().getNumOfLayers(),
				gmssParams.getParameters().getHeightOfTrees(), gmssParams.getParameters().getWinternitzParameter(),
				gmssParams.getParameters().getK());
	...
	private void initializeDefault() {
		...
		KeyGenerationParameters kgp = new GMSSKeyGenerationParameters(new SecureRandom(),
				new GMSSParameters(defh.length, defh, defw, defk));
		this.initialize(kgp);
