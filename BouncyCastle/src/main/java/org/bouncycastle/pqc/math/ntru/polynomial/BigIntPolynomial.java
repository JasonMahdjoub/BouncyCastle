...
public class BigIntPolynomial {
	...
	/**
	 * Constructs a <code>BigIntPolynomial</code> from a
	 * <code>IntegerPolynomial</code>. The two polynomials are independent of each
	 * other.
	 *
	 * @param p
	...
	/**
	 * Generates a random polynomial with <code>numOnes</code> coefficients equal to
	 * 1, <code>numNegOnes</code> coefficients equal to -1, and the rest equal to 0.
	 *
	 * @param N
	...
	/**
	 * Multiplies each coefficient by a <code>BigInteger</code>. Does not return a
	...
	/**
	 * Divides each coefficient by a <code>BigInteger</code> and rounds the result
	...
	/**
	 * Divides each coefficient by a <code>BigDecimal</code> and rounds the result
	 * to <code>decimalPlaces</code> places.
	 *
	 * @param divisor
	 *            the number to divide by
	 * @param decimalPlaces
	...
	public BigDecimalPolynomial div(BigDecimal divisor, int decimalPlaces) {
		...
		// factor = 1/divisor
		BigDecimal factor = Constants.BIGDEC_ONE.divide(divisor, coeffLength + decimalPlaces + 1,
				BigDecimal.ROUND_HALF_EVEN);

		// multiply each coefficient by factor
		BigDecimalPolynomial p = new BigDecimalPolynomial(coeffs.length);
		for (int i = 0; i < coeffs.length; i++)
		// multiply, then truncate after decimalPlaces so subsequent operations aren't
		// slowed down
		{
			p.coeffs[i] = new BigDecimal(coeffs[i]).multiply(factor).setScale(decimalPlaces,
					BigDecimal.ROUND_HALF_EVEN);
		}
