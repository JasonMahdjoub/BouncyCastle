...
public abstract class GF2nField {
	...
	/**
	 * holds a list of GF2nFields to which elements have been converted and thus a
	 * COB-Matrix exists
	 */
	...
	/**
	 * Computes a random root from the given irreducible fieldpolynomial according
	 * to IEEE 1363 algorithm A.5.6. This cal take very long for big degrees.
	 *
	 * @param B0FieldPolynomial
	 *            the fieldpolynomial if the other basis as a Bitstring
	 ...
	 *
	 * @param matrix
	 *            the matrix to invert as a Bitstring[]
	 * @return matrix^(-1)
	 */
	...
	/**
	 * Converts the given element in representation according to this field to a new
	 * element in representation according to B1 using the change-of-basis matrix
	 * calculated by computeCOBMatrix.
	 *
	 * @param elem
	 *            the GF2nElement to convert
	 * @param basis
	 *            the basis to convert <tt>elem</tt> to
	 * @return <tt>elem</tt> converted to a new element representation according to
	 *         <tt>basis</tt>
	 * @throws DifferentFieldsException
	 *             if <tt>elem</tt> cannot be converted according to <tt>basis</tt>.
	...
	public final GF2nElement convert(GF2nElement elem, GF2nField basis) throws RuntimeException {
		...
		if (mDegree != basis.mDegree) {
			throw new RuntimeException(
					"GF2nField.convert: B1 has a" + " different degree and thus cannot be coverted to!");
		}
