...
public class ArrayEncoder {
	 ...
	 * <p>
	 * Convert each three-bit quantity to two ternary coefficients as follows, and
	 * concatenate the resulting ternary quantities to obtain [the output].
	 * </p>
	 * <p>
	 * <code>
	 * {0, 0, 0} -> {0, 0}<br/>
	 ...
	 * {@link http://stackoverflow.com/questions/1562548/how-to-make-a-message-into-a-polynomial}
	 * <p>
	 * Convert each set of two ternary coefficients to three bits as follows, and
	 * concatenate the resulting bit quantities to obtain [the output]:
	 * </p>
	 * <p>
	 * <code>
	...
	/**
	 * Encodes an int array whose elements are between 0 and <code>q</code>, to a
	 * byte array leaving no gaps between bits.<br>
	...
	/**
	 * Decodes a <code>byte</code> array encoded with
	 * {@link #encodeModQ(int[], int)} back to an <code>int</code> array.<br>
	 * <code>N</code> is the number of coefficients. <code>q</code> must be a power
	 * of <code>2</code>.<br>
	 ...
	 * @param q
	 * @return an array containing <code>N</code> coefficients between
	 *         <code>0</code> and <code>q-1</code>
	 */
	...
	/**
	 * Decodes data encoded with {@link #encodeModQ(int[], int)} back to an
	...
	/**
	 * Decodes a <code>byte</code> array encoded with {@link #encodeMod3Sves(int[])}
	 * back to an <code>int</code> array with <code>N</code> coefficients between
	 * <code>-1</code> and <code>1</code>.<br>
	...
	/**
	 * Encodes an <code>int</code> array whose elements are between <code>-1</code>
	 * and <code>1</code>, to a byte array. <code>coeffs[2*i]</code> and
	 * <code>coeffs[2*i+1]</code> must not both equal -1 for any integer
	 * <code>i</code>, so this method is only safe to use with arrays produced by
	 * {@link #decodeMod3Sves(byte[], int)}.<br>
	...
	/**
	 * Encodes an <code>int</code> array whose elements are between <code>-1</code>
	 * and <code>1</code>, to a byte array.
	 *
	 * @return the encoded array
	 */
	...
	/**
	 * Converts a byte array produced by {@link #encodeMod3Tight(int[])} back to an
	 * <code>int</code> array.
	 *
	 * @param b
	 *            a byte array
	...
	/**
	 * Converts data produced by {@link #encodeMod3Tight(int[])} back to an
	 * <code>int</code> array.
	 *
	 * @param is
