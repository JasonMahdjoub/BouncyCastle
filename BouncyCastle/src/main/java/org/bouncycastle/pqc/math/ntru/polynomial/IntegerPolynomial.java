 ...
 * Some methods (like <code>add</code>) change the polynomial, others (like
 * <code>mult</code>) do not but return the result as a new polynomial.
 */
public class IntegerPolynomial implements Polynomial {
	private static final int NUM_EQUAL_RESULTANTS = 3;
	/**
	 * Prime numbers &gt; 4500 for resultant computation. Starting them below ~4400
	 * causes incorrect results occasionally. Fortunately, 4500 is about the optimum
	 * number for performance.<br/>
	 * This array contains enough prime numbers so primes never have to be computed
	 * on-line for any standard
	 * {@link org.bouncycastle.pqc.crypto.ntru.NTRUSigningParameters}.
	 */
	private static final int[] PRIMES = new int[] { 4507, 4513, 4517, 4519, 4523, 4547, 4549, 4561, 4567, 4583, 4591,
	...
	/**
	 * Constructs a new polynomial with <code>N</code> coefficients initialized to
	 * 0.
	 *
	 * @param N
	...
	/**
	 * Constructs a <code>IntegerPolynomial</code> from a
	 * <code>BigIntPolynomial</code>. The two polynomials are independent of each
	 * other.
	 *
	 * @param p
	...
	/**
	 * Decodes a byte array to a polynomial with <code>N</code> ternary
	 * coefficients<br>
	...
	/**
	 * Reads data produced by {@link #toBinary3Tight()} from an input stream and
	 * converts it to a polynomial.
	 *
	 * @param is
	 *            an input stream
	 * @param N
	...
	/**
	 * Returns a polynomial with N coefficients between <code>0</code> and
	 * <code>q-1</code>.<br>
	 ...
	 * <code>coeffs[2*i]</code> and <code>coeffs[2*i+1]</code> must not both equal
	 * -1 for any integer <code>i</code>, so this method is only safe to use with
	 * polynomials produced by <code>fromBinary3Sves()</code>.
	 *
	...
	/**
	 * Encodes a polynomial whose coefficients are between 0 and q, to binary. q
	...
	/**
	 * Multiplies the polynomial with another, taking the values mod modulus and the
	 * indices mod N
	 */
	...
	/**
	 * Computes the inverse mod 3. Returns <code>null</code> if the polynomial is
	 * not invertible.
	 *
	 * @return a new polynomial
	 */
	 ...
	 * algorithm.
	 * <p>
	 * Unlike EESS, this implementation does not compute all resultants modulo
	 * primes such that their product exceeds the maximum possible resultant, but
	 * rather stops when <code>NUM_EQUAL_RESULTANTS</code> consecutive modular
	 * resultants are equal.<br>
	 * This means the return value may be incorrect. Experiments show this happens
	 * in about 1 out of 100 cases when <code>N=439</code> and
	 * <code>NUM_EQUAL_RESULTANTS=2</code>, so the likelyhood of leaving the loop
	 * too early is <code>(1/100)^(NUM_EQUAL_RESULTANTS-1)</code>.
	 * <p>
	 * Because of the above, callers must verify the output and try a different
	 * polynomial if necessary.
	 *
	 * @return <code>(rho, res)</code> satisfying
	 *         <code>res = rho*this + t*(x^n-1)</code> for some integer
	 *         <code>t</code>.
	 */
	public Resultant resultant() {
		int N = coeffs.length;

		// Compute resultants modulo prime numbers. Continue until NUM_EQUAL_RESULTANTS
	 ...
	 *
	 * @return <code>(rho, res)</code> satisfying
	 *         <code>res = rho*this + t*(x^n-1) mod p</code> for some integer
	 *         <code>t</code>.
	 */
	public ModularResultant resultant(int p) {
		// Add a coefficient as the following operations involve polynomials of degree
		// deg(f)+1
	...
	/**
	 * Computes <code>this-b*c*(x^k) mod p</code> and stores the result in this
	 * polynomial.<br/>
	...
	/**
	 * Subtracts another polynomial which can have a different number of
	 * coefficients, and takes the coefficient values mod <code>modulus</code>.
	 *
	 * @param b
	 *            another polynomial
	 */
	...
	/**
	 * Subtracts a <code>int</code> from each coefficient. Does not return a new
	...
	/**
	 * Multiplies each coefficient by a 2 and applies a modulus. Does not return a
	 * new polynomial but modifies this polynomial.
	 *
	 * @param modulus
	 *            a modulus
	 */
	...
	/**
	 * Shifts all coefficients so the largest gap is centered around
	 * <code>-q/2</code>.
	 *
	 * @param q
	 *            a modulus
	 ...
	 *
	 * @return true iff all coefficients are equal to zero, except for the lowest
	 *         coefficient which must equal 1
	 */
	 ...
	 *
	 * @return true iff all coefficients are equal to zero, except for the lowest
	 *         coefficient which must equal 1 or -1
	 */
