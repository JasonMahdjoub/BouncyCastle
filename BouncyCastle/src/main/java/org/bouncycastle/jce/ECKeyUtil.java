...
public class ECKeyUtil {
	/**
	 * Convert a passed in public EC key to have explicit parameters. If the key is
	 * already using explicit parameters it is returned.
	 *
	 * @param key
	 *            key to be converted
	 * @param providerName
	 *            provider name to be used.
		...
		try {
			...
			if (info.getAlgorithmId().getAlgorithm().equals(CryptoProObjectIdentifiers.gostR3410_2001)) {
				...
				if (params.isNamedCurve()) {
					...
					curveParams = new X9ECParameters(curveParams.getCurve(), curveParams.getG(), curveParams.getN(),
							curveParams.getH());
				} else if (params.isImplicitlyCA()) {
					curveParams = new X9ECParameters(BouncyCastleProvider.CONFIGURATION.getEcImplicitlyCa().getCurve(),
							BouncyCastleProvider.CONFIGURATION.getEcImplicitlyCa().getG(),
				...
				info = new SubjectPublicKeyInfo(new AlgorithmIdentifier(X9ObjectIdentifiers.id_ecPublicKey, params),
						info.getPublicKeyData().getBytes());
			...
			throw e;
		} catch (Exception e) { // shouldn't really happen...
			throw new UnexpectedException(e);
		}
	}

	/**
	 * Convert a passed in private EC key to have explicit parameters. If the key is
	 * already using explicit parameters it is returned.
	 *
	 * @param key
	 *            key to be converted
