 ...
 *
 * Authority Key Identifier Issuer Alternative Name CRL Number Delta CRL
 * Indicator (critical) Issuing Distribution Point (critical)
 * 
 * @deprecated Do not use this class directly - either use org.bouncycastle.cert
 *             (bcpkix) or CertificateFactory.
 */
public class X509CRLObject extends X509CRL {
	...
	public X509CRLObject(CertificateList c) throws CRLException {
		...
		try {
			...
			if (c.getSignatureAlgorithm().getParameters() != null) {
				this.sigAlgParams = ((ASN1Encodable) c.getSignatureAlgorithm().getParameters()).toASN1Primitive()
	 ...
	 * currently dont handle any extensions!
	 */
		...
		while (certs.hasMoreElements()) {
			...
			if (isIndirect && entry.hasExtensions()) {
				...
				if (currentCaName != null) {
					previousCertificateIssuer = X500Name.getInstance(
