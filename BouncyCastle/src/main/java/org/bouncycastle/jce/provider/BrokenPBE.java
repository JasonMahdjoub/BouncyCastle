...
/**
 * Generator for PBE derived keys and ivs as defined by PKCS 12 V1.0, with a bug
 * affecting 180 bit plus keys - this class is only here to allow smooth
 * migration of the version 0 keystore to version 1. Don't use it (it won't be
 * staying around).
 * <p>
...
class OldPKCS12ParametersGenerator extends PBEParametersGenerator {
	...
	/**
	 * Construct a PKCS 12 Parameters generator. This constructor will accept MD5,
	 * SHA1, and RIPEMD160.
	 *
	 * @param digest
	 ...
	 * BigInteger of length (b.length * 8) bits. The result is modulo 2^b.length in
	 * case of overflow.
	 */
	...
	/**
	 * Generate a key parameter derived from the password, salt, and iteration count
	 * we are currently initialised with.
	 *
	 * @param keySize
	...
	/**
	 * Generate a key parameter for use with a MAC derived from the password, salt,
	 * and iteration count we are currently initialised with.
	 *
	 * @param keySize
	 *            the size of the key we want (in bits)
	 * @return a KeyParameter object.
...
public interface BrokenPBE {
	...
	static class Util {
		 ...
		 */
		static private void setOddParity(byte[] bytes) {
		...
		static private PBEParametersGenerator makePBEGenerator(int type, int hash) {
		 ...
		 */
		static CipherParameters makePBEParameters(BCPBEKey pbeKey, AlgorithmParameterSpec spec, int type, int hash,
		 ...
		 */
		static CipherParameters makePBEMacParameters(BCPBEKey pbeKey, AlgorithmParameterSpec spec, int type, int hash,
				int keySize) {
			if ((spec == null) || !(spec instanceof PBEParameterSpec)) {
				throw new IllegalArgumentException("Need a PBEParameter spec with a PBE key.");
			}
