...
/**
 * A parser for ASN.1 streams which also returns, where possible, parsers for
...
public class ASN1StreamParser {
	...
	ASN1Primitive readTaggedObject(boolean constructed, int tag) throws IOException {
		...
		if (_in instanceof IndefiniteLengthInputStream) {
			return v.size() == 1 ? new BERTaggedObject(true, tag, v.get(0))
					: new BERTaggedObject(false, tag, BERFactory.createSequence(v));
		}

		return v.size() == 1 ? new DERTaggedObject(true, tag, v.get(0))
