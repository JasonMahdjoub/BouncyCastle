...
/**
 * ASN.1 TaggedObject - in ASN.1 notation this is any object preceded by a [n]
 * where n is some number - these are assumed to follow the construction rules
 * (as with sequences).
 */
public abstract class ASN1TaggedObject extends ASN1Primitive implements ASN1TaggedObjectParser {
	 ...
	 * <p>
	 * If the object implements ASN1Choice the tag style will always be changed to
	 * explicit in accordance with the ASN.1 encoding rules.
	 * </p>
	 * 
	 * @param explicit
	 ...
	 * <p>
	 * Note: if the object has been read from an input stream, the only time you can
	 * be sure if isExplicit is returning the true state of affairs is if it returns
	 * false. An implicitly tagged object may appear to be explicitly tagged, so you
	 * need to understand the context under which the reading was done as well, see
	 * getObject below.
	 */
	...
	/**
	 * Return the object held in this tagged object as a parser assuming it has the
	 * type of the passed in tag. If the object doesn't have a parser associated
	 * with it, the base object is returned.
	 */
