...
public class CertificateBody extends ASN1Object {
	ASN1InputStream seq;
	private DERApplicationSpecific certificateProfileIdentifier;// version of the certificate format. Must be 0 (version
																// 1)
	private DERApplicationSpecific certificationAuthorityReference;// uniquely identifies the issuinng CA's signature
																	// key pair
	private PublicKeyDataObject publicKey;// stores the encoded public key
	private DERApplicationSpecific certificateHolderReference;// associates the public key contained in the certificate
																// with a unique name
	private CertificateHolderAuthorization certificateHolderAuthorization;// Encodes the role of the holder (i.e. CVCA,
																			// DV, IS) and assigns read/write access
																			// rights to data groups storing sensitive
																			// data
	...
	private void setIso7816CertificateBody(ASN1ApplicationSpecific appSpe) throws IOException {
		...
		while ((obj = aIS.readObject()) != null) {
			...
			if (obj instanceof DERApplicationSpecific) {
				...
				throw new IOException("Not a valid iso7816 content : not a DERApplicationSpecific Object :"
						+ EACTags.encodeTag(appSpe) + obj.getClass());
			}
	...
	public CertificateBody(DERApplicationSpecific certificateProfileIdentifier,
			CertificationAuthorityReference certificationAuthorityReference, PublicKeyDataObject publicKey,
			CertificateHolderReference certificateHolderReference,
			CertificateHolderAuthorization certificateHolderAuthorization, PackedDate certificateEffectiveDate,
			PackedDate certificateExpirationDate) {
		setCertificateProfileIdentifier(certificateProfileIdentifier);
		setCertificationAuthorityReference(new DERApplicationSpecific(EACTags.ISSUER_IDENTIFICATION_NUMBER,
				certificationAuthorityReference.getEncoded()));
		setPublicKey(publicKey);
		setCertificateHolderReference(
				new DERApplicationSpecific(EACTags.CARDHOLDER_NAME, certificateHolderReference.getEncoded()));
		setCertificateHolderAuthorization(certificateHolderAuthorization);
		try {
			setCertificateEffectiveDate(new DERApplicationSpecific(false, EACTags.APPLICATION_EFFECTIVE_DATE,
					new DEROctetString(certificateEffectiveDate.getEncoding())));
			setCertificateExpirationDate(new DERApplicationSpecific(false, EACTags.APPLICATION_EXPIRATION_DATE,
					new DEROctetString(certificateExpirationDate.getEncoding())));
		} catch (IOException e) {
	 ...
	 *
	 * @param obj
	 *            DERApplicationSpecific containing the whole body.
	 * @throws IOException
	 ...
	 * @throws IOException
	 *             if the DERApplicationSpecific cannot be created.
	 */
	...
	private void setCertificateProfileIdentifier(DERApplicationSpecific certificateProfileIdentifier)
		...
		if (certificateProfileIdentifier.getApplicationTag() == EACTags.INTERCHANGE_PROFILE) {
			...
			throw new IllegalArgumentException(
					"Not an Iso7816Tags.INTERCHANGE_PROFILE tag :" + EACTags.encodeTag(certificateProfileIdentifier));
		}
	}
	 ...
	 *
	 * @param certificationAuthorityReference
	 *            the DERApplicationSpecific containing the
	 *            CertificationAuthorityReference.
	 * @throws IllegalArgumentException
	 *             if the DERApplicationSpecific is not valid.
	 */
	...
	/**
	 * create a "request" or "profile" type Iso7816CertificateBody according to the
	 * variables sets.
	 *
	 * @return return the ASN1Primitive representing the "request" or "profile" type
	 *         certificate body.
	 */
	...
	/**
	 * gives the type of the certificate (value should be profileType or requestType
	 * if all data are set).
	 *
	 ...
	 *
	 * @param obj
	 *            is the Object to extract the certificate body from.
	 ...
	 *
	 * @param ced
	 *            DERApplicationSpecific containing the date of the certificate
	 *            generation
	 * @throws IllegalArgumentException
	 *             if the tag is not Iso7816Tags.APPLICATION_EFFECTIVE_DATE
	 */
	private void setCertificateEffectiveDate(DERApplicationSpecific ced) throws IllegalArgumentException {
		if (ced.getApplicationTag() == EACTags.APPLICATION_EFFECTIVE_DATE) {
			...
			throw new IllegalArgumentException(
					"Not an Iso7816Tags.APPLICATION_EFFECTIVE_DATE tag :" + EACTags.encodeTag(ced));
		}
	}
	 ...
	 *
	 * @param ced
	 *            DERApplicationSpecific containing the date after wich the
	 *            certificate expires
	 * @throws IllegalArgumentException
	 *             if the tag is not Iso7816Tags.APPLICATION_EXPIRATION_DATE
	 */
	...
	/**
	 * certificateHolderReference : associates the public key contained in the
	 * certificate with a unique name
	 *
	...
	/**
	 * CertificateProfileIdentifier : version of the certificate format. Must be 0
	 * (version 1)
	 *
	...
	/**
	 * get the certificationAuthorityReference certificationAuthorityReference :
	 * uniquely identifies the issuinng CA's signature key pair
	 *
