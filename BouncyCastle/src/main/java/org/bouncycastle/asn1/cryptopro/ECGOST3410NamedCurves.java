...
public class ECGOST3410NamedCurves {
	...
	static {
		...
		BigInteger mod_q = new BigInteger(
				"115792089237316195423570985008687907853073762908499243225378155805079068850323");

		ECCurve.Fp curve = new ECCurve.Fp(mod_p, // p
				new BigInteger("115792089237316195423570985008687907853269984665640564039457584007913129639316"), // a
				new BigInteger("166"), // b
				mod_q, ECConstants.ONE);

		ECDomainParameters ecParams = new ECDomainParameters(curve, curve.createPoint(new BigInteger("1"), // x
		...
		curve = new ECCurve.Fp(mod_p, // p
				new BigInteger("115792089237316195423570985008687907853269984665640564039457584007913129639316"),
				new BigInteger("166"), mod_q, ECConstants.ONE);

		ecParams = new ECDomainParameters(curve, curve.createPoint(new BigInteger("1"), // x
				new BigInteger("64033881142927202683649881450433473985931760268884941288852745803908878638612")), // y
				mod_q);
		...
		curve = new ECCurve.Fp(mod_p, // p
				...
				new BigInteger("32858"), mod_q, ECConstants.ONE);

		ecParams = new ECDomainParameters(curve,
				curve.createPoint(new BigInteger("0"),
						new BigInteger(
		...
		ecParams = new ECDomainParameters(curve, curve.createPoint(new BigInteger("0"), // x
	...
	/**
	 * return the ECDomainParameters object for the given OID, null if it isn't
	 * present.
	 *
	 * @param oid
	...
	/**
	 * returns an enumeration containing the name strings for parameters contained
	 * in this structure.
	 */
