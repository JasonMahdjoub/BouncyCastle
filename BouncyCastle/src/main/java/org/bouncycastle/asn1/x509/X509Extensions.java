...
public class X509Extensions extends ASN1Object {
	 ...
	 *
	 * the extensions are a list of constructed sequences, either with (OID,
	...
	public X509Extensions(ASN1Sequence seq) {
		...
		while (e.hasMoreElements()) {
			...
			if (s.size() == 3) {
				extensions.put(s.getObjectAt(0), new X509Extension(ASN1Boolean.getInstance(s.getObjectAt(1)),
						ASN1OctetString.getInstance(s.getObjectAt(2))));
			} else if (s.size() == 2) {
				extensions.put(s.getObjectAt(0),
