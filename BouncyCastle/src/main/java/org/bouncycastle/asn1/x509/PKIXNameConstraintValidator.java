...
public class PKIXNameConstraintValidator implements NameConstraintValidator {
	...
	public int hashCode() {
		return hashCollection(excludedSubtreesDN) + hashCollection(excludedSubtreesDNS)
				+ hashCollection(excludedSubtreesEmail) + hashCollection(excludedSubtreesIP)
				+ hashCollection(excludedSubtreesURI) + hashCollection(permittedSubtreesDN)
				+ hashCollection(permittedSubtreesDNS) + hashCollection(permittedSubtreesEmail)
				+ hashCollection(permittedSubtreesIP) + hashCollection(permittedSubtreesURI);
	}
	...
	private Set intersectDN(Set permitted, Set dns) {
		...
		for (Iterator it = dns.iterator(); it.hasNext();) {
			ASN1Sequence dn = ASN1Sequence
					.getInstance(((GeneralSubtree) it.next()).getBase().getName().toASN1Primitive());
			if (permitted == null) {
				if (dn != null) {
					intersect.add(dn);
				}
	...
	/**
	 * Returns the intersection of the permitted IP ranges in <code>permitted</code>
	 * with <code>ip</code>.
	 *
	 * @param permitted
	 ...
	 *
	 * @param excluded
	 *            A <code>Set</code> of excluded IP addresses with their subnet mask
	 *            as byte arrays.
	 * @param ip
	 *            The IP address with its subnet mask.
	 ...
	 * @param ipWithSubmask2
	 *            The second IP address with its subnet mask.
	 * @return A <code>Set</code> with the single IP address with its subnet mask as
	 *         a byte array or an empty <code>Set</code>.
	 */
	 ...
	 * @return An array with two elements. Each element contains the IP address and
	 *         the subnet mask in this order.
	 */
	...
	/**
	 * Based on the two IP addresses and their subnet masks the IP range is computed
	 * for each IP address - subnet mask pair and returned as the minimum IP address
	 * and the maximum address of the range.
	 *
	 * @param ip1
	 ...
	 * @return A array with two elements. The first/second element contains the min
	 *         and max IP address of the first/second IP address and its subnet
	 *         mask.
	 */
	...
	/**
	 * The common part of <code>email1</code> and <code>email2</code> is added to
	 * the union <code>union</code>. If <code>email1</code> and <code>email2</code>
	...
	/**
	 * The most restricting part from <code>email1</code> and <code>email2</code> is
	 * added to the intersection <code>intersect</code>.
	 *
	 * @param email1
	 *            Email address constraint 1.
	 * @param email2
	...
	/**
	 * Compares IP address <code>ip1</code> with <code>ip2</code>. If ip1 is equal
	 * to ip2 0 is returned. If ip1 is bigger 1 is returned, -1 otherwise.
	 *
	 * @param ip1
	 *            The first IP address.
	 * @param ip2
