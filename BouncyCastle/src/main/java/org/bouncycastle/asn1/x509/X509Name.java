...
public class X509Name extends ASN1Object {
	...
	/**
	 * RFC 3039 CountryOfCitizenship - PrintableString (SIZE (2)) -- ISO 3166 codes
	 * only
	 */
	public static final ASN1ObjectIdentifier COUNTRY_OF_CITIZENSHIP = new ASN1ObjectIdentifier("1.3.6.1.5.5.7.9.4");

	/**
	 * RFC 3039 CountryOfResidence - PrintableString (SIZE (2)) -- ISO 3166 codes
	 * only
	 */
	 ...
	 * <p>
	 * Note: if you're trying to be ultra orthodox, don't use this! It shouldn't be
	 * in here.
	 * 
	 * @deprecated use a X500NameStyle
	 */
	...
	/**
	 * determines whether or not strings should be processed and printed from back
	 * to front.
	 */
	...
	/**
	 * look up table translating OID values into their common symbols following the
	 * convention in RFC 2253
	 * 
	 */
	 ...
	 *
	 * the principal will be a list of constructed sets, each containing an (OID,
	 * String) pair.
	 * 
		...
		while (e.hasMoreElements()) {
			...
			for (int i = 0; i < set.size(); i++) {
				...
				if (value instanceof ASN1String && !(value instanceof DERUniversalString)) {
					...
					try {
						values.addElement(
								"#" + bytesToString(Hex.encode(value.toASN1Primitive().getEncoded(ASN1Encoding.DER))));
	 ...
	 * <p>
	 * it's is assumed the table contains OID/String pairs, and the contents of the
	 * table are copied into an internal table as part of the construction process.
	 * <p>
	 * <b>Note:</b> if the name you are trying to generate should be following a
	 * specific ordering, you should use the constructor with the ordering specified
	 * below.
	 * 
	 * @deprecated use an ordered constructor! The hashtable ordering is rarely
	 *             correct
	 */
	 ...
	 * it's is assumed the table contains OID/String pairs, and the contents of the
	 * table are copied into an internal table as part of the construction process.
	 * The ordering vector should contain the OIDs in the order they are meant to be
	 * encoded or printed in toString.
	 */
	 ...
	 * encoded or printed in toString.
	 * <p>
	 * The passed in converter will be used to convert the strings into their ASN.1
	 * counterparts.
	 * 
	...
	/**
	 * Takes an X509 dir name as a string of the format "C=AU, ST=Victoria", or some
	...
	/**
	 * Takes an X509 dir name as a string of the format "C=AU, ST=Victoria", or some
	 * such, converting it into an ordered set of name attributes with each string
	 * value being converted to its associated ASN.1 type using the passed in
	 * converter.
	 * 
	 * @deprecated use X500Name, X500NameBuilder
	 */
	...
	/**
	 * Takes an X509 dir name as a string of the format "C=AU, ST=Victoria", or some
	 * such, converting it into an ordered set of name attributes. If reverse is
	 * true, create the encoded version of the sequence starting from the last
	 ...
	 * such, converting it into an ordered set of name attributes with each string
	 * value being converted to its associated ASN.1 type using the passed in
	 * converter. If reverse is true the ASN.1 sequence representing the DN will be
	 * built by starting at the end of the string, rather than the start.
	 * 
	 * @deprecated use X500Name, X500NameBuilder
	 */
	...
	/**
	 * Takes an X509 dir name as a string of the format "C=AU, ST=Victoria", or some
	 * such, converting it into an ordered set of name attributes. lookUp should
	 * provide a table of lookups, indexed by lowercase only strings and yielding a
	 * ASN1ObjectIdentifier, other than that OID. and numeric oids will be processed
	 * automatically. <br>
	 * If reverse is true, create the encoded version of the sequence starting from
	 ...
	 * provide a table of lookups, indexed by lowercase only strings and yielding a
	 * ASN1ObjectIdentifier, other than that OID. and numeric oids will be processed
	 * automatically. The passed in converter is used to convert the string values
	 * to the right of each equals sign to their ASN.1 counterparts. <br>
	 * 
	 * @param reverse
	 ...
	 *            the string dirName
	 * @param converter
	 *            the converter to convert string values into their ASN.1
	 *            equivalents
	 */
	...
	/**
	 * return a vector of the values found in the name, in the order they were
	 * found.
	 */
	...
	/**
	 * return a vector of the values found in the name, in the order they were
	 * found, with the DN label corresponding to passed in oid.
	 */
	...
	/**
	 * @param inOrder
	 *            if true the order of both X509 names must be the same, as well as
	 *            the values associated with each element.
	 */
	...
	/**
	 * convert the structure to a string - if reverse is true the oids and values
	 * are listed out starting with the last element in the sequence (ala RFC 2253),
	 * otherwise the string will begin with the first element of the structure. If
	 * no string definition for the oid is found in oidSymbols the string value of
	 * the oid is added. Two standard symbol tables are provided DefaultSymbols, and
	...
	public String toString(boolean reverse, Hashtable oidSymbols) {
		...
		for (int i = 0; i < ordering.size(); i++) {
			if (((Boolean) added.elementAt(i)).booleanValue()) {
				ava.append('+');
				appendValue(ava, oidSymbols, (ASN1ObjectIdentifier) ordering.elementAt(i),
