...
public abstract class Primes {
	 ...
	 * {@linkplain Primes#enhancedMRProbablePrimeTest(BigInteger, SecureRandom, int)
	...
	/**
	 * Used to return the output from the
	 * {@linkplain Primes#generateSTRandomPrime(Digest, int, byte[]) Shawe-Taylor
	 * Random_Prime Routine}
	 */
	 ...
	 * @param length
	 *            the length (in bits) of the prime to be generated. Must be at
	 *            least 2.
	 * @param inputSeed
	 ...
	 * bases. This is an alternative to
	 * {@link #isMRProbablePrime(BigInteger, SecureRandom, int)} that provides more
	 * information about a composite candidate, which may be useful when generating
	 * or validating RSA moduli.
	 *
	 * @param candidate
	 ...
	 *
	 * @param candidate
	 *            the {@link BigInteger} instance to test for division by small
	 *            factors.
	 *
	 ...
	 * @param iterations
	 *            the number of randomly-chosen bases to perform the test for.
	 * @return <code>false</code> if any witness to compositeness is found amongst
	 *         the chosen bases (so <code>candidate</code> is definitely NOT prime),
	 *         or else <code>true</code> (indicating primality with some probability
	 *         dependent on the number of iterations that were performed).
	 */
	 ...
	 *
	 * Run a single iteration of the Miller-Rabin algorithm against the specified
	 * base.
	 *
	 * @param candidate
	 *            the {@link BigInteger} instance to test for primality.
	 ...
	 * @return <code>false</code> if the specified base is a witness to
	 *         compositeness (so <code>candidate</code> is definitely NOT prime), or
	 *         else <code>true</code>.
	 */
	...
	private static boolean implHasAnySmallFactors(BigInteger x) {
		/*
		 * Bundle trial divisors into ~32-bit moduli then use fast tests on the ~32-bit
		 * remainders.
		 */
		...
		/*
		 * NOTE: Unit tests depend on SMALL_FACTOR_LIMIT matching the highest small
		 * factor tested here.
		 */
		return false;
	}

	private static boolean implMRProbablePrimeToBase(BigInteger w, BigInteger wSubOne, BigInteger m, int a,
			BigInteger b) {
		BigInteger z = b.modPow(m, w);
	...
	private static STOutput implSTRandomPrime(Digest d, int length, byte[] primeSeed) {
		...
		for (;;) {
			...
			/*
			 * This is an optimization of the original algorithm, using trial division to
			 * screen out many non-primes quickly.
			 * 
