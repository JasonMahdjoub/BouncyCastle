...
public abstract class ECCurve {
	...
	/**
	 * Adds <code>PreCompInfo</code> for a point on this curve, under a given name.
	 * Used by <code>ECMultiplier</code>s to save the precomputation for this
	 * <code>ECPoint</code> for use by subsequent multiplication.
	 * 
	 * @param point
	 *            The <code>ECPoint</code> to store precomputations for.
	 * @param name
	 *            A <code>String</code> used to index precomputations of different
	 *            types.
	 * @param preCompInfo
	...
	public ECPoint importPoint(ECPoint p) {
		...
		// TODO Default behaviour could be improved if the two curves have the same
		// coordinate system by copying any Z coordinates.
		p = p.normalize();
	 ...
	 * coordinate system. Where more than one point is to be normalized, this method
	 * will generally be more efficient than normalizing each point separately.
	 * 
	 * @param points
	 ...
	 * the x, y coordinates reflect those of the equivalent point in an affine
	 * coordinate system. Where more than one point is to be normalized, this method
	 * will generally be more efficient than normalizing each point separately. An
	 * (optional) z-scaling factor can be applied; effectively each z coordinate is
	 * scaled by this value prior to normalization (but only one actual
	...
	/**
	 * Decode a point on this curve from its ASN.1 encoding. The different encodings
	 * are taken account of, including point compression for
	...
	public static abstract class AbstractF2m extends ECCurve {
		 ...
		 * <code>s<sub>1</sub></code> used for partial modular reduction for Koblitz
		 * curves.
		 */
		...
		public ECPoint createPoint(BigInteger x, BigInteger y, boolean withCompression) {
			...
			case ECCurve.COORD_LAMBDA_PROJECTIVE: {
				...
				/*
				 * NOTE: A division could be avoided using a projective result, except at
				 * present callers will expect that the result is already normalized.
				 */
	...
	public static class F2m extends AbstractF2m {
		 ...
		 * x<sup>k3</sup> + x<sup>k2</sup> + x<sup>k1</sup> + 1</code> represents the
		 * reduction polynomial <code>f(z)</code>.<br>
		 */
		 ...
		 * x<sup>k3</sup> + x<sup>k2</sup> + x<sup>k1</sup> + 1</code> represents the
		 *            reduction polynomial <code>f(z)</code>.
		 * @param k2
