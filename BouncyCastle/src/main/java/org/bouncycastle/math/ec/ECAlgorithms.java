...
public class ECAlgorithms {
	...
	public static ECPoint sumOfTwoMultiplies(ECPoint P, BigInteger a, ECPoint Q, BigInteger b) {
		...
		if (endomorphism instanceof GLVEndomorphism) {
			return validatePoint(implSumOfMultipliesGLV(new ECPoint[] { P, Q }, new BigInteger[] { a, b },
					(GLVEndomorphism) endomorphism));
		}
	...
	/*
	 * "Shamir's Trick", originally due to E. G. Straus (Addition chains of vectors.
	 * American Mathematical Monthly, 71(7):806-808, Aug./Sept. 1964) <pre> Input:
	 * The points P, Q, scalar k = (km?, ... , k1, k0) and scalar l = (lm?, ... ,
	 * l1, l0). Output: R = k * P + l * Q. 1: Z <- P + Q 2: R <- O 3: for i from m-1
	 * down to 0 do 4: R <- R + R {point doubling} 5: if (ki = 1) and (li = 0) then
	 * R <- R + P end if 6: if (ki = 0) and (li = 1) then R <- R + Q end if 7: if
	 * (ki = 1) and (li = 1) then R <- R + Z end if 8: end for 9: return R </pre>
	 */
	...
	public static void montgomeryTrick(ECFieldElement[] zs, int off, int len, ECFieldElement scale) {
		/*
		 * Uses the "Montgomery Trick" to invert many field elements, with only a single
		 * actual field inversion. See e.g. the paper:
	...
	/**
	 * Simple shift-and-add multiplication. Serves as reference implementation to
	 * verify (possibly faster) implementations, and for very small scalars.
	 * 
	 * @param p
	...
	static ECPoint implShamirsTrickJsf(ECPoint P, BigInteger k, ECPoint Q, BigInteger l) {
		...
		ECPoint[] table = new ECPoint[] { points[3].negate(), points[2].negate(), points[1].negate(),
				points[0].negate(), infinity, points[0], points[1], points[2], points[3] };
