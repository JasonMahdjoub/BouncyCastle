...
public abstract class ECPoint {
	 ...
	 * 
	 * Note: normalization can be expensive, this method is deprecated in favour of
	 ...
	 * 
	 * Caution: depending on the curve's coordinate system, this may not be the same
	 * value as in an affine coordinate system; use normalize() to get a point where
	 * the coordinates have their affine values, or use getAffineXCoord() if you
	 * expect the point to already have been normalized.
	 * 
	 ...
	 * Caution: depending on the curve's coordinate system, this may not be the same
	 * value as in an affine coordinate system; use normalize() to get a point where
	 * the coordinates have their affine values, or use getAffineYCoord() if you
	 * expect the point to already have been normalized.
	 * 
	...
	/**
	 * Normalization ensures that any projective coordinate is 1, and therefore that
	 * the x, y coordinates reflect those of the equivalent point in an affine
	 * coordinate system.
	 * 
	 * @return a new ECPoint instance representing the same point, but with
	...
	public boolean equals(ECPoint other) {
		...
		if (n1 && n2) {
			...
			// TODO Consider just requiring already normalized, to avoid silent performance
			// degradation

			ECPoint[] points = new ECPoint[] { this, c1.importPoint(p2) };

			// TODO This is a little strong, really only requires coZNormalizeAll to get Zs
			// equal
	...
	public static class Fp extends AbstractFp {
		...
		public ECPoint add(ECPoint b) {
			...
			case ECCurve.COORD_JACOBIAN_MODIFIED: {
				...
				if (coord == ECCurve.COORD_JACOBIAN_MODIFIED) {
					// TODO If the result will only be used in a subsequent addition, we don't need
					// W3
		...
		public ECPoint twicePlus(ECPoint b) {
			...
			case ECCurve.COORD_AFFINE: {
				 ...
				 * Multiplications in Elliptic Curve Cryptography", by Ciet, Joye, Lauter,
				 * Montgomery.
				 */
		...
		protected ECFieldElement doubleProductFromSquares(ECFieldElement a, ECFieldElement b, ECFieldElement aSquared,
			...
			/*
			 * NOTE: If squaring in the field is faster than multiplication, then this is a
			 * quicker way to calculate 2.A.B, if A^2 and B^2 are already known.
			 */
		...
		protected ECFieldElement getJacobianModifiedW() {
			...
			if (W == null) {
				// NOTE: Rarely, twicePlus will result in the need for a lazy W1 calculation
				// here
	...
	public static abstract class AbstractF2m extends ECPoint {
		...
		public ECPoint scaleX(ECFieldElement scale) {
			...
			case ECCurve.COORD_LAMBDA_PROJECTIVE: {
				// Y is actually Lambda (X + Y/X) here
				ECFieldElement X = this.getRawXCoord(), L = this.getRawYCoord(), Z = this.getRawZCoords()[0]; // earlier
																												// JDK
				...
				return this.getCurve().createRawPoint(X2, L2, new ECFieldElement[] { Z2 }, this.withCompression); // earlier
																													// JDK
			}
			default: {
	...
	public static class F2m extends AbstractF2m {
		...
		public ECPoint twicePlus(ECPoint b) {
			...
			case ECCurve.COORD_LAMBDA_PROJECTIVE: {
				...
				ECFieldElement A = curve.getA().add(L2plus1).multiply(Z1Sq).add(L1Sq).multiplyPlusProduct(T, X1Sq,
						Z1Sq);
