...
public class DiscoverEndomorphisms {
	...
	private static void printGLVTypeBParameters(X9ECParameters x9) {
		...
		/*
		 * The 'Beta' values are field elements of order 3. There are only two such
		 * values besides 1, each corresponding to one choice for 'Lambda'.
		 */
	...
	private static void printGLVTypeBParameters(X9ECParameters x9, BigInteger lambda, ECFieldElement[] betas) {
		/*
		 * Check the basic premise of the endomorphism: that multiplying a point by
		 * lambda preserves the y-coordinate
		 */
		...
		/*
		 * Determine which of the beta values corresponds with this choice of lambda, by
		 * checking that it scales the x-coordinate the same way a point-multiplication
		 * by lambda does.
		 */
		...
		/*
		 * Now search for parameters to allow efficient decomposition of full-length
		 * scalars
		 */
		...
		/*
		 * If elements of v2 are not bounded by sqrt(n), then if r1/t1 are relatively
		 * prime there _may_ yet be a GLV generator, so search for it. See
		 * "Integer Decomposition for Fast Scalar Multiplication on Elliptic Curves", D.
		 * Kim, S. Lim (SAC 2002)
		 */
		if (!isVectorBoundedBySqrt(v2, n) && areRelativelyPrime(v1[0], v1[1])) {
			...
			if (vw != null) {
				...
				/*
				 * We calculate the range(s) conservatively large to avoid messy rounding
				 * issues, so there may be spurious candidate generators, but we won't miss any.
				 */
				...
				if (range != null) {
					for (BigInteger alpha = range[0]; alpha.compareTo(range[1]) <= 0; alpha = alpha
							.add(ECConstants.ONE)) {
						BigInteger[] candidate = new BigInteger[] { x.add(alpha.multiply(r)),
								v.add(alpha.multiply(t)) };
		...
		/*
		 * These parameters are used to avoid division when decomposing the scalar in a
		 * GLV point multiplication. The precision is determined by 'bits', even 2 bits
		 * is enough, but we try to get more whilst keeping it 8-bit aligned and
		 * limiting the possible growth of product sizes on a 32-bit machine.
		 */
	...
	private static BigInteger[] extEuclidBezout(BigInteger[] ab) {
		...
		if (r1.signum() <= 0) {
			/*
			 * NOTE: This case occurred while testing on curves over tiny fields; probably
			 * due to a 0 input.
			 */
			return null;
		}
