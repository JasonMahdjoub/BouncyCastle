...
public class BcKeyStoreSpi extends KeyStoreSpi implements BCKeyStore {
			...
			if (type == SEALED) {
				...
				try {
					...
					try {
						...
						cipher = makePBECipher("Broken" + KEY_CIPHER, Cipher.DECRYPT_MODE, password, salt,
								iterationCount);
						...
						try {
							...
							cipher = makePBECipher("Old" + KEY_CIPHER, Cipher.DECRYPT_MODE, password, salt,
									iterationCount);
	 ...
	 * stored encrypted on disk, so the password is mandatory, however if you hard
	 * drive is in a bad part of town and you absolutely, positively, don't want
	 * nobody peeking at your things, this is the one to use, no problem! After all
	 * in a Bouncy Castle nothing can touch you.
	 *
