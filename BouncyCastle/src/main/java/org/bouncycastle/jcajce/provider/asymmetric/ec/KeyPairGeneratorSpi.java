...
public abstract class KeyPairGeneratorSpi extends java.security.KeyPairGenerator {
	...
	public static class EC extends KeyPairGeneratorSpi {
		...
		protected ECNamedCurveSpec createNamedCurveSpec(String curveName) throws InvalidAlgorithmParameterException {
			// NOTE: Don't bother with custom curves here as the curve will be converted to
			// JCE type shortly

			X9ECParameters p = ECUtils.getDomainParametersFromName(curveName);
			if (p == null) {
				try {
					// Check whether it's actually an OID string (SunJSSE ServerHandshaker
					// setupEphemeralECDHKeys bug)
			...
			// Work-around for JDK bug -- it won't look up named curves properly if seed is
			// present
