...
public class BaseBlockCipher extends BaseWrapCipher implements PBE {
	...
	// specs we can handle.
	//
	private Class[] availableSpecs = { RC2ParameterSpec.class, RC5ParameterSpec.class, gcmSpecClass,
			IvParameterSpec.class, PBEParameterSpec.class, GOST28147ParameterSpec.class };
	...
	protected BaseBlockCipher(org.bouncycastle.crypto.BlockCipher engine, int ivLength) {
		baseEngine = engine;
		...
		if (engineParams == null) {
			if (pbeSpec != null) {
				...
				try {
					...
					engineParams
							.init(new GCMParameters(aeadParams.getNonce(), aeadParams.getMacSize() / 8).getEncoded());
				} catch (Exception e) {
	...
	protected void engineInit(int opmode, Key key, AlgorithmParameterSpec params, SecureRandom random)
		...
		if (!(key instanceof SecretKey)) {
			throw new InvalidKeyException("Key for algorithm " + ((key != null) ? key.getAlgorithm() : null)
		...
		if (scheme == PKCS12 || key instanceof PKCS12Key) {
			...
			if (key instanceof BCPBEKey) {
				// PKCS#12 sets an IV, if we get a key that doesn't have ParametersWithIV we
				// need to reject it. If the
				// key has no parameters it means it's an old-school JCE PBE Key - we use
				// getEncoded() on it.
				CipherParameters pbeKeyParam = ((BCPBEKey) key).getParam();
				if (pbeKeyParam instanceof ParametersWithIV) {
					param = pbeKeyParam;
				} else if (pbeKeyParam == null) {
					param = PBE.Util.makePBEParameters(k.getEncoded(), PKCS12, digest, keySizeInBits, ivLength * 8,
							pbeSpec, cipher.getAlgorithmName());
				} else {
					throw new InvalidKeyException("Algorithm requires a PBE key suitable for PKCS12");
				}
			} else {
				param = PBE.Util.makePBEParameters(k.getEncoded(), PKCS12, digest, keySizeInBits, ivLength * 8, pbeSpec,
			...
			if (k instanceof PBKDF1KeyWithParameters && pbeSpec == null) {
				pbeSpec = new PBEParameterSpec(((PBKDF1KeyWithParameters) k).getSalt(),
						((PBKDF1KeyWithParameters) k).getIterationCount());
			}

			param = PBE.Util.makePBEParameters(k.getEncoded(), PKCS5S1, digest, keySizeInBits, ivLength * 8, pbeSpec,
					cipher.getAlgorithmName());
			if (param instanceof ParametersWithIV) {
				ivParam = (ParametersWithIV) param;
			}
			...
			param = PBE.Util.makePBEParameters(k.getEncoded(), scheme, digest, keySizeInBits, ivLength * 8, pbeSpec,
					cipher.getAlgorithmName());
			if (param instanceof ParametersWithIV) {
				ivParam = (ParametersWithIV) param;
			}
		...
		if (params instanceof AEADParameterSpec) {
			...
			param = aeadParams = new AEADParameters(keyParam, aeadSpec.getMacSizeInBits(), aeadSpec.getNonce(),
					aeadSpec.getAssociatedData());
			...
			param = new ParametersWithSBox(new KeyParameter(key.getEncoded()),
					((GOST28147ParameterSpec) params).getSbox());
				...
				if (baseEngine.getAlgorithmName().equals("RC5-32")) {
					if (rc5Param.getWordSize() != 32) {
						throw new InvalidAlgorithmParameterException(
								"RC5 already set up for a word size of 32 not " + rc5Param.getWordSize() + ".");
					}
				} else if (baseEngine.getAlgorithmName().equals("RC5-64")) {
					if (rc5Param.getWordSize() != 64) {
						throw new InvalidAlgorithmParameterException(
								"RC5 already set up for a word size of 64 not " + rc5Param.getWordSize() + ".");
					}
				}
			} else {
			...
			try {
				...
				param = aeadParams = new AEADParameters(keyParam,
		...
		try {
			...
			if (cipher instanceof AEADGenericBlockCipher && aeadParams == null) {
				...
				aeadParams = new AEADParameters((KeyParameter) ivParam.getParameters(), aeadCipher.getMac().length * 8,
						ivParam.getIV());
			}
