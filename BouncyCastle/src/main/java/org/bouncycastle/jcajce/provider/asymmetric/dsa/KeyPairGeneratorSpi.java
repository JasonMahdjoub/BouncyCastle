...
public class KeyPairGeneratorSpi extends java.security.KeyPairGenerator {
	...
	public void initialize(int strength, SecureRandom random) {
		if (strength < 512 || strength > 4096 || ((strength < 1024) && strength % 64 != 0)
			...
			if (params.containsKey(paramStrength)) {
				...
				synchronized (lock) {
					...
					if (params.containsKey(paramStrength)) {
						...
						// For simplicity if keysize is greater than 1024 then we choose q's size to be
						// 256.
						// For legacy keysize that is less than 1024-bit, we just use the 186-2 style
						// parameters
						if (strength == 1024) {
