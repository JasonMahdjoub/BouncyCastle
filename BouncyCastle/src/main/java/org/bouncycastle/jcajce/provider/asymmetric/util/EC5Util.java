...
public class EC5Util {
	...
	static {
		...
		customCurves.put(
				new ECCurve.Fp(c25519.getCurve().getField().getCharacteristic(),
						c25519.getCurve().getA().toBigInteger(), c25519.getCurve().getB().toBigInteger()),
				c25519.getCurve());
	}
	...
	public static ECDomainParameters getDomainParameters(ProviderConfiguration configuration,
		...
		if (params == null) {
			...
			domainParameters = new ECDomainParameters(iSpec.getCurve(), iSpec.getG(), iSpec.getN(), iSpec.getH(),
					iSpec.getSeed());
		} else {
	...
	public static ECParameterSpec convertToSpec(X962Parameters params, ECCurve curve) {
		...
		if (params.isNamedCurve()) {
			...
			if (ecP.getH() != null) {
				ecSpec = new ECParameterSpec(ellipticCurve, new ECPoint(ecP.getG().getAffineXCoord().toBigInteger(),
						ecP.getG().getAffineYCoord().toBigInteger()), ecP.getN(), ecP.getH().intValue());
			} else {
				ecSpec = new ECParameterSpec(ellipticCurve, new ECPoint(ecP.getG().getAffineXCoord().toBigInteger(),
						ecP.getG().getAffineYCoord().toBigInteger()), ecP.getN(), 1); // TODO: not strictly correct...
																						// need to fix the test data...
			}
		}
	...
	public static ECParameterSpec convertToSpec(X9ECParameters domainParameters) {
		return new ECParameterSpec(convertCurve(domainParameters.getCurve(), null), // JDK 1.5 has trouble with this if
																					// it's not null...
				new ECPoint(domainParameters.getG().getAffineXCoord().toBigInteger(),
						domainParameters.getG().getAffineYCoord().toBigInteger()),
				domainParameters.getN(), domainParameters.getH().intValue());
	}
	...
	public static EllipticCurve convertCurve(ECCurve curve, byte[] seed) {
		...
		// so at the moment it's set to null. Should probably look at making this
		// configurable
	...
	public static ECParameterSpec convertSpec(EllipticCurve ellipticCurve,
		...
		if (spec instanceof ECNamedCurveParameterSpec) {
			return new ECNamedCurveSpec(((ECNamedCurveParameterSpec) spec).getName(), ellipticCurve,
					new ECPoint(spec.getG().getAffineXCoord().toBigInteger(),
							spec.getG().getAffineYCoord().toBigInteger()),
					spec.getN(), spec.getH());
		} else {
			return new ECParameterSpec(ellipticCurve, new ECPoint(spec.getG().getAffineXCoord().toBigInteger(),
	...
	public static org.bouncycastle.jce.spec.ECParameterSpec convertSpec(ECParameterSpec ecSpec,
		...
		return new org.bouncycastle.jce.spec.ECParameterSpec(curve,
				convertPoint(curve, ecSpec.getGenerator(), withCompression), ecSpec.getOrder(),
