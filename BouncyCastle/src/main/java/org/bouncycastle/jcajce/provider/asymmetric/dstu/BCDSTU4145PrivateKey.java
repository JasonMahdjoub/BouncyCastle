...
public class BCDSTU4145PrivateKey implements ECPrivateKey, org.bouncycastle.jce.interfaces.ECPrivateKey,
	...
	public BCDSTU4145PrivateKey(String algorithm, ECPrivateKeyParameters params, BCDSTU4145PublicKey pubKey,
	...
	private void populateFromPrivKeyInfo(PrivateKeyInfo info) throws IOException {
		...
		if (params.isNamedCurve()) {
			...
			if (ecP == null) // DSTU Curve
			{
				...
				ecSpec = new ECNamedCurveSpec(oid.getId(), ellipticCurve,
			...
			this.ecSpec = new ECParameterSpec(ellipticCurve, new ECPoint(ecP.getG().getAffineXCoord().toBigInteger(),
		...
		try {
			if (algorithm.equals("DSTU4145")) {
				info = new PrivateKeyInfo(
						new AlgorithmIdentifier(UAObjectIdentifiers.dstu4145be, params.toASN1Primitive()),
