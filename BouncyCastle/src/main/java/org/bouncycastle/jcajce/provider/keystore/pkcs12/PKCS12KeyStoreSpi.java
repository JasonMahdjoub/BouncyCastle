...
public class PKCS12KeyStoreSpi extends KeyStoreSpi implements PKCSObjectIdentifiers, X509ObjectIdentifiers, BCKeyStore {
	...
	/**
	 * this is not quite complete - we should follow up on the chain, a bit tricky
	 * if a certificate appears in more than one chain... the store method now
	 * prunes out unused certificates from the chain map if they are present.
	 */
	...
	protected byte[] cryptData(boolean forEncryption, AlgorithmIdentifier algId, char[] password,
			boolean wrongPKCS12Zero, byte[] data) throws IOException {
		ASN1ObjectIdentifier algorithm = algId.getAlgorithm();
	...
	private Cipher createCipher(int mode, char[] password, AlgorithmIdentifier algId)
		...
		if (func.isDefaultPrf()) {
			key = keyFact.generateSecret(new PBEKeySpec(password, func.getSalt(), func.getIterationCount().intValue(),
					keySizeProvider.getKeySize(encScheme)));
		} else {
			key = keyFact.generateSecret(new PBKDF2KeySpec(password, func.getSalt(),
					func.getIterationCount().intValue(), keySizeProvider.getKeySize(encScheme), func.getPrf()));
		}
				...
				if (c[i].getContentType().equals(data)) {
					...
					for (int j = 0; j != seq.size(); j++) {
						...
						if (b.getBagId().equals(pkcs8ShroudedKeyBag)) {
							...
							PrivateKey privKey = unwrapKey(eIn.getEncryptionAlgorithm(), eIn.getEncryptedData(),
									password, wrongPKCS12Zero);
					...
					byte[] octets = cryptData(false, d.getEncryptionAlgorithm(), password, wrongPKCS12Zero,
							d.getContent().getOctets());
					...
					for (int j = 0; j != seq.size(); j++) {
						...
						if (b.getBagId().equals(certBag)) {
							...
							org.bouncycastle.asn1.pkcs.PrivateKeyInfo kInfo = org.bouncycastle.asn1.pkcs.PrivateKeyInfo
									.getInstance(b.getBagValue());
	...
	private void doStore(OutputStream stream, char[] password, boolean useDEREncoding) throws IOException {
		...
		while (ks.hasMoreElements()) {
			...
			org.bouncycastle.asn1.pkcs.EncryptedPrivateKeyInfo kInfo = new org.bouncycastle.asn1.pkcs.EncryptedPrivateKeyInfo(
					kAlgId, kBytes);
	...
	private byte[] calculatePbeMac(ASN1ObjectIdentifier oid, byte[] salt, int itCount, char[] password,
			boolean wrongPkcs12Zero, byte[] data) throws Exception {
