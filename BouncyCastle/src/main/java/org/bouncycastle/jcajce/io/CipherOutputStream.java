 ...
 * write() methods process the written data with the cipher, and the output of
 * the cipher is in turn written to the underlying OutputStream. The cipher must
 * be fully initialized before being used by a CipherInputStream.
 * <p>
 * For example, if the cipher is initialized for encryption, the
 * CipherOutputStream will encrypt the data before writing the encrypted data to
 * the underlying stream.
 * </p>
 * <p>
 * This is a reimplementation of {@link javax.crypto.CipherOutputStream} that is
...
public class CipherOutputStream extends FilterOutputStream {
	 ...
	 * cipher object, which causes any bytes buffered by the encapsulated cipher to
	 * be processed. The result is written out by calling the <code>flush</code>
	 ...
	 *             if the data written to this stream was invalid ciphertext (e.g.
	 *             the cipher is an AEAD cipher and the ciphertext tag check fails).
	 */
