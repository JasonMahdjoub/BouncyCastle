...
/**
 * Implements the Galois/Counter mode (GCM) detailed in NIST Special Publication
 * 800-38D.
 */
public class GCMBlockCipher implements AEADBlockCipher {
	...
	/**
	 * NOTE: MAC sizes from 32 bits to 128 bits (must be a multiple of 8) are
	 * supported. The default is 128 bits. Sizes less than 96 are not recommended,
	 * but are supported for specialized applications.
	 */
	public void init(boolean forEncryption, CipherParameters params) throws IllegalArgumentException {
		...
		if (keyParam != null) {
			...
			// GCMMultiplier tables don't change unless the key changes (and are expensive
			// to init)
			multiplier.init(H);
			exp = null;
		} else if (this.H == null) {
	...
	public int doFinal(byte[] out, int outOff) throws IllegalStateException, InvalidCipherTextException {
		...
		if (atLength > atLengthPre) {
			 ...
			 * the hash value we actually used when the cipher started (S_atPre) and the
			 * final hash value calculated (S_at). Then we carry this difference forward by
			 * multiplying by H^c, where c is the number of (full or partial) cipher-text
			 * blocks produced, and adjust the current hash.
			 */
