...
/**
 * Implements OpenPGP's rather strange version of Cipher-FeedBack (CFB) mode on
 * top of a simple cipher. For further info see
 * <a href="http://www.ietf.org/rfc/rfc2440.html">RFC 2440</a>.
 */
public class PGPCFBBlockCipher implements BlockCipher {
	 ...
	 *
	 * @return the name of the underlying algorithm followed by "/PGPCFB" and the
	 *         block size in bits.
	 */
	...
	public int processBlock(byte[] in, int inOff, byte[] out, int outOff)
		...
		if (inlineIv) {
			return (forEncryption) ? encryptBlockWithIV(in, inOff, out, outOff)
					: decryptBlockWithIV(in, inOff, out, outOff);
		} else {
	 ...
	 *            the byte to encrypt
	 * @param blockOff
	 *            where am i in the current block, determines when to resync the
	 *            block
