...
public class SICBlockCipher extends StreamBlockCipher implements SkippingStreamCipher {
		...
		if (params instanceof ParametersWithIV) {
			...
			if (blockSize < IV.length) {
				throw new IllegalArgumentException(
						"CTR/SIC mode requires IV no greater than: " + blockSize + " bytes.");
			}
			...
			if (blockSize - IV.length > maxCounterSize) {
				throw new IllegalArgumentException(
						"CTR/SIC mode requires IV of at least: " + (blockSize - maxCounterSize) + " bytes.");
			}
	...
	private void checkCounter() {
		// if the IV is the same as the blocksize we assume the user knows what they are
		// doing
