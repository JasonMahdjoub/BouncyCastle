...
/**
 * this does your basic PKCS 1 v1.5 padding - whether or not you should be using
 * this depends on your application - see PKCS1 Version 2 for details.
 */
public class PKCS1Encoding implements AsymmetricBlockCipher {
	...
	/**
	 * some providers fail to include the leading zero in PKCS1 encoded blocks. If
	 * you need to work with one of these set the system property
	 * org.bouncycastle.pkcs1.not_strict to true.
	 * <p>
	 * The system property is checked during construction of the encoding object, it
	...
	/**
	 * Constructor for decryption with a fixed plaintext length and a fallback value
	 * that is returned, if the padding is incorrect.
	 *
	 * @param cipher
	 *            The cipher to use for cryptographic operation.
	 * @param fallback
	...
	/**
	 * Checks if the argument is a correctly PKCS#1.5 encoded Plaintext for
	 * encryption.
	 *
	 * @param encoded
	 *            The Plaintext.
	...
	/**
	 * Decode PKCS#1.5 encoding, and return a random value if the padding is not
	 * correct.
	 *
	 * @param in
	 *            The encrypted block.
	 * @param inOff
	 *            Offset in the encrypted block.
	 * @param inLen
	 *            Length of the encrypted block. //@param pLen Length of the desired
	 *            output.
	 * @return The plaintext without padding, or a random value if the padding was
	 *         incorrect.
	...
	private byte[] decodeBlockOrRandom(byte[] in, int inOff, int inLen) throws InvalidCipherTextException {
		...
		/*
		 * Now, to a constant time constant memory copy of the decrypted value or the
		 * random value, depending on the validity of the padding.
		 */
	...
	private byte[] decodeBlock(byte[] in, int inOff, int inLen) throws InvalidCipherTextException {
		/*
		 * If the length of the expected plaintext is known, we use a constant-time
		 * decryption. If the decryption fails, we return a random value.
		 */
		if (this.pLen != -1) {
