...
public class CipherInputStream extends FilterInputStream {
	 ...
	 * and a specified internal buffer size.
	 */
	...
	/**
	 * Constructs a CipherInputStream from an InputStream, a StreamCipher, and a
	 * specified internal buffer size.
	 */
	...
	/**
	 * Constructs a CipherInputStream from an InputStream, an AEADBlockCipher, and a
	 * specified internal buffer size.
	 */
	...
	/**
	 * Read data from underlying stream and process with cipher until end of stream
	 * or some data is available after cipher processing.
	 *
	...
	/**
	 * Reads data from the underlying stream and processes it with the cipher until
	 * the cipher outputs data, and returns the next available byte.
	 * <p>
	 * If the underlying stream is exhausted by this call, the cipher will be
	 * finalised.
	 * </p>
	 * 
	 * @throws IOException
	 *             if there was an error closing the input stream.
	 * @throws InvalidCipherTextIOException
	 *             if the data read from the stream was invalid ciphertext (e.g. the
	 *             cipher is an AEAD cipher and the ciphertext tag check fails).
	 */
	...
	/**
	 * Reads data from the underlying stream and processes it with the cipher until
	 * the cipher outputs data, and then returns up to <code>b.length</code> bytes
	...
	/**
	 * Reads data from the underlying stream and processes it with the cipher until
	 * the cipher outputs data, and then returns up to <code>len</code> bytes in the
	 * provided array.
	 * <p>
	 * If the underlying stream is exhausted by this call, the cipher will be
	 * finalised.
	...
	/**
	 * Ensure the cipher text buffer has space sufficient to accept an upcoming
	 * output.
	 *
	 * @param updateSize
	 ...
	 * <p>
	 * This method only works if markSupported() returns true - which means the
	 * underlying stream supports marking, and the cipher passed in to this stream's
	 * constructor is a SkippingCipher (so capable of being reset to an arbitrary
	 * point easily).
	 * </p>
	 * 
	 * @param readlimit
	 ...
	 *
	 * @throws IOException
	 *             if marking not supported by the cipher used, or the underlying
	 *             stream.
	 */
	...
	/**
	 * Return true if mark(readlimit) is supported. This will be true if the
	 * underlying stream supports marking and the cipher used is a SkippingCipher,
	 *
