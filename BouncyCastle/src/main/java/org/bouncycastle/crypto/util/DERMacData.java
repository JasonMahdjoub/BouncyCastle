...
/**
 * Builder and holder class for preparing SP 800-56A compliant MacData. Elements
 * in the data are encoded as DER objects with empty octet strings used to
 * represent nulls in compulsory fields.
 */
public final class DERMacData {
	public enum Type {
		UNILATERALU("KC_1_U"), UNILATERALV("KC_1_V"), BILATERALU("KC_2_U"), BILATERALV("KC_2_V");
	...
	public static final class Builder {
		 ...
		 *
		 * @param text
		 *            optional agreed text to add to the MAC.
		...
		public DERMacData build() {
			...
			case UNILATERALU:
			case BILATERALU:
				return new DERMacData(concatenate(type.getHeader(), DerUtil.toByteArray(idU), DerUtil.toByteArray(idV),
						DerUtil.toByteArray(ephemDataU), DerUtil.toByteArray(ephemDataV), text));
			case UNILATERALV:
			case BILATERALV:
				return new DERMacData(concatenate(type.getHeader(), DerUtil.toByteArray(idV), DerUtil.toByteArray(idU),
