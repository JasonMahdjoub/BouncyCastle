...
class DTLSReliableHandshake {
						...
						if (seq == next_receive_seq) {
							...
							if (body != null) {
								...
								return updateHandshakeMessagesDigest(
										new Message(next_receive_seq++, reassembler.getMsgType(), body));
							}
						}
					}
				}
	...
	void finish() {
		...
		if (!sending) {
			...
			/*
			 * RFC 6347 4.2.4. In addition, for at least twice the default MSL defined for
			 * [TCP], when in the FINISHED state, the node that transmits the last flight
			 * (the server in an ordinary handshake or the client in a resumed handshake)
			 * MUST respond to a retransmit of the peer's last flight with a retransmit of
			 * the last flight.
			 */
