...
class DTLSRecordLayer implements DatagramTransport {
	...
	public int receive(byte[] buf, int off, int len, int waitMillis) throws IOException {
		...
		for (;;) {
			...
			try {
				...
				/*
				 * NOTE: If we receive any non-handshake data in the new epoch implies the peer
				 * has received our final flight.
				 */
	...
	private void closeTransport() {
		if (!closed) {
			/*
			 * RFC 5246 7.2.1. Unless some other fatal alert has been transmitted, each
			 * party is required to send a close_notify alert before closing the write side
			 * of the connection. The other party MUST respond with a close_notify alert of
			 * its own and close down the connection immediately, discarding any pending
			 * writes.
			 */
	...
	private void sendRecord(short contentType, byte[] buf, int off, int len) throws IOException {
		...
		byte[] ciphertext = writeEpoch.getCipher()
				.encodePlaintext(getMacSequenceNumber(recordEpoch, recordSequenceNumber), contentType, buf, off, len);
