...
/**
 * A generic TLS MAC implementation, acting as an HMAC based on some underlying
 * Digest.
 */
public class TlsMac {
	...
	public TlsMac(TlsContext context, Digest digest, byte[] key, int keyOff, int keyLen) {
		...
		if (TlsUtils.isSSL(context)) {
			...
			// TODO This should check the actual algorithm, not assume based on the digest
			// size
	...
	public byte[] calculateMacConstantTime(long seqNo, short type, byte[] message, int offset, int length,
		...
		/*
		 * ...but ensure a constant number of complete digest blocks are processed (as
		 * many as would be needed for 'fullLength' bytes of input).
		 */
