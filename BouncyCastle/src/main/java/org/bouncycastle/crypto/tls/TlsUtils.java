...
public class TlsUtils {
	...
	public static ASN1Primitive readDERObject(byte[] encoding) throws IOException {
		 ...
		 * since DER is canonical, we can check it by re-encoding the result and
	...
	public static SignatureAndHashAlgorithm getSignatureAndHashAlgorithm(TlsContext context,
			TlsSignerCredentials signerCredentials) throws IOException {
	 ...
	 *
	 * @param extensions
	 *            A {@link Hashtable} to add the extension to.
	 * @param supportedSignatureAlgorithms
	 *            {@link Vector} containing at least 1
	 *            {@link SignatureAndHashAlgorithm}.
	 * @throws IOException
	 */
	 ...
	 *
	 * @param extensions
	 *            A {@link Hashtable} to get the extension from, if it is present.
	 * @return A {@link Vector} containing at least 1
	 *         {@link SignatureAndHashAlgorithm}, or null.
	 * @throws IOException
	 */
	 ...
	 *
	 * @param supportedSignatureAlgorithms
	 *            A {@link Vector} containing at least 1
	 *            {@link SignatureAndHashAlgorithm}.
	...
	public static void encodeSupportedSignatureAlgorithms(Vector supportedSignatureAlgorithms, boolean allowAnonymous,
		...
		for (int i = 0; i < supportedSignatureAlgorithms.size(); ++i) {
			...
			if (!allowAnonymous && entry.getSignature() == SignatureAlgorithm.anonymous) {
				/*
				 * RFC 5246 7.4.1.4.1 The "anonymous" value is meaningless in this context but
				 * used in Section 7.4.3. It MUST NOT appear in this extension.
				 */
	...
	public static void verifySupportedSignatureAlgorithm(Vector supportedSignatureAlgorithms,
		...
		if (signatureAlgorithm.getSignature() != SignatureAlgorithm.anonymous) {
			for (int i = 0; i < supportedSignatureAlgorithms.size(); ++i) {
				SignatureAndHashAlgorithm entry = (SignatureAndHashAlgorithm) supportedSignatureAlgorithms.elementAt(i);
				if (entry.getHash() == signatureAlgorithm.getHash()
						&& entry.getSignature() == signatureAlgorithm.getSignature()) {
					return;
				}
			}
		}
	...
	static byte[] calculateMasterSecret(TlsContext context, byte[] pre_master_secret) {
		...
		String asciiLabel = securityParameters.extendedMasterSecret ? ExporterLabel.extended_master_secret
	...
	static short getClientCertificateType(Certificate clientCertificate, Certificate serverCertificate)
		...
		try {
			...
			/*
			 * TODO RFC 5246 7.4.6. The certificates MUST be signed using an acceptable
			 * hash/ signature algorithm pair, as described in Section 7.4.4. Note that this
			 * relaxes the constraints on certificate-signing algorithms found in prior
			 * versions of TLS.
			 */
			...
			/*
			 * RSA public key; the certificate MUST allow the key to be used for signing
			 * with the signature scheme and hash algorithm that will be employed in the
			...
			/*
			 * DSA public key; the certificate MUST allow the key to be used for signing
			 * with the hash algorithm that will be employed in the certificate verify
			 * message.
			 */
			...
			/*
			 * ECDSA-capable public key; the certificate MUST allow the key to be used for
			 * signing with the hash algorithm that will be employed in the certificate
			 * verify message; the public key MUST use a curve and point format supported by
			 * the server.
			 */
	...
	static void trackHashAlgorithms(TlsHandshakeHash handshakeHash, Vector supportedSignatureAlgorithms) {
		if (supportedSignatureAlgorithms != null) {
			for (int i = 0; i < supportedSignatureAlgorithms.size(); ++i) {
				SignatureAndHashAlgorithm signatureAndHashAlgorithm = (SignatureAndHashAlgorithm) supportedSignatureAlgorithms
						.elementAt(i);
