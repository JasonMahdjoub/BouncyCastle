...
abstract class AbstractTlsContext implements TlsContext {
	...
	public byte[] exportKeyingMaterial(String asciiLabel, byte[] context_value, int length) {
		 ...
		 * TODO[session-hash]
		 * 
		 * draft-ietf-tls-session-hash-04 5.4. If a client or server chooses to continue
		 * with a full handshake without the extended master secret extension, [..] the
		 * client or server MUST NOT export any key material based on the new master
		 * secret for any subsequent application-level authentication. In particular, it
		 * MUST disable [RFC5705] [..].
		 */
