...
public abstract class AbstractTlsClient extends AbstractTlsPeer implements TlsClient {
	...
	protected boolean allowUnexpectedServerExtension(Integer extensionType, byte[] extensionData) throws IOException {
		...
		case ExtensionType.elliptic_curves:
			/*
			 * Exception added based on field reports that some servers do send this,
			 * although the Supported Elliptic Curves Extension is clearly intended to be
			 * client-only. If present, we still require that it is a valid
			 * EllipticCurveList.
			 */
	...
	public boolean isFallback() {
		/*
		 * RFC 7507 4. The TLS_FALLBACK_SCSV cipher suite value is meant for use by
		 * clients that repeat a connection attempt with a downgraded protocol (perform
		 * a "fallback retry") in order to work around interoperability problems with
		 * legacy servers.
		 */
		return false;
	}
	...
	public Hashtable getClientExtensions() throws IOException {
		...
		if (TlsUtils.isSignatureAlgorithmsExtensionAllowed(clientVersion)) {
			// TODO Provide a way for the user to specify the acceptable hash/signature
			// algorithms.
		...
		if (TlsECCUtils.containsECCCipherSuites(getCipherSuites())) {
			/*
			 * RFC 4492 5.1. A client that proposes ECC cipher suites in its ClientHello
			 * message appends these extensions (along with any others), enumerating the
			 * curves it supports and the point formats it can parse. Clients SHOULD send
			 * both the Supported Elliptic Curves Extension and the Supported Point Formats
			 * Extension.
			 */
			/*
			 * TODO Could just add all the curves since we support them all, but users may
			 * not want to use unnecessarily large fields. Need configuration options.
			 */
	...
	public void processServerExtensions(Hashtable serverExtensions) throws IOException {
		/*
		 * TlsProtocol implementation validates that any server extensions received
		 * correspond to client extensions sent. By default, we don't send any, and this
	...
	public TlsCompression getCompression() throws IOException {
		...
		default:
			 ...
			 * server-selected compression method was in the list of client-offered
			 * compression methods, so if we now can't produce an implementation, we
