...
public class TlsECDHKeyExchange extends AbstractTlsKeyExchange {
	...
	public void processServerKeyExchange(InputStream input) throws IOException {
		...
		this.ecAgreePublicKey = TlsECCUtils
				.validateECPublicKey(TlsECCUtils.deserializeECPublicKey(clientECPointFormats, curve_params, point));
	}

	public void validateCertificateRequest(CertificateRequest certificateRequest) throws IOException {
		/*
		 * RFC 4492 3. [...] The ECDSA_fixed_ECDH and RSA_fixed_ECDH mechanisms are
		 * usable with ECDH_ECDSA and ECDH_RSA. Their use with ECDHE_ECDSA and ECDHE_RSA
		 * is prohibited because the use of a long-term ECDH client key would jeopardize
	...
	public void processClientCredentials(TlsCredentials clientCredentials) throws IOException {
		...
		if (clientCredentials instanceof TlsAgreementCredentials) {
			// TODO Validate client cert has matching parameters (see
			// 'TlsECCUtils.areOnSameCurve')?
	...
	public void processClientCertificate(Certificate clientCertificate) throws IOException {
		...
		// TODO If the certificate is 'fixed', take the public key as
		// ecAgreeClientPublicKey
	}

	public void processClientKeyExchange(InputStream input) throws IOException {
		if (ecAgreePublicKey != null) {
			// For ecdsa_fixed_ecdh and rsa_fixed_ecdh, the key arrived in the client
			// certificate
			return;
		}
		...
		this.ecAgreePublicKey = TlsECCUtils
				.validateECPublicKey(TlsECCUtils.deserializeECPublicKey(serverECPointFormats, curve_params, point));
	}
