...
public class AlertDescription {
	 ...
	 * messages on this connection. Note that as of TLS 1.1, failure to properly
	 * close a connection no longer requires that a session not be resumed. This is
	 * a change from TLS 1.0 ("The session becomes unresumable if any connection is
	 * terminated without proper close_notify messages with level equal to
	 * warning.") to conform with widespread implementation practice.
	 */
	 ...
	 * alert also MUST be returned if an alert is sent because a TLSCiphertext
	 * decrypted in an invalid way: either it wasn't an even multiple of the block
	 * length, or its padding values, when checked, weren't correct. This message is
	 * always fatal and should never be observed in communication between proper
	 * implementations (except when messages were corrupted in the network).
	 */
	public static final short bad_record_mac = 20;

	/**
	 * This alert was used in some earlier versions of TLS, and may have permitted
	 * certain attacks against the CBC mode [CBCATT]. It MUST NOT be sent by
	 ...
	 * 2^14+1024 bytes. This message is always fatal and should never be observed in
	 * communication between proper implementations (except when messages were
	 ...
	 * expand to excessive length). This message is always fatal and should never be
	...
	/**
	 * This alert was used in SSLv3 but not any version of TLS. It MUST NOT be sent
	 ...
	 * sender decided not to proceed with negotiation. This message is always fatal.
	 */
	 ...
	 * correctly verify a signature or validate a Finished message. This message is
	 * always fatal.
	 */
	public static final short decrypt_error = 51;

	/**
	 * This alert was used in some earlier versions of TLS. It MUST NOT be sent by
	 * compliant implementations.
	 */
	public static final short export_restriction = 60;

	/**
	 * The protocol version the client has attempted to negotiate is recognized but
	 * not supported. (For example, old protocol versions might be avoided for
	 ...
	 * (such as a memory allocation failure) makes it impossible to continue. This
	 ...
	 * appropriate is where a server has spawned a process to satisfy a request; the
	 * process might receive security parameters (key length, authentication, etc.)
	 * at startup, and it might be difficult to communicate changes to these
	 * parameters after that point. This message is always a warning.
	 */
	public static final short no_renegotiation = 100;

	/**
	 * Sent by clients that receive an extended server hello containing an extension
	 * that they did not put in the corresponding client hello. This message is
	 * always fatal.
	 */
	...
	/**
	 * This alert is sent by servers who are unable to retrieve a certificate chain
	 * from the URL supplied by the client (see Section 3.3). This message MAY be
	 * fatal - for example if client authentication is required by the server for
	...
	/**
	 * This alert is sent by servers that receive a server_name extension request,
	 * but do not recognize the server name. This message MAY be fatal.
	 */
	public static final short unrecognized_name = 112;

	/**
	 * This alert is sent by clients that receive an invalid certificate status
	 * response (see Section 3.6). This message is always fatal.
	 */
	public static final short bad_certificate_status_response = 113;

	/**
	 * This alert is sent by servers when a certificate hash does not match a client
	...
	/**
	 * If TLS_FALLBACK_SCSV appears in ClientHello.cipher_suites and the highest
	 * protocol version supported by the server is higher than the version indicated
	 * in ClientHello.client_version, the server MUST respond with a fatal
