...
/**
 * A generic TLS 1.0-1.2 / SSLv3 block cipher. This can be used for AES or 3DES
 * for example.
 */
	...
	public byte[] decodeCiphertext(long seqNo, short type, byte[] ciphertext, int offset, int len) throws IOException {
		...
		if (encryptThenMAC) {
			...
			if (badMac) {
				 ...
				 * decryption is performed, and if the MAC verification fails, then processing
				 * SHALL terminate immediately. For TLS, a fatal bad_record_mac MUST be
				 * generated [2]. For DTLS, the record MUST be discarded, and a fatal
				 * bad_record_mac MAY be generated [4]. This immediate response to a bad MAC
