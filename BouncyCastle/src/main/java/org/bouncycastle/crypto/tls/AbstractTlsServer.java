...
public abstract class AbstractTlsServer extends AbstractTlsPeer implements TlsServer {
	...
	protected boolean supportsClientECCCapabilities(int[] namedCurves, short[] ecPointFormats) {
		// NOTE: BC supports all the current set of point formats so we don't check them
		// here

		if (namedCurves == null) {
			/*
			 * RFC 4492 4. A client that proposes ECC cipher suites may choose not to
			 * include these extensions. In this case, the server is free to choose any one
			 * of the elliptic curves or point formats [...].
			 */
		...
		for (int i = 0; i < namedCurves.length; ++i) {
			...
			if (NamedCurve.isValid(namedCurve) && (!NamedCurve.refersToASpecificNamedCurve(namedCurve)
					|| TlsECCUtils.isSupportedNamedCurve(namedCurve))) {
				return true;
			}
		}
	...
	public void notifyFallback(boolean isFallback) throws IOException {
		/*
		 * RFC 7507 3. If TLS_FALLBACK_SCSV appears in ClientHello.cipher_suites and the
		 * highest protocol version supported by the server is higher than the version
		 * indicated in ClientHello.client_version, the server MUST respond with a fatal
	...
	public void processClientExtensions(Hashtable clientExtensions) throws IOException {
		...
		if (clientExtensions != null) {
			...
			if (this.supportedSignatureAlgorithms != null) {
				/*
				 * RFC 5246 7.4.1.4.1. Note: this extension is not meaningful for TLS versions
				 * prior to 1.2. Clients MUST NOT offer it if they are offering prior versions.
				 */
		...
		/*
		 * RFC 4429 4. The client MUST NOT include these extensions in the ClientHello
		 * message if it does not propose any ECC cipher suites.
		 * 
		 * NOTE: This was overly strict as there may be ECC cipher suites that we don't
		 * recognize. Also, draft-ietf-tls-negotiated-ff-dhe will be overloading the
		 * 'elliptic_curves' extension to explicitly allow FFDHE (i.e. non-ECC) groups.
		 */
		// if (!this.eccCipherSuitesOffered && (this.namedCurves != null ||
	...
	public int getSelectedCipherSuite() throws IOException {
		/*
		 * RFC 5246 7.4.3. In order to negotiate correctly, the server MUST check any
		 * candidate cipher suites against the "signature_algorithms" extension before
		 * selecting them. This is somewhat inelegant but is a compromise designed to
		...
		/*
		 * RFC 4429 5.1. A server that receives a ClientHello containing one or both of
		 * these extensions MUST use the client's enumerated capabilities to guide its
		 * selection of an appropriate cipher suite. One of the proposed ECC cipher
		 * suites must be negotiated only if the server can successfully complete the
		 * handshake while using the curves and point formats supported by the client
		 * [...].
		 */
	...
	public Hashtable getServerExtensions() throws IOException {
		...
		if (this.clientECPointFormats != null && TlsECCUtils.isECCCipherSuite(this.selectedCipherSuite)) {
			/*
			 * RFC 4492 5.2. A server that selects an ECC cipher suite in response to a
			 * ClientHello message including a Supported Point Formats Extension appends
			 * this extension (along with others) to its ServerHello message, enumerating
			 * the point formats it can parse.
			 */
	...
	public TlsCompression getCompression() throws IOException {
		...
		default:
			/*
			 * Note: internal error here; we selected the compression method, so if we now
			 * can't produce an implementation, we shouldn't have chosen it!
			 */
			throw new TlsFatalAlert(AlertDescription.internal_error);
		}
	}
	...
	public NewSessionTicket getNewSessionTicket() throws IOException {
		/*
		 * RFC 5077 3.3. If the server determines that it does not want to include a
