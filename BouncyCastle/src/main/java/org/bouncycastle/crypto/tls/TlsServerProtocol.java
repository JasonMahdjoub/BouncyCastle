...
public class TlsServerProtocol extends TlsProtocol {
	 ...
	 * 
	 * @param input
	 *            The stream of data from the client
	 * @param output
				...
				if (this.serverCredentials != null) {
					...
					if (this.certificateRequest != null) {
						if (TlsUtils.isTLSv12(
								getContext()) != (certificateRequest.getSupportedSignatureAlgorithms() != null)) {
	...
	protected void receiveCertificateVerifyMessage(ByteArrayInputStream buf) throws IOException {
		...
		try {
			...
			byte[] hash;
			if (TlsUtils.isTLSv12(getContext())) {
				TlsUtils.verifySupportedSignatureAlgorithm(certificateRequest.getSupportedSignatureAlgorithms(),
						signatureAlgorithm);
	...
	protected void receiveClientHelloMessage(ByteArrayInputStream buf) throws IOException {
		...
		/*
		 * TODO RFC 5077 3.4. If a ticket is presented by the client, the server MUST
		 * NOT attempt to use the Session ID in the ClientHello for stateful session
		 * resumption.
		 */
		...
		/*
		 * TODO RFC 5246 7.4.1.2. If the session_id field is not empty (implying a
		 * session resumption request), this vector MUST include at least the
		...
		/*
		 * TODO RFC 5246 7.4.1.2. If the session_id field is not empty (implying a
		 * session resumption request), it MUST include the compression_method from that
		 * session.
		 */
		...
		this.securityParameters.extendedMasterSecret = TlsExtensionsUtils
				.hasExtendedMasterSecretExtension(clientExtensions);
	...
	protected void sendServerHelloMessage() throws IOException {
		...
		if (this.serverExtensions != null) {
			 ...
			 */
			this.allowCertificateStatus = !resumedSession && TlsUtils.hasExpectedEmptyExtensionData(serverExtensions,
					TlsExtensionsUtils.EXT_status_request, AlertDescription.internal_error);

			this.expectSessionTicket = !resumedSession && TlsUtils.hasExpectedEmptyExtensionData(serverExtensions,
