...
public class TlsSRPKeyExchange extends AbstractTlsKeyExchange {
	...
	public byte[] generateServerKeyExchange() throws IOException {
		...
		if (serverCredentials != null) {
			 ...
			 * TLS 1.2
			 */
			SignatureAndHashAlgorithm signatureAndHashAlgorithm = TlsUtils.getSignatureAndHashAlgorithm(context,
					serverCredentials);
	...
	public void processServerKeyExchange(InputStream input) throws IOException {
		...
		/*
		 * RFC 5054 2.5.3: The client MUST abort the handshake with an
		 * "illegal_parameter" alert if B % N = 0.
		 */
		try {
	...
	public void processClientKeyExchange(InputStream input) throws IOException {
		/*
		 * RFC 5054 2.5.4: The server MUST abort the handshake with an
		 * "illegal_parameter" alert if A % N = 0.
		 */
		try {
			this.srpPeerCredentials = SRP6Util.validatePublicValue(srpGroup.getN(),
	...
	public byte[] generatePremasterSecret() throws IOException {
		try {
			BigInteger S = srpServer != null ? srpServer.calculateSecret(srpPeerCredentials)
	...
	protected Signer initVerifyer(TlsSigner tlsSigner, SignatureAndHashAlgorithm algorithm,
