...
public class DTLSClientProtocol extends DTLSProtocol {
	...
	public DTLSTransport connect(TlsClient client, DatagramTransport transport) throws IOException {
		...
		DTLSRecordLayer recordLayer = new DTLSRecordLayer(transport, state.clientContext, client,
	...
	protected void abortClientHandshake(ClientHandshakeState state, DTLSRecordLayer recordLayer,
	...
	protected DTLSTransport clientHandshake(ClientHandshakeState state, DTLSRecordLayer recordLayer)
		...
		while (serverMessage.getType() == HandshakeType.hello_verify_request) {
			 ...
			 * regardless of the version of TLS that is expected to be negotiated. DTLS 1.2
			 * and 1.0 clients MUST use the version solely to indicate packet formatting
			 * (which is the same in both DTLS 1.2 and 1.0) and not as part of version
			 * negotiation.
			 */
		...
		if (state.clientCredentials != null && state.clientCredentials instanceof TlsSignerCredentials) {
			...
			SignatureAndHashAlgorithm signatureAndHashAlgorithm = TlsUtils
					.getSignatureAndHashAlgorithm(state.clientContext, signerCredentials);
		...
		if (state.tlsSession != null) {
			state.sessionParameters = new SessionParameters.Builder()
					...
					.setMasterSecret(securityParameters.getMasterSecret()).setPeerCertificate(serverCertificate)
					.setPSKIdentity(securityParameters.getPSKIdentity())
					.setSRPIdentity(securityParameters.getSRPIdentity())
					// TODO Consider filtering extensions that aren't relevant to resumed sessions
					.setServerExtensions(state.serverExtensions).build();
	...
	protected byte[] processHelloVerifyRequest(ClientHandshakeState state, byte[] body) throws IOException {
		...
		/*
		 * RFC 6347 This specification increases the cookie size limit to 255 bytes for
		 * greater future flexibility. The limit remains 32 for previous versions of
		 * DTLS.
		 */
	...
	protected void processServerHello(ClientHandshakeState state, byte[] body) throws IOException {
		...
		if (!Arrays.contains(state.offeredCipherSuites, selectedCipherSuite)
				...
				|| !TlsUtils.isValidCipherSuiteForVersion(selectedCipherSuite,
						state.clientContext.getServerVersion())) {
			throw new TlsFatalAlert(AlertDescription.illegal_parameter);
		}
		 ...
		 * response to an extended client hello message. However, see RFC 5746 exception
		 * below. We always include the SCSV, so an Extended Server Hello is always
		 * allowed.
		 */
		...
		if (sessionServerExtensions != null) {
			...
			securityParameters.extendedMasterSecret = TlsExtensionsUtils
			 ...
			 */
			state.allowCertificateStatus = !state.resumedSession && TlsUtils.hasExpectedEmptyExtensionData(
					sessionServerExtensions, TlsExtensionsUtils.EXT_status_request, AlertDescription.illegal_parameter);

			state.expectSessionTicket = !state.resumedSession && TlsUtils.hasExpectedEmptyExtensionData(
