...
public class TlsClientProtocol extends TlsProtocol {
	 ...
	 * {@link #readOutput(byte[], int, int)} to get the corresponding ciphertext.
	 * 
	 * @param secureRandom
	 *            Random number generator for various cryptographic functions
	 */
	 ...
	 * <br>
	 * In blocking mode, this will not return until the handshake is complete. In
	 * non-blocking mode, use {@link TlsPeer#notifyHandshakeComplete()} to receive a
	 * callback when the handshake is complete.
	 *
	 * @param tlsClient
	 *            The {@link TlsClient} to use for the handshake.
	 * @throws IOException
	...
	protected void handleHandshakeMessage(short type, ByteArrayInputStream buf) throws IOException {
		...
		case HandshakeType.certificate: {
			...
			case CS_SERVER_SUPPLEMENTAL_DATA: {
				...
				// TODO[RFC 3546] Check whether empty certificates is possible, allowed, or
				// excludes CertificateStatus
		...
		case HandshakeType.server_hello_done: {
			...
			case CS_CERTIFICATE_REQUEST: {
				...
				/*
				 * Send the client key exchange message, depending on the key exchange we are
				 * using in our CipherSuite.
				 */
				...
				if (clientCreds != null && clientCreds instanceof TlsSignerCredentials) {
					...
					/*
					 * RFC 5246 4.7. digitally-signed element needs SignatureAndHashAlgorithm from
					 * TLS 1.2
					 */
					SignatureAndHashAlgorithm signatureAndHashAlgorithm = TlsUtils
							.getSignatureAndHashAlgorithm(getContext(), signerCredentials);
		...
		case HandshakeType.certificate_request: {
			...
			case CS_SERVER_KEY_EXCHANGE: {
				...
				/*
				 * TODO Give the client a chance to immediately select the CertificateVerify
				 * hash algorithm here to avoid tracking the other hash algorithms
				 * unnecessarily?
				 */
		...
		case HandshakeType.session_ticket: {
			...
			case CS_CLIENT_FINISHED: {
				...
				/*
				 * RFC 5077 3.4. If the client receives a session ticket from the server, then
				 * it discards any Session ID that was sent in the ServerHello.
				 */
				invalidateSession();
		...
		case HandshakeType.hello_request: {
			...
			/*
			 * RFC 2246 7.4.1.1 Hello request This message will be ignored by the client if
			 * the client is currently negotiating a session. This message may be ignored by
			 * the client if it does not wish to renegotiate a session, or the client may,
			 * if it wishes, respond with a no_renegotiation alert.
			 */
	...
	protected void receiveServerHelloMessage(ByteArrayInputStream buf) throws IOException {
		...
		/*
		 * Find out which CipherSuite the server has chosen and check that it was one of
		 * the offered ones, and is a valid selection for the negotiated version.
		 */
		int selectedCipherSuite = TlsUtils.readUint16(buf);
		if (!Arrays.contains(this.offeredCipherSuites, selectedCipherSuite)
				|| selectedCipherSuite == CipherSuite.TLS_NULL_WITH_NULL_NULL || CipherSuite.isSCSV(selectedCipherSuite)
		...
		/*
		 * Find out which CompressionMethod the server has chosen and check that it was
		...
		/*
		 * RFC3546 2.2 The extended server hello message format MAY be sent in place of
		 * the server hello message when the client has requested extended functionality
		 * via the extended client hello message specified in Section 2.1. ... Note that
		 * the extended server hello message is only sent in response to an extended
		 * client hello message. This prevents the possibility that the extended server
		 * hello message could "break" existing TLS 1.0 clients.
		 */
		this.serverExtensions = readExtensions(buf);

		/*
		 * RFC 3546 2.2 Note that the extended server hello message is only sent in
		 * response to an extended client hello message.
		 * 
		...
		if (this.serverExtensions != null) {
			...
			while (e.hasMoreElements()) {
				...
				/*
				 * RFC 5746 3.6. Note that sending a "renegotiation_info" extension in response
				 * to a ClientHello containing only the SCSV is an explicit exception to the
				 * prohibition in RFC 5246, Section 7.4.1.4, on the server sending unsolicited
				 * extensions and is only allowed because the client is signaling its
				 * willingness to receive the extension via the
				...
				/*
				 * RFC 3546 2.3. If [...] the older session is resumed, then the server MUST
				 * ignore extensions appearing in the client hello, and send a server hello
				 * containing no extensions[.]
				 */
				if (this.resumedSession) {
					// TODO[compat-gnutls] GnuTLS test server sends server extensions e.g.
					// ec_point_formats
					// TODO[compat-openssl] OpenSSL test server sends server extensions e.g.
					// ec_point_formats
					// TODO[compat-polarssl] PolarSSL test server sends server extensions e.g.
					// ec_point_formats
					// throw new TlsFatalAlert(AlertDescription.illegal_parameter);
				}
			}
		...
		// TODO[compat-gnutls] GnuTLS test server fails to send renegotiation_info
		// extension when resuming
		...
		if (sessionServerExtensions != null) {
			{
				/*
				 * RFC 7366 3. If a server receives an encrypt-then-MAC request extension from a
				 * client and then selects a stream or Authenticated Encryption with Associated
				 * Data (AEAD) ciphersuite, it MUST NOT send an encrypt-then-MAC response
				 * extension back to the client.
				 */
				boolean serverSentEncryptThenMAC = TlsExtensionsUtils
						.hasEncryptThenMACExtension(sessionServerExtensions);
			...
			this.securityParameters.extendedMasterSecret = TlsExtensionsUtils
					.hasExtendedMasterSecretExtension(sessionServerExtensions);
			...
			/*
			 * TODO It's surprising that there's no provision to allow a 'fresh'
			 * CertificateStatus to be sent in a session resumption handshake.
			 */
		 ...
		 * TODO[session-hash]
		 * 
		 * draft-ietf-tls-session-hash-04 4. Clients and servers SHOULD NOT accept
		 * handshakes that do not use the extended master secret [..]. (and see 5.2,
		 * 5.3)
		 */
	...
	protected void sendClientHelloMessage() throws IOException {
		...
		/*
		 * TODO RFC 5077 3.4. When presenting a ticket, the client MAY generate and
		 * include a Session ID in the TLS ClientHello.
		 */
		...
		// Cipher Suites (and SCSV)
		{
			...
			if (noRenegExt && noRenegSCSV) {
				...
				// TlsExtensionsUtils.ensureExtensionsInitialised(this.clientExtensions);
				// this.clientExtensions.put(EXT_RenegotiationInfo,
			...
			/*
			 * RFC 7507 4. If a client sends a ClientHello.client_version containing a lower
			 * value than the latest (highest-valued) version supported by the client, it
			 * SHOULD include the TLS_FALLBACK_SCSV cipher suite value in
			 * ClientHello.cipher_suites [..]. (The client SHOULD put TLS_FALLBACK_SCSV
			 * after all cipher suites that it actually intends to negotiate.)
			 */
