...
public class TlsECCUtils {
	...
	public static byte[] createSupportedPointFormatsExtension(short[] ecPointFormats) throws IOException {
		if (ecPointFormats == null || !Arrays.contains(ecPointFormats, ECPointFormat.uncompressed)) {
			/*
			 * RFC 4492 5.1. If the Supported Point Formats Extension is indeed sent, it
			 * MUST contain the value 0 (uncompressed) as one of the items in the list of
			 * point formats.
			 */
	...
	public static byte[] serializeECPoint(short[] ecPointFormats, ECPoint point) throws IOException {
		...
		/*
		 * RFC 4492 5.7. ...an elliptic curve point in uncompressed or compressed
		 * format. Here, the format MUST conform to what the server has requested
		 * through a Supported Point Formats Extension if this extension was used, and
		 * MUST be uncompressed if this extension was not used.
		 */
	...
	public static byte[] calculateECDHBasicAgreement(ECPublicKeyParameters publicKey,
		...
		/*
		 * RFC 4492 5.10. Note that this octet string (Z in IEEE 1363 terminology) as
		 * output by FE2OSP, the Field Element to Octet String Conversion Primitive, has
		 * constant length for any given field; leading zeros found in this octet string
	...
	static ECPrivateKeyParameters generateEphemeralServerKeyExchange(SecureRandom random, int[] namedCurves,
		...
		if (namedCurve >= 0) {
			...
			/*
			 * If no named curves are suitable, check if the client supports explicit
			 * curves.
			 */
		...
		if (ecParams == null) {
			/*
			 * NOTE: We shouldn't have negotiated ECDHE key exchange since we apparently
			 * can't find a suitable curve.
			 */
			throw new TlsFatalAlert(AlertDescription.internal_error);
		}
