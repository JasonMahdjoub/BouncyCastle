...
/**
 * An implementation of the TLS 1.0/1.1/1.2 record layer, allowing downgrade to
 * SSLv3.
 */
class RecordStream {
	...
	/**
	 * RFC 5246 E.1. "Earlier versions of the TLS specification were not fully clear
	 * on what the record layer version number (TLSPlaintext.version) should contain
	 * when sending ClientHello (i.e., before it is known which version of the
	 * protocol will be employed). Thus, TLS servers compliant with this
	 * specification MUST accept any value {03,XX} as the record layer version
	...
	void checkRecordHeader(byte[] recordHeader) throws IOException {
		...
		/*
		 * RFC 5246 6. If a TLS implementation receives an unexpected record type, it
		 * MUST send an unexpected_message alert.
		 */
	...
	byte[] decodeAndVerify(short type, InputStream input, int len) throws IOException {
		...
		/*
		 * TODO RFC 5246 6.2.2. Implementation note: Decompression functions are
		 * responsible for ensuring that messages cannot cause internal buffer
		 * overflows.
		 */
		...
		/*
		 * RFC 5246 6.2.2. If the decompression function encounters a
		 * TLSCompressed.fragment that would decompress to a length in excess of 2^14
		 * bytes, it should report a fatal decompression failure error.
		 */
	...
	void writeRecord(short type, byte[] plaintext, int plaintextOffset, int plaintextLength) throws IOException {
		...
		/*
		 * RFC 5246 6. Implementations MUST NOT send record types not defined in this
		...
		if (cOut == buffer) {
			...
			/*
			 * RFC 5246 6.2.2. Compression must be lossless and may not increase the content
