...
public class TlsRSAUtils {
	...
	public static byte[] safeDecryptPreMasterSecret(TlsContext context, RSAKeyParameters rsaServerPrivateKey,
		...
		/*
		 * Generate 48 random bytes we can use as a Pre-Master-Secret, if the PKCS1
		 * padding check should fail.
		 */
		...
		if (versionNumberCheckDisabled && clientVersion.isEqualOrEarlierVersionOf(ProtocolVersion.TLSv10)) {
			...
			/*
			 * OK, we need to compare the version number in the decrypted Pre-Master-Secret
			 * with the clientVersion received during the handshake. If they don't match, we
			 * replace the decrypted Pre-Master-Secret with a random one.
			 */
