...
public abstract class AbstractTlsKeyExchange implements TlsKeyExchange {
	...
	public void init(TlsContext context) {
		...
		if (TlsUtils.isSignatureAlgorithmsExtensionAllowed(clientVersion)) {
			/*
			 * RFC 5246 7.4.1.4.1. If the client does not send the signature_algorithms
			 * extension, the server MUST do the following:
			 * 
	...
	public void processServerCertificate(Certificate serverCertificate) throws IOException {
		if (supportedSignatureAlgorithms == null) {
			/*
			 * TODO RFC 2246 7.4.2. Unless otherwise specified, the signing algorithm for
			 * the certificate must be the same as the algorithm for the certificate key.
			 */
		} else {
			/*
			 * TODO RFC 5246 7.4.2. If the client provided a "signature_algorithms"
			 * extension, then all certificates provided by the server MUST be signed by a
