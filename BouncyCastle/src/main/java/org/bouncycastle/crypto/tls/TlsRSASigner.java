...
public class TlsRSASigner extends AbstractTlsSigner {
	...
	protected Signer makeSigner(SignatureAndHashAlgorithm algorithm, boolean raw, boolean forSigning,
		...
		if (algorithm != null) {
			/*
			 * RFC 5246 4.7. In RSA signing, the opaque vector contains the signature
			 * generated using the RSASSA-PKCS1-v1_5 signature scheme defined in [PKCS1].
			 */
			s = new RSADigestSigner(d, TlsUtils.getOIDForHashAlgorithm(algorithm.getHash()));
		} else {
			/*
			 * RFC 5246 4.7. Note that earlier versions of TLS used a different RSA
	...
	protected AsymmetricBlockCipher createRSAImpl() {
		/*
		 * RFC 5246 7.4.7.1. Implementation note: It is now known that remote
		 * timing-based attacks on TLS are possible, at least when the client and server
		 * are on the same LAN. Accordingly, implementations that use static RSA keys
		 * MUST use RSA blinding or some other anti-timing technique, as described in
		 * [TIMING].
		 */
