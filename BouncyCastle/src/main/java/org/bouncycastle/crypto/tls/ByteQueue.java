...
public class ByteQueue {
	/**
	 * @return The smallest number which can be written as 2^x which is bigger than
	 *         i.
	 */
	 ...
	 *
	 * @param output
	 *            The {@link OutputStream} to copy the bytes to.
	 * @param length
	...
	public void read(byte[] buf, int offset, int len, int skip) {
		if ((buf.length - offset) < len) {
			throw new IllegalArgumentException(
					"Buffer size of " + buf.length + " is too small for a read of " + len + " bytes");
		}
	...
	/**
	 * Return a {@link ByteArrayInputStream} over some bytes at the beginning of the
	 * data.
	 * 
	 * @param length
