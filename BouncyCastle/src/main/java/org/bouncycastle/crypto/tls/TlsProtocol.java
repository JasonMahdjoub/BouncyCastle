...
public abstract class TlsProtocol {
	...
	protected abstract void handleHandshakeMessage(short type, ByteArrayInputStream buf) throws IOException;
	...
	protected void completeHandshake() throws IOException {
		try {
			...
			/*
			 * If this was an initial handshake, we are now ready to send and receive
			 * application data.
			 */
			if (!appDataReady) {
			...
			if (this.tlsSession != null) {
				if (this.sessionParameters == null) {
					this.sessionParameters = new SessionParameters.Builder()
							...
							.setServerExtensions(this.serverExtensions).build();
	...
	private void processApplicationDataQueue() {
		 ...
		 * 
		 * This function could be used for callbacks when application data arrives in
		 * the future.
		 */
	}
	...
	private void processAlertQueue() throws IOException {
		while (alertQueue.available() >= 2) {
			...
			if (level == AlertLevel.fatal) {
				/*
				 * RFC 2246 7.2.1. The session becomes unresumable if any connection is
				 * terminated without proper close_notify messages with level equal to warning.
				 */
				invalidateSession();
				...
				/*
				 * RFC 5246 7.2.1. The other party MUST respond with a close_notify alert of its
	 ...
	 *
	 * @throws IOException
	 *             If the message has an invalid content or the handshake is not in
	 *             the correct state.
	 */
	 ...
	 * still some data left in the buffer, or block until some application data has
	 * been read from the network.
	 *
	 * @param buf
	...
	protected void writeData(byte[] buf, int offset, int len) throws IOException {
		...
		while (len > 0) {
			/*
			 * RFC 5246 6.2.1. Zero-length fragments of Application data MAY be sent as they
			 * are potentially useful as a traffic analysis countermeasure.
			 * 
			 * NOTE: Actually, implementations appear to have settled on 1/n-1 record
			 * splitting.
			 */
	...
	/**
	 * @return An OutputStream which can be used to send data. Only allowed in
	 *         blocking mode.
	 */
	...
	/**
	 * @return An InputStream which can be used to read data. Only allowed in
	 *         blocking mode.
	 */
	 ...
	 * <br>
	 * After this method returns, the input buffer is "owned" by this object. Other
	 * code must not attempt to do anything with it.<br>
	 * <br>
	 * This method will decrypt and process all records that are fully available. If
	 * only part of a record is available, the buffer will be retained until the
	...
	public void offerInput(byte[] input) throws IOException {
		...
		while (inputBuffers.available() >= RecordStream.TLS_HEADER_SIZE) {
			...
			int totalLength = TlsUtils.readUint16(recordHeader, RecordStream.TLS_HEADER_LENGTH_OFFSET)
					+ RecordStream.TLS_HEADER_SIZE;
	...
	/**
	 * Gets the amount of received application data. A call to
	 * {@link #readInput(byte[], int, int)} is guaranteed to be able to return at
	...
	public int getAvailableInputBytes() {
		if (blocking) {
			throw new IllegalStateException(
					"Cannot use getAvailableInputBytes() in blocking mode! Use getInputStream().available() instead.");
		}
		return applicationDataAvailable();
	}

	/**
	 * Retrieves received application data. Use {@link #getAvailableInputBytes()} to
	 * check how much application data is currently available. This method functions
	 * similarly to {@link InputStream#read(byte[], int, int)}, except that it never
	 * blocks. If no data is available, nothing will be copied and zero will be
	 * returned.<br>
	 * <br>
	 * Only allowed in non-blocking mode.
	 * 
	 ...
	 *         length specified if the length was greater than the amount of
	 *         available data.
	 */
	public int readInput(byte[] buffer, int offset, int length) {
		...
		try {
			...
			// readApplicationData() only throws if there is no data available, so this
			// should never happen
	 ...
	 * <br>
	 * This method must not be called until after the handshake is complete!
	 * Attempting to call it before the handshake is complete will result in an
	 * exception.
	 * 
	 * @param buffer
	 *            The buffer containing application data to encrypt
	 * @param offset
	 *            The offset at which to begin reading data
	 * @param length
	 *            The number of bytes of data to read
	 * @throws IOException
	 *             If an error occurs encrypting the data, or the handshake is not
	 *             complete
	 */
	public void offerOutput(byte[] buffer, int offset, int length) throws IOException {
		if (blocking) {
			throw new IllegalStateException(
					"Cannot use offerOutput() in blocking mode! Use getOutputStream() instead.");
		}
	...
	public int getAvailableOutputBytes() {
		if (blocking) {
			throw new IllegalStateException(
					"Cannot use getAvailableOutputBytes() in blocking mode! Use getOutputStream() instead.");
		}
	...
	/**
	 * Retrieves encrypted data to be sent. Use {@link #getAvailableOutputBytes()}
	 * to check how much encrypted data is currently available. This method
	 * functions similarly to {@link InputStream#read(byte[], int, int)}, except
	 * that it never blocks. If no data is available, nothing will be copied and
	 * zero will be returned.<br>
	 * <br>
	 * Only allowed in non-blocking mode.
	 * 
	 * @param buffer
	...
	protected void failWithError(short alertLevel, short alertDescription, String message, Throwable cause)
		...
		if (!closed) {
			...
			if (alertLevel == AlertLevel.fatal) {
				 ...
				 * terminated without proper close_notify messages with level equal to warning.
				 */
				// TODO This isn't quite in the right place. Also, as of TLS 1.1 the above is
				// obsolete.
				invalidateSession();
	...
	protected short processMaxFragmentLengthExtension(Hashtable clientExtensions, Hashtable serverExtensions,
		...
		if (maxFragmentLength >= 0) {
			if (!MaxFragmentLength.isValid(maxFragmentLength) || (!this.resumedSession
	...
	protected static void writeExtensions(OutputStream output, Hashtable extensions) throws IOException {
		...
		/*
		 * NOTE: There are reports of servers that don't accept a zero-length extension
		 * as the last one, so we write out any zero-length ones first as a best-effort
		 * workaround.
		 */
