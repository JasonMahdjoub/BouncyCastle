...
public class DTLSServerProtocol extends DTLSProtocol {
	...
	public DTLSTransport accept(TlsServer server, DatagramTransport transport) throws IOException {
		...
		DTLSRecordLayer recordLayer = new DTLSRecordLayer(transport, state.serverContext, server,
				ContentType.handshake);

		// TODO Need to handle sending of HelloVerifyRequest without entering a full
		// connection
	...
	protected void abortServerHandshake(ServerHandshakeState state, DTLSRecordLayer recordLayer,
	...
	protected DTLSTransport serverHandshake(ServerHandshakeState state, DTLSRecordLayer recordLayer)
		...
		// NOTE: DTLSRecordLayer requires any DTLS version, we don't otherwise constrain
		// this
		...
		if (state.serverCredentials != null) {
			...
			if (state.certificateRequest != null) {
				if (TlsUtils.isTLSv12(
						state.serverContext) != (state.certificateRequest.getSupportedSignatureAlgorithms() != null)) {
		...
		/*
		 * RFC 5246 7.4.8 This message is only sent following a client certificate that
	...
	protected byte[] generateServerHello(ServerHandshakeState state) throws IOException {
		...
		/*
		 * The server may return an empty session_id to indicate that the session will
		...
		if (!Arrays.contains(state.offeredCipherSuites, selectedCipherSuite)
				...
				|| !TlsUtils.isValidCipherSuiteForVersion(selectedCipherSuite,
						state.serverContext.getServerVersion())) {
			throw new TlsFatalAlert(AlertDescription.internal_error);
		}
		...
		if (state.secure_renegotiation) {
			...
			if (noRenegExt) {
				...
				/*
				 * If the secure_renegotiation flag is set to TRUE, the server MUST include an
		...
		if (state.serverExtensions != null) {
			...
			state.allowCertificateStatus = !state.resumedSession && TlsUtils.hasExpectedEmptyExtensionData(
					state.serverExtensions, TlsExtensionsUtils.EXT_status_request, AlertDescription.internal_error);

			state.expectSessionTicket = !state.resumedSession && TlsUtils.hasExpectedEmptyExtensionData(
					state.serverExtensions, TlsProtocol.EXT_SessionTicket, AlertDescription.internal_error);
	...
	protected void notifyClientCertificate(ServerHandshakeState state, Certificate clientCertificate)
		...
		if (clientCertificate.isEmpty()) {
			...
			/*
			 * TODO RFC 5246 7.4.6. If the certificate_authorities list in the certificate
			 * request message was non-empty, one of the certificates in the certificate
		...
		/*
		 * RFC 5246 7.4.6. If the client does not send any certificates, the server MAY
		 * at its discretion either continue the handshake without client
		 * authentication, or respond with a fatal handshake_failure alert. Also, if
		 * some aspect of the certificate chain was unacceptable (e.g., it was not
		 * signed by a known, trusted CA), the server MAY at its discretion either
		 * continue the handshake (considering the client unauthenticated) or send a
		 * fatal alert.
		 */
	...
	protected void processCertificateVerify(ServerHandshakeState state, byte[] body, TlsHandshakeHash prepareFinishHash)
		...
		try {
			...
			if (TlsUtils.isTLSv12(context)) {
				TlsUtils.verifySupportedSignatureAlgorithm(state.certificateRequest.getSupportedSignatureAlgorithms(),
						signatureAlgorithm);
			...
			if (!tlsSigner.verifyRawSignature(signatureAlgorithm, clientCertificateVerify.getSignature(), publicKey,
					hash)) {
	...
	protected void processClientHello(ServerHandshakeState state, byte[] body) throws IOException {
		...
		/*
		 * NOTE: "If the session_id field is not empty (implying a session resumption
		 * request) this vector must include at least the cipher_suite from that
		 * session."
		 */
		...
		securityParameters.extendedMasterSecret = TlsExtensionsUtils
				.hasExtendedMasterSecretExtension(state.clientExtensions);
		 ...
		 */
		{
			 ...
			 * TLS_EMPTY_RENEGOTIATION_INFO_SCSV SCSV. If it does, set the
			...
			/*
			 * The server MUST check if the "renegotiation_info" extension is included in
			 * the ClientHello.
			 */
