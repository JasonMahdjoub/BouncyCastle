 ...
 * <p>
 * Blake2b offers a built-in keying mechanism to be used directly for
 * authentication ("Prefix-MAC") rather than a HMAC construction.
 * <p>
 * Blake2b offers a built-in support for a salt for randomized hashing and a
 * personal string for defining a unique hash function for each application.
 * <p>
...
public class Blake2bDigest implements ExtendedDigest {
	...
	/*
	 * private int fanout = 1; // 0-255 private int depth = 1; // 1 - 255 private
	 * int leafLength= 0; private long nodeOffset = 0L; private int nodeDepth = 0;
	...
	/**
	 * Blake2b for authentication ("Prefix-MAC mode"). After calling the doFinal()
	 * method, the key will remain to be used for further computations of this
	 * instance. The key can be overwritten using the clearKey() method.
	 *
	 * @param key
	...
	/**
	 * Blake2b with key, required digest length (in bytes), salt and
	 * personalization. After calling the doFinal() method, the key, the salt and
	 * the personal string will remain and might be used for further computations
	 * with this instance. The key can be overwritten using the clearKey() method,
	 * the salt (pepper) can be overwritten using the clearSalt() method.
	 *
	 * @param key
	 *            A key up to 64 bytes or null
	 * @param digestLength
	...
	public void update(byte[] message, int offset, int len) {
		...
		for (messagePos = offset + remainingLength; messagePos < blockWiseLastPos; messagePos += BLOCK_LENGTH_BYTES) { // block
																														// wise
																														// 128
																														// bytes
																														// without
	 ...
	 * the digest reset. Key, salt and personal string remain.
	 *
	 * @param out
	...
	/**
	 * Reset the digest back to it's initial state. The key, the salt and the
	...
	private final byte[] long2bytes(long longValue) {
		return new byte[] { (byte) longValue, (byte) (longValue >> 8), (byte) (longValue >> 16),
				(byte) (longValue >> 24), (byte) (longValue >> 32), (byte) (longValue >> 40), (byte) (longValue >> 48),
				(byte) (longValue >> 56) };
	}
