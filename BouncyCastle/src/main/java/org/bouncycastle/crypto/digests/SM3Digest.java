 ...
 * http://tools.ietf.org/html/draft-shen-sm3-hash-01 and at .... ( Chinese PDF )
 * <p>
 * The specification says "process a bit stream", but this is written to process
 * bytes in blocks of 4, meaning this will process 32-bit word groups. But so do
 * also most other digest specifications, including the SHA-256 which was a
 * origin for this specification.
 */
public class SM3Digest extends GeneralDigest {
	...
	// Round constant T for processBlock() which is 32 bit integer rolled left up to
	// (63 MOD 32) bit positions.
	 ...
	 * 
	 * 
	 * FFj(X;Y;Z) = X XOR Y XOR Z when 0 < = j < = 15 = (X AND Y) OR (X AND Z) OR (Y
	 * AND Z) when 16 < = j < = 63
	 * 
	 * GGj(X;Y;Z) = X XOR Y XOR Z when 0 < = j < = 15 = (X AND Y) OR (NOT X AND Z)
	 * when 16 < = j < = 63
	 * 
	 ...
	 * 
	 * 
	 * P0(X) = X XOR (X <<< 9) XOR (X <<< 17) ## ROLL, not SHIFT P1(X) = X XOR (X
	 * <<< 15) XOR (X <<< 23) ## ROLL, not SHIFT
	 * 
	 ...
	 * 
	 * ROLL 9 : ((x << 9) | (x >>> (32-9)))) ROLL 17 : ((x << 17) | (x >>> (32-17)))
	 * ROLL 15 : ((x << 15) | (x >>> (32-15))) ROLL 23 : ((x << 23) | (x >>>
	 * (32-23)))
	 * 
	 */
