...
/**
 * Implementation of the Skein family of parameterised hash functions in 256,
 * 512 and 1024 bit block sizes, based on the {@link ThreefishEngine Threefish}
 * tweakable block cipher.
 * <p>
 * This is the 1.3 version of Skein defined in the Skein hash function
 * submission to the NIST SHA-3 competition in October 2010.
 * <p>
 * Skein was designed by Niels Ferguson - Stefan Lucks - Bruce Schneier - Doug
 * Whiting - Mihir Bellare - Tadayoshi Kohno - Jon Callas - Jesse Walker.
 * <p>
 * This implementation is the basis for {@link SkeinDigest} and
 * {@link SkeinMac}, implementing the parameter based configuration system that
 * allows Skein to be adapted to multiple applications. <br>
 * Initialising the engine with {@link SkeinParameters} allows standard and
 * arbitrary parameters to be applied during the Skein hash function.
 * <p>
 * Implemented:
 * <ul>
 * <li>256, 512 and 1024 bit internal states.</li>
 * <li>Full 96 bit input length.</li>
 * <li>Parameters defined in the Skein specification, and arbitrary other pre
 * and post message parameters.</li>
...
public class SkeinEngine implements Memoable {
	...
	/**
	 * Precalculated UBI(CFG) states for common state/output combinations without
	 * key or other pre-message params.
	 */
	private static final Hashtable INITIAL_STATES = new Hashtable();

	static {
		// From Appendix C of the Skein 1.3 NIST submission
		initialState(SKEIN_256, 128,
				new long[] { 0xe1111906964d7260L, 0x883daaa77c8d811cL, 0x10080df491960f7aL, 0xccf7dde5b45bc1c2L });

		initialState(SKEIN_256, 160,
				new long[] { 0x1420231472825e98L, 0x2ac4e9a25a77e590L, 0xd47a58568838d63eL, 0x2dd2e4968586ab7dL });

		initialState(SKEIN_256, 224,
				new long[] { 0xc6098a8c9ae5ea0bL, 0x876d568608c5191cL, 0x99cb88d7d7f53884L, 0x384bddb1aeddb5deL });

		initialState(SKEIN_256, 256,
				new long[] { 0xfc9da860d048b449L, 0x2fca66479fa7d833L, 0xb33bc3896656840fL, 0x6a54e920fde8da69L });

		initialState(SKEIN_512, 128,
				new long[] { 0xa8bc7bf36fbf9f52L, 0x1e9872cebd1af0aaL, 0x309b1790b32190d3L, 0xbcfbb8543f94805cL,
						0x0da61bcd6e31b11bL, 0x1a18ebead46a32e3L, 0xa2cc5b18ce84aa82L, 0x6982ab289d46982dL });

		initialState(SKEIN_512, 160,
				new long[] { 0x28b81a2ae013bd91L, 0xc2f11668b5bdf78fL, 0x1760d8f3f6a56f12L, 0x4fb747588239904fL,
						0x21ede07f7eaf5056L, 0xd908922e63ed70b8L, 0xb8ec76ffeccb52faL, 0x01a47bb8a3f27a6eL });

		initialState(SKEIN_512, 224,
				new long[] { 0xccd0616248677224L, 0xcba65cf3a92339efL, 0x8ccd69d652ff4b64L, 0x398aed7b3ab890b4L,
						0x0f59d1b1457d2bd0L, 0x6776fe6575d4eb3dL, 0x99fbc70e997413e9L, 0x9e2cfccfe1c41ef7L });

		initialState(SKEIN_512, 384,
				new long[] { 0xa3f6c6bf3a75ef5fL, 0xb0fef9ccfd84faa4L, 0x9d77dd663d770cfeL, 0xd798cbf3b468fddaL,
						0x1bc4a6668a0e4465L, 0x7ed7d434e5807407L, 0x548fc1acd4ec44d6L, 0x266e17546aa18ff8L });

		initialState(SKEIN_512, 512,
				new long[] { 0x4903adff749c51ceL, 0x0d95de399746df03L, 0x8fd1934127c79bceL, 0x9a255629ff352cb1L,
	...
	private static class UbiTweak {
		/**
		 * Point at which position might overflow long, so switch to add with carry
		 * logic
		 */
	...
	private class UBI {
		...
		public void update(byte[] value, int offset, int len, long[] output) {
			/*
			 * Buffer complete blocks for the underlying Threefish cipher, only flushing
			 * when there are subsequent bytes (last block must be processed in doFinal()
			 * with final=true set).
			 */
			int copied = 0;
			while (len > copied) {
	 ...
	 *
	 * @param blockSizeBits
	 *            the internal state size in bits - one of {@link #SKEIN_256},
	 *            {@link #SKEIN_512} or {@link #SKEIN_1024}.
	 * @param outputSizeBits
	 *            the output/digest size to produce in bits, which must be an
	...
	/**
	 * Initialises the Skein engine with the provided parameters. See
	 * {@link SkeinParameters} for details on the parameterisation of the Skein hash
	 * function.
	 *
	 * @param params
	 *            the parameters to apply to this engine, or <code>null</code> to
	 *            use no parameters.
	 */
	...
	/**
	 * Reset the engine to the initial state (with the key and any pre-message
	 * parameters , ready to accept message input.
	 */
	public void reset() {
	...
	private void output(long outputSequence, byte[] out, int outOff, int outputBytes) {
		...
		// Output is a sequence of UBI invocations all of which use and preserve the
		// pre-output
		// state
