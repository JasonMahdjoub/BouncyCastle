...
public class KeccakDigest implements ExtendedDigest {
		...
		while (i < databitlen) {
			if ((bitsInQueue == 0) && (databitlen >= rate) && (i <= (databitlen - rate))) {
				...
				for (j = 0; j < wholeBlocks; j++) {
					...
					// displayIntermediateValues.displayBytes(1, "Block to be absorbed", curData,
					// rate / 8);
	...
	private void padAndSwitchToSqueezingPhase() {
		...
		// displayIntermediateValues.displayBytes(1, "Block available for squeezing",
		// dataQueue, bitsAvailableForSqueezing / 8);
	...
	protected void squeeze(byte[] output, int offset, long outputLength) {
		...
		while (i < outputLength) {
			...
			System.arraycopy(dataQueue, (rate - bitsAvailableForSqueezing) / 8, output, offset + (int) (i / 8),
					partialBlock / 8);
	...
	private void keccakPermutation(byte[] state) {
		...
		// displayIntermediateValues.displayStateAsBytes(1, "State after permutation",
		// longState);
	...
	private void keccakPermutationOnWords(long[] state) {
		...
		// displayIntermediateValues.displayStateAs64bitWords(3, "Same, with lanes as
		// 64-bit words", state);
