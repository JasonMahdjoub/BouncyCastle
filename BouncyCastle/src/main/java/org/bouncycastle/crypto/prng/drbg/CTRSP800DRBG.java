...
public class CTRSP800DRBG implements SP80090DRBG {
	 ...
	 * 
	 * @param engine
	 *            underlying block cipher to use to support DRBG
	 * @param keySizeInBits
	 *            size of the key to use with the block cipher.
	 ...
	 */
	public CTRSP800DRBG(BlockCipher engine, int keySizeInBits, int securityStrength, EntropySource entropySource,
	...
	/*
	 * 1. chaining_value = 0^outlen . Comment: Set the first chaining value to
	 * outlen zeros. 2. n = len (data)/outlen. 3. Starting with the leftmost bits of
	 * data, split the data into n blocks of outlen bits each, forming block(1) to
	 * block(n). 4. For i = 1 to n do 4.1 input_block = chaining_value ^ block(i) .
	 * 4.2 chaining_value = Block_Encrypt (Key, input_block). 5. output_block =
	 * chaining_value. 6. Return output_block.
	 */
	...
	public int generate(byte[] output, byte[] additionalInput, boolean predictionResistant) {
		...
		for (int i = 0; i <= output.length / out.length; i++) {
			int bytesToCopy = ((output.length - i * out.length) > out.length) ? out.length
