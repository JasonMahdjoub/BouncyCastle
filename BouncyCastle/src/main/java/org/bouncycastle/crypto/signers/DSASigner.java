...
public class DSASigner implements DSA {
	...
	private BigInteger getRandomizer(BigInteger q, SecureRandom provided) {
		// Calculate a random multiple of q to add to k. Note that g^q = 1 (mod p), so
		// adding multiple of q to k does not change r.
		int randomBits = 7;

		return new BigInteger(randomBits, provided != null ? provided : new SecureRandom()).add(BigInteger.valueOf(128))
				.multiply(q);
	}
}
