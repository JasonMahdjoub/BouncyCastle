...
public class ECNRSigner implements DSA {
	 ...
	 * with. Generally, the order of the curve should be at least as long as the
	 * hash of the message of interest, and with ECNR it *must* be at least as long.
	 *
	 * @param digest
	...
	/**
	 * return true if the value r and s represent a signature for the message passed
	 * in. Generally, the order of the curve should be at least as long as the hash
	 * of the message of interest, and with ECNR, it *must* be at least as long. But
	 * just in case the signer applied mod(n) to the longer digest, this
	 * implementation will apply mod(n) during verification.
	 *
	 * @param digest
