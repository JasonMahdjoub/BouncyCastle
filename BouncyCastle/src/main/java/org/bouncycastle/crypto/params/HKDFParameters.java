...
public class HKDFParameters implements DerivationParameters {
	...
	private HKDFParameters(final byte[] ikm, final boolean skip, final byte[] salt, final byte[] info) {
		if (ikm == null) {
	...
	/**
	 * Generates parameters for HKDF, specifying both the optional salt and optional
	 * info. Step 1: Extract won't be skipped.
	 *
	 * @param ikm
	 ...
	 */
	public static HKDFParameters skipExtractParameters(final byte[] ikm, final byte[] info) {
	...
	/**
	 * Returns the salt, or null if the salt should be generated as a byte array of
	 * HashLen zeros.
	 *
