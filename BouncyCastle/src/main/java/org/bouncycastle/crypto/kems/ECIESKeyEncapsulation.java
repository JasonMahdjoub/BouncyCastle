...
public class ECIESKeyEncapsulation implements KeyEncapsulation {
	 ...
	 *            the random source for the session key.
	 * @param cofactorMode
	 *            if true use the new cofactor ECDH.
	 * @param oldCofactorMode
	 *            if true use the old cofactor ECDH.
	 * @param singleHashMode
	 *            if true use single hash mode.
	 */
	public ECIESKeyEncapsulation(DerivationFunction kdf, SecureRandom rnd, boolean cofactorMode,
			boolean oldCofactorMode, boolean singleHashMode) {
		this.kdf = kdf;
		this.rnd = rnd;
	...
	public CipherParameters encrypt(byte[] out, int outOff, int keyLen) throws IllegalArgumentException {
		...
		ECPoint[] ghTilde = new ECPoint[] { basePointMultiplier.multiply(ecParams.getG(), r),
