...
public class RSAKeyPairGenerator implements AsymmetricCipherKeyPairGenerator {
	...
	public AsymmetricCipherKeyPair generateKeyPair() {
		...
		while (!done) {
			...
			for (;;) {
				 ...
				 * composites may be weak against a version of the number-field-sieve for
				 * factoring.
				 *
	...
	private static int getNumberOfIterations(int bits, int certainty) {
		/*
		 * NOTE: We enforce a minimum 'certainty' of 100 for bits >= 1024 (else 80).
		 * Where the certainty is higher than the FIPS 186-4 tables (C.2/C.3) cater to,
		 * extra iterations are added at the "worst case rate" for the excess.
		 */
		if (bits >= 1536) {
