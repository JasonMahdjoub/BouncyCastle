...
public class DSAParametersGenerator {
	...
	private DSAParameters generateParameters_FIPS186_3() {
		// A.1.1.2 Generation of the Probable Primes p and q Using an Approved Hash
		// Function
		// FIXME This should be configurable (digest size in bits must be >= N)
		Digest d = digest;
		int outlen = d.getDigestSize() * 8;

		// 1. Check that the (L, N) pair is in the list of acceptable (L, N pairs) (see
		// Section 4.2). If
		...
		for (;;) {
			...
			for (int counter = 0; counter < counterLimit; ++counter) {
				...
				// 11.2 W = V0 + (V1 ∗ 2^outlen) + ... + (V^(n–1) ∗ 2^((n–1) ∗ outlen)) + ((Vn
				// mod 2^b) ∗ 2^(n ∗ outlen)).
				{
					for (int j = 1; j <= n; ++j) {
						inc(offset);
				...
				if (isProbablePrime(p)) {
					// 11.8 If p is determined to be prime, then return VALID and the values of p, q
					// and
	...
	private boolean isProbablePrime(BigInteger x) {
		/*
		 * TODO Use Primes class for FIPS 186-4 C.3 primality checking - but it breaks
		 * existing tests using FixedSecureRandom
		 */
		// return !Primes.hasAnySmallFactors(x) && Primes.isMRProbablePrime(x, random,
		// iterations);
	...
	private static BigInteger calculateGenerator_FIPS186_3_Verifiable(Digest d, BigInteger p, BigInteger q, byte[] seed,
			int index) {
