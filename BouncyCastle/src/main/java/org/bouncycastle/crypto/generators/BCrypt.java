...
/**
 * Core of password hashing scheme Bcrypt, designed by Niels Provos and David
 * Mazières, corresponds to the C reference implementation.
 * <p>
 * This implementation does not correspondent to the 1999 published paper "A
 * Future-Adaptable Password Scheme" of Niels Provos and David Mazières, see:
 ...
 * state <- ExpandKey(state, 0, key) state <- ExpandKey(state, 0, salt) This
 * corresponds to the OpenBSD reference implementation of Bcrypt.
 * </p>
 * <p>
 * Note: There is no successful cryptanalysis (status 2015), but the amount of
 * memory and the band width of Bcrypt may be insufficient to effectively
 * prevent attacks with custom hardware like FPGAs, ASICs
 * </p>
 * <p>
...
public final class BCrypt {
	...
	private static final int[] MAGIC_STRING = { 0x4F727068, 0x65616E42, 0x65686F6C, 0x64657253, 0x63727944,
			0x6F756274 };
	...
	private final static int[] KP = { 0x243F6A88, 0x85A308D3, 0x13198A2E, 0x03707344, 0xA4093822, 0x299F31D0,
			...
			KS0 = { 0xD1310BA6, 0x98DFB5AC, 0x2FFD72DB, 0xD01ADFB7, 0xB8E1AFED, 0x6A267E96, 0xBA7C9045, 0xF12C7F99,
			...
			KS1 = { 0x4B7A70E9, 0xB5B32944, 0xDB75092E, 0xC4192623, 0xAD6EA6B0, 0x49A7DF7D, 0x9CEE60B8, 0x8FEDB266,
			...
			KS2 = { 0xE93D5A68, 0x948140F7, 0xF64C261C, 0x94692934, 0x411520F7, 0x7602D4F7, 0xBCF46B2E, 0xD4A20068,
			...
			KS3 = { 0x3A39CE37, 0xD3FAF5CF, 0xABC27737, 0x5AC52D1B, 0x5CB0679E, 0x4FA33742, 0xD3822740, 0x99BC9BBE,
	...
	private int F(int x) {
		return (((S[(x >>> 24)] + S[SBOX_SK + ((x >>> 16) & 0xff)]) ^ S[SBOX_SK2 + ((x >>> 8) & 0xff)])
				+ S[SBOX_SK3 + (x & 0xff)]);
	}
	...
	/*
	 * Initialize the S-boxes and the P-array, with a fixed string This string
	...
	/*
	 * XOR P with key cyclic. This is the first part of ExpandKey function
	 */
	...
	private byte[] encryptMagicString() {
		int[] text = { MAGIC_STRING[0], MAGIC_STRING[1], MAGIC_STRING[2], MAGIC_STRING[3], MAGIC_STRING[4],
				MAGIC_STRING[5] };
		for (int i = 0; i < 64; i++) {
	 ...
	 */
	// Blowfish spec limits keys to 448bit/56 bytes to ensure all bits of key affect
	// all ciphertext
	// bits, but technically algorithm handles 72 byte keys and most implementations
	// support this.
	 ...
	 * <p>
	 * This implements the raw <b>bcrypt</b> function as defined in the bcrypt
	 * specification, not the crypt encoded version implemented in OpenBSD.
	 * </p>
	 * 
	 * @param password
	 *            the password bytes (up to 72 bytes) to use for this invocation.
	 * @param salt
	 *            the 128 bit salt to use for this invocation.
	 * @param cost
	 *            the bcrypt cost parameter. The cost of the bcrypt function grows
	 *            as <code>2^cost</code>. Legal values are 4..31 inclusive.
