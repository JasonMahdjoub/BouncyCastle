...
/**
 * Password hashing scheme BCrypt, designed by Niels Provos and David Mazières,
 * using the String format and the Base64 encoding of the reference
 * implementation on OpenBSD
 */
public class OpenBSDBCrypt {
	private static final byte[] encodingTable = // the Bcrypts encoding table for OpenBSD
			{ (byte) '.', (byte) '/', (byte) 'A', (byte) 'B', (byte) 'C', (byte) 'D', (byte) 'E', (byte) 'F',
					...
					(byte) 'u', (byte) 'v', (byte) 'w', (byte) 'x', (byte) 'y', (byte) 'z', (byte) '0', (byte) '1',
	...
	/**
	 * Creates a 60 character Bcrypt String, including version, cost factor, salt
	 * and hash, separated by '$'
	 *
	 * @param cost
	 ...
	 *
	 * @param bcryptString
	 *            a 60 character Bcrypt String, including version, cost factor, salt
	 *            and hash, separated by '$'
	 * @param password
	...
	public static boolean checkPassword(String bcryptString, char[] password) {
		...
		byte[] salt = decodeSaltString(
				bcryptString.substring(bcryptString.lastIndexOf('$') + 1, bcryptString.length() - 31));
	 ...
	 * 
	 * @exception InvalidArgumentException if the parameter contains a value other
	 * than from Bcrypts base 64 encoding table
	 */
