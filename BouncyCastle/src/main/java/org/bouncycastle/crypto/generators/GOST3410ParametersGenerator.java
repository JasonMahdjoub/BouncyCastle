...
public class GOST3410ParametersGenerator {
		...
		for (int i = 0; i < s; i++) {
			...
			step6: for (;;) {
				...
				BigInteger N = TWO.pow(t[m] - 1).divide(p[m + 1])
						.add((TWO.pow(t[m] - 1).multiply(Ym)).divide(p[m + 1].multiply(TWO.pow(16 * rm))));
	...
	private long procedure_Aa(long x0, long c, BigInteger[] pq, int size) {
		...
		for (int i = 0; i < s; i++) {
			...
			step6: for (;;) {
				...
				// step 9
				BigInteger N = TWO.pow(t[m] - 1).divide(p[m + 1])
						.add((TWO.pow(t[m] - 1).multiply(Ym)).divide(p[m + 1].multiply(TWO.pow(32 * rm))));
	...
	private void procedure_B(int x0, int c, BigInteger[] pq) {
		...
		step3: for (;;) {
			...
			BigInteger N = TWO.pow(tp - 1).divide(q.multiply(Q))
					.add((TWO.pow(tp - 1).multiply(Y)).divide(q.multiply(Q).multiply(TWO.pow(1024))));
	...
	/**
	 * Procedure C procedure generates the a value from the given p,q, returning the
	 * a value.
	 */
	...
	/**
	 * which generates the p , q and a values from the given parameters, returning
	 * the GOST3410Parameters object.
	 */
