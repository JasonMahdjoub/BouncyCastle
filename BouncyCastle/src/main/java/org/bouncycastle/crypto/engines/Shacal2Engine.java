...
/**
 * Block cipher Shacal2, designed by Helena Handschuh and David Naccache, based
 * on hash function SHA-256, using SHA-256-Initialization-Values as data and
 * SHA-256-Data as key.
 * <p>
 * A description of Shacal can be found at:
 * http://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.3.4066 Best known
 * cryptanalytic (Wikipedia 11.2013): Related-key rectangle attack on 44-rounds
 * (Jiqiang Lu, Jongsung Kim). Comments are related to SHA-256-Naming as
...
public class Shacal2Engine implements BlockCipher {
	...
	public void setKey(byte[] kb) {
		...
		for (int i = 16; i < 64; i++) { // Key-Expansion, implicitly Zero-Padding for 16 > i > kb.length/4
			workingKey[i] = ((workingKey[i - 2] >>> 17 | workingKey[i - 2] << -17) // corresponds to ROTL n(x) of FIPS
																					// PUB 180-2
					^ (workingKey[i - 2] >>> 19 | workingKey[i - 2] << -19) ^ (workingKey[i - 2] >>> 10)) // corresponds
																											// to
																											// sigma1(x)-Function
																											// of FIPS
																											// PUB 180-2
					...
					+ ((workingKey[i - 15] >>> 7 | workingKey[i - 15] << -7)
							^ (workingKey[i - 15] >>> 18 | workingKey[i - 15] << -18) ^ (workingKey[i - 15] >>> 3)) // corresponds
																													// to
																													// sigma0(x)-Function
																													// of
																													// FIPS
	...
	private void encryptBlock(byte[] in, int inOffset, byte[] out, int outOffset) {
		...
		for (int i = 0; i < ROUNDS; i++) {
			int tmp = (((block[4] >>> 6) | (block[4] << -6)) ^ ((block[4] >>> 11) | (block[4] << -11))
					^ ((block[4] >>> 25) | (block[4] << -25))) + ((block[4] & block[5]) ^ ((~block[4]) & block[6]))
			...
			block[1] = block[0];
			block[0] = tmp
					+ (((block[0] >>> 2) | (block[0] << -2)) ^ ((block[0] >>> 13) | (block[0] << -13))
	...
	private void decryptBlock(byte[] in, int inOffset, byte[] out, int outOffset) {
		...
		for (int i = ROUNDS - 1; i > -1; i--) {
			int tmp = block[0]
					- (((block[1] >>> 2) | (block[1] << -2)) ^ ((block[1] >>> 13) | (block[1] << -13))
			...
			block[7] = tmp - K[i] - workingKey[i]
					- (((block[4] >>> 6) | (block[4] << -6)) ^ ((block[4] >>> 11) | (block[4] << -11))
	...
	private void byteBlockToInts(byte[] bytes, int[] block, int bytesPos, int blockPos) {
		for (int i = blockPos; i < BLOCK_SIZE / 4; i++) {
			block[i] = ((bytes[bytesPos++] & 0xFF) << 24) | ((bytes[bytesPos++] & 0xFF) << 16)
					| ((bytes[bytesPos++] & 0xFF) << 8) | (bytes[bytesPos++] & 0xFF);
		}
	}
