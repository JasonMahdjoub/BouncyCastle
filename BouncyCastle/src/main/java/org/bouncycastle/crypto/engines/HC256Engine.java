 ...
 * </p>
 * <p>
 * Its brother, HC-128, is a third phase candidate in the eStream contest. The
 * algorithm is patent-free. No attacks are known as of today (April 2007). See
 * 
...
public class HC256Engine implements StreamCipher {
	...
	private int step() {
		...
		if (cnt < 1024) {
			...
			x = p[(j - 12 & 0x3FF)];
			ret = (q[x & 0xFF] + q[((x >> 8) & 0xFF) + 256] + q[((x >> 16) & 0xFF) + 512] + q[((x >> 24) & 0xFF) + 768])
					^ p[j];
		} else {
			int x = q[(j - 3 & 0x3FF)];
			...
			x = q[(j - 12 & 0x3FF)];
			ret = (p[x & 0xFF] + p[((x >> 8) & 0xFF) + 256] + p[((x >> 16) & 0xFF) + 512] + p[((x >> 24) & 0xFF) + 768])
					^ q[j];
		}
		cnt = cnt + 1 & 0x7FF;
		return ret;
	 ...
	 *
	 * @param forEncryption
	 *            whether or not we are for encryption. Irrelevant, as encryption
	 *            and decryption are the same.
	 * @param params
	 *            the parameters required to set up the cipher.
	 * @throws IllegalArgumentException
	 *             if the params argument is inappropriate (ie. the key is not 256
	 *             bit long).
	 */
