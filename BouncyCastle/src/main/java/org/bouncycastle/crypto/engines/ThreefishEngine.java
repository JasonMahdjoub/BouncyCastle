...
/**
 * Implementation of the Threefish tweakable large block cipher in 256, 512 and
 * 1024 bit block sizes.
 * <p>
 * This is the 1.3 version of Threefish defined in the Skein hash function
 * submission to the NIST SHA-3 competition in October 2010.
 * <p>
 * Threefish was designed by Niels Ferguson - Stefan Lucks - Bruce Schneier -
 * Doug Whiting - Mihir Bellare - Tadayoshi Kohno - Jon Callas - Jesse Walker.
 * <p>
 * This implementation inlines all round functions, unrolls 8 rounds, and uses
 * 1.2k of static tables to speed up key schedule injection. <br>
...
public class ThreefishEngine implements BlockCipher {
	...
	/**
	 * Tweak bytes (2 byte t1,t2, calculated t3 and repeat of t1,t2 for modulo free
	 * lookup
	 */
	 ...
	 *
	 * @param blocksizeBits
	 *            the block size in bits, one of {@link #BLOCKSIZE_256},
	 *            {@link #BLOCKSIZE_512}, {@link #BLOCKSIZE_1024}.
	 */
	public ThreefishEngine(final int blocksizeBits) {
		...
		/*
		 * Provide room for original key words, extended key word and repeat of key
		 * words for modulo free lookup of key schedule words.
		 */
	 ...
	 *
	 * @param params
	 *            an instance of {@link TweakableBlockCipherParameters}, or
	 *            {@link KeyParameter} (to use a 0 tweak)
	 */
	public void init(boolean forEncryption, CipherParameters params) throws IllegalArgumentException {
		...
		if (params instanceof TweakableBlockCipherParameters) {
			...
			throw new IllegalArgumentException(
					"Invalid parameter passed to Threefish init - " + params.getClass().getName());
		}
		...
		if (keyBytes != null) {
			if (keyBytes.length != this.blocksizeBytes) {
				throw new IllegalArgumentException(
						"Threefish key must be same size as block (" + blocksizeBytes + " bytes)");
			}
	 ...
	 * @param tweak
	 *            the 2 word (128 bit) tweak, or <code>null</code> to use the
	 *            current tweak.
	 */
	...
	private void setKey(long[] key) {
		if (key.length != this.blocksizeWords) {
			throw new IllegalArgumentException(
					"Threefish key must be same size as block (" + blocksizeWords + " words)");
		}

		/*
		 * Full subkey schedule is deferred to execution to avoid per cipher overhead
		 * (10k for 512, 20k for 1024).
		 * 
		 * Key and tweak word sequences are repeated, and static MOD17/MOD9/MOD5/MOD3
		 * calculations used, to avoid expensive mod computations during cipher
		 * operation.
		 */
	...
	private void setTweak(long[] tweak) {
		...
		/*
		 * Tweak schedule partially repeated to avoid mod computations during cipher
		 * operation
		 */
		t[0] = tweak[0];
		t[1] = tweak[1];
	 ...
	 * @param out
	 *            a block sized buffer of words to receive the output of the
	 *            operation.
	 * @return the number of 8 byte words processed (which will be the same as the
	 *         block size).
	...
	private static final class Threefish256Cipher extends ThreefishCipher {
		...
		void encryptBlock(long[] block, long[] out) {
			 ...
			 * 
			 * Unrolling to multiples of 4 avoids the mod 4 check for key injection, and
			 * allows inlining of the permutations, which cycle every of 2 rounds (avoiding
			 * array index/lookup).
			 * 
			 * Unrolling to multiples of 8 avoids the mod 8 rotation constant lookup, and
			 * allows inlining constant rotation values (avoiding array index/lookup).
			 */
	...
	private static final class Threefish512Cipher extends ThreefishCipher {
		...
		public void encryptBlock(long[] block, long[] out) {
			 ...
			 * 
			 * Unrolling to multiples of 4 avoids the mod 4 check for key injection, and
			 * allows inlining of the permutations, which cycle every of 4 rounds (avoiding
			 * array index/lookup).
			 * 
			 * Unrolling to multiples of 8 avoids the mod 8 rotation constant lookup, and
			 * allows inlining constant rotation values (avoiding array index/lookup).
