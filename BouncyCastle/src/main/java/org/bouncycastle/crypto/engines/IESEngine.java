...
/**
 * Support class for constructing integrated encryption ciphers for doing basic
 * message exchanges on top of key agreement ciphers. Follows the description
 * given in IEEE Std 1363a.
 */
public class IESEngine {
	...
	/**
	 * set up for use with stream mode, where the key derivation function is used to
	 * provide a stream of bytes to xor with the message.
	 *
	 * @param agree
	...
	/**
	 * Set up for use in conjunction with a block cipher to handle the message.It is
	 * <b>strongly</b> recommended that the cipher is not in ECB mode.
	 *
	 * @param agree
	 *            the key agreement used as the basis for the encryption
	 * @param kdf
	 ...
	 * @param params
	 *            encoding and derivation parameters, may be wrapped to include an
	 *            IV for an underlying block cipher.
	 */
	...
	private byte[] decryptBlock(byte[] in_enc, int inOff, int inLen) throws InvalidCipherTextException {
		...
		// note order is important: set up keys, do simple encryptions, check mac, do
		// final encryption.
		if (cipher == null) {
			// Streaming mode.
		...
		if (forEncryption) {
			...
			if (keyParser != null) {
				...
				try {
					...
					throw new InvalidCipherTextException("unable to recover ephemeral public key: " + e.getMessage(),
							e);
