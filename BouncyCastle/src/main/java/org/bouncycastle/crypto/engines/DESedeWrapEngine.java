...
/**
 * Wrap keys according to <A HREF="https://www.ietf.org/rfc/rfc3217.txt"> RFC
 * 3217</A>.
 * <p>
 * Note:
 * <ul>
 * <li>if you are using this to wrap triple-des keys you need to set the parity
 * bits on the key and, if it's a two-key triple-des key, pad it yourself.
 * </ul>
 */
public class DESedeWrapEngine implements Wrapper {
	...
	/** Field IV2 */
	private static final byte[] IV2 = { (byte) 0x4a, (byte) 0xdd, (byte) 0xa2, (byte) 0x2c, (byte) 0x79, (byte) 0xe8,
			(byte) 0x21, (byte) 0x05 };
	 ...
	 *
	 * @param forWrapping
	 *            true if for wrapping, false otherwise.
	 * @param param
	 *            necessary parameters, may include KeyParameter,
	 *            ParametersWithRandom, and ParametersWithIV
	 */
	 ...
	 *
	 * @param in
	 *            byte array containing the wrapped key.
	 * @param inOff
	 *            off set into in that the data starts at.
	 * @param inLen
	 *            length of the data.
	...
	public byte[] unwrap(byte[] in, int inOff, int inLen) throws InvalidCipherTextException {
		 ...
		 * type. It must be 40 bytes for a 168 bit key and either 32, 40, or // 48 bytes
		 * for a 128, 192, or 256 bit key. If the length is not supported // or
		 * inconsistent with the algorithm for which the key is intended, // return
		 * error. // // we do not accept 168 bit keys. it has to be 192 bit. int lengthA
		 * = (estimatedKeyLengthInBit / 8) + 16; int lengthB = estimatedKeyLengthInBit %
		 * 8;
		 * 
		 * if ((lengthA != keyToBeUnwrapped.length) || (lengthB != 0)) { throw new
		 * XMLSecurityException("empty"); }
		 */

		// Decrypt the cipher text with TRIPLedeS in CBC mode using the KEK
		// and an initialization vector (IV) of 0x4adda22c79e82105. Call the output
		// TEMP3.
		ParametersWithIV param2 = new ParametersWithIV(this.param, IV2);
	...
	/**
	 * Some key wrap algorithms make use of the Key Checksum defined in CMS
	 * [CMS-Algorithms]. This is used to provide an integrity check value for the
	 * key being wrapped. The algorithm is
	 *
	 * - Compute the 20 octet SHA-1 hash on the key being wrapped. - Use the first 8
	 * octets of this hash as the checksum value.
	 *
