...
public class Salsa20Engine implements SkippingStreamCipher {
	...
	private final static int[] TAU_SIGMA = Pack
			.littleEndianToInt(Strings.toByteArray("expand 16-byte k" + "expand 32-byte k"), 0, 8);
	...
	/** @deprecated */
	protected final static byte[] sigma = Strings.toByteArray("expand 32-byte k"),
	...
	public void init(boolean forEncryption, CipherParameters params) {
		...
		if (iv == null || iv.length != getNonceSize()) {
			throw new IllegalArgumentException(
					getAlgorithmName() + " requires exactly " + getNonceSize() + " bytes of IV");
		}
		...
		if (keyParam == null) {
			if (!initialised) {
				throw new IllegalStateException(
						getAlgorithmName() + " KeyParameter can not be null for first initialisation");
			}
			...
			throw new IllegalArgumentException(
					getAlgorithmName() + " Init parameters must contain a KeyParameter (or null for re-init)");
		}
