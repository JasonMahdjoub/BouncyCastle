...
public class NaccacheSternEngine implements AsymmetricBlockCipher {
	 ...
	 * 
	 * @see org.bouncycastle.crypto.AsymmetricBlockCipher#processBlock(byte[], int,
	 *      int)
	 */
	public byte[] processBlock(byte[] in, int inOff, int len) throws InvalidCipherTextException {
		...
		if (forEncryption) {
			...
			for (int i = 0; i < primes.size(); i++) {
				BigInteger exp = input.modPow(priv.getPhi_n().divide((BigInteger) primes.elementAt(i)),
						priv.getModulus());
				Vector al = lookup[i];
				if (lookup[i].size() != ((BigInteger) primes.elementAt(i)).intValue()) {
					...
					throw new InvalidCipherTextException(
							"Error in lookup Array for " + ((BigInteger) primes.elementAt(i)).intValue()
				...
				if (lookedup == -1) {
					if (debug) {
						...
						System.out.println(
								"LookupList for " + primes.elementAt(i) + " with size " + lookup[i].size() + " is: ");
	...
	public byte[] addCryptedBlocks(byte[] block1, byte[] block2) throws InvalidCipherTextException {
		...
		byte[] output = key.getModulus().toByteArray();
		Arrays.fill(output, (byte) 0);
		System.arraycopy(m1m2Crypt.toByteArray(), 0, output, output.length - m1m2Crypt.toByteArray().length,
