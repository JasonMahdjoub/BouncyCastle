...
/**
 * SM2 Key Exchange protocol - based on
...
public class SM2KeyExchange {
	...
	public byte[][] calculateKeyWithConfirmation(int kLen, byte[] confirmationTag, CipherParameters pubParam) {
		...
		if (initiator) {
			...
			byte[] inner = calculateInnerHash(digest, U, za, zb, ephemeralPubPoint,
			...
			byte[] inner = calculateInnerHash(digest, U, zb, za, otherPub.getEphemeralPublicKey().getQ(),
					ephemeralPubPoint);
