 ...
 * <p>
 * The recommended way to perform a J-PAKE exchange is by using two
 * {@link JPAKEParticipant}s. Internally, those participants call these
 * primitive operations in {@link JPAKEUtil}.
 * <p>
 * The primitives, however, can be used without a {@link JPAKEParticipant} if
 * needed.
 */
public class JPAKEUtil {
	...
	/**
	 * Converts the given password to a {@link BigInteger} for use in arithmetic
	 * calculations.
	 */
	 ...
	 * array has two elements {g^v, r = v-x*h} for x.
	 * 
	 * @throws IOException
	 */
	public static BigInteger[] calculateZeroKnowledgeProof(BigInteger p, BigInteger q, BigInteger g, BigInteger gx,
			BigInteger x, Serializable participantId, Digest digest, SecureRandom random) throws IOException {
	...
	private static BigInteger calculateHashForZeroKnowledgeProof(BigInteger g, BigInteger gr, BigInteger gx,
			Serializable participantId, Digest digest) throws IOException {
		digest.reset();
	 ...
	 * <p>
	 * <blockquote> Alice could simply check ga != 1 to ensure it is a generator. In
	 * fact, as we will explain in Section 3, (x1 + x3 + x4 ) is random over Zq even
	 * in the face of active attacks. Hence, the probability for ga = 1 is extremely
	 * small - on the order of 2^160 for 160-bit q. </blockquote>
	 *
	 * @throws CryptoException
	 *             if ga is 1
	 */
	 ...
	 * @throws IOException
	 */
	public static void validateZeroKnowledgeProof(BigInteger p, BigInteger q, BigInteger g, BigInteger gx,
			BigInteger[] zeroKnowledgeProof, Serializable participantId, Digest digest)
		...
		if (!(gx.compareTo(ZERO) == 1 && // g^x > 0
				...
				/*
				 * Below, I took an straightforward way to compute g^r * g^x^h, which needs 2
				 * exp. Using a simultaneous computation technique would only need 1 exp.
				 */
	...
	/**
	 * Calculates the keying material, which can be done after round 2 has
	 * completed. A session key must be derived from this key material using a
	 * secure key derivation function (KDF). The KDF used to derive the key is
	 * handled externally (i.e. not by {@link JPAKEParticipant}).
	 * 
	 * <pre>
	 * KeyingMaterial = (B/g^{x2*x4*s})^x2
	 * </pre>
	 */
	public static BigInteger calculateKeyingMaterial(BigInteger p, BigInteger q, BigInteger gx4, BigInteger x2,
			BigInteger s, BigInteger B) {
		return gx4.modPow(x2.multiply(s).negate().mod(q), p).multiply(B).modPow(x2, p);
	}

	/**
	 * Validates that the given participant ids are not equal. (For the J-PAKE
	 * exchange, each participant must use a unique id.)
	 *
	 * @throws CryptoException
	...
	public static void validateParticipantIdsDiffer(Serializable participantId1, Serializable participantId2)
		...
		if (participantId1.equals(participantId2)) {
			throw new CryptoException("Both participants are using the same participantId (" + participantId1
					+ "). This is not allowed. " + "Each participant must use a unique participantId.");
		}
	}

	/**
	 * Validates that the given participant ids are equal. This is used to ensure
	 * that the payloads received from each round all come from the same
	 * participant.
	 *
	 * @throws CryptoException
	 *             if the participantId strings are equal.
	...
	public static void validateParticipantIdsEqual(Serializable expectedParticipantId, Serializable actualParticipantId)
		...
		if (!expectedParticipantId.equals(actualParticipantId)) {
			throw new CryptoException("Received payload from incorrect partner (" + actualParticipantId
					+ "). Expected to receive payload from " + expectedParticipantId + ".");
		}
	}
	 ...
	 * @throws IOException
	 */
	public static BigInteger calculateMacTag(Serializable participantId, Serializable partnerParticipantId,
			BigInteger gx1, BigInteger gx2, BigInteger gx3, BigInteger gx4, BigInteger keyingMaterial, Digest digest)
			throws IOException {
		...
		/*
		 * MacData = "KC_1_U" || participantId_Alice || participantId_Bob || gx1 || gx2
		 * || gx3 || gx4.
		 */
	...
	/**
	 * Calculates the MacKey (i.e. the key to use when calculating the MagTag for
	 * key confirmation).
	 * 
	 * <pre>
	 * MacKey = H(K || "JPAKE_KC")
	...
	private static byte[] calculateMacKey(BigInteger keyingMaterial, Digest digest) {
		...
		/*
		 * This constant is used to ensure that the macKey is NOT the same as the
		 * derived key.
		 */
	 ...
	 * @throws IOException
	 */
	public static void validateMacTag(Serializable participantId, Serializable partnerParticipantId, BigInteger gx1,
			BigInteger gx2, BigInteger gx3, BigInteger gx4, BigInteger keyingMaterial, Digest digest,
			BigInteger partnerMacTag) throws CryptoException, IOException {
		/*
		 * Calculate the expected MacTag using the parameters as the partner would have
		 * used when the partner called calculateMacTag.
		 * 
		 * i.e. basically all the parameters are reversed. participantId <->
		 * partnerParticipantId x1 <-> x3 x2 <-> x4
		 */
		BigInteger expectedMacTag = calculateMacTag(partnerParticipantId, participantId, gx3, gx4, gx1, gx2,
				keyingMaterial, digest);

		if (!expectedMacTag.equals(partnerMacTag)) {
			throw new CryptoException("Partner MacTag validation failed. "
