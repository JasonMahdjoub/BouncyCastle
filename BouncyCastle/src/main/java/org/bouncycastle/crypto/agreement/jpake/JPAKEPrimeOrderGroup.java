 ...
 * <p>
 * Typically a Schnorr group is used. In general, J-PAKE can use any prime order
 * group that is suitable for public key cryptography, including elliptic curve
 * cryptography.
 * <p>
 * See {@link JPAKEPrimeOrderGroups} for convenient standard groups.
 * <p>
 * NIST <a href=
 * "http://csrc.nist.gov/groups/ST/toolkit/documents/Examples/DSA2_All.pdf">publishes</a>
...
public class JPAKEPrimeOrderGroup {
	 ...
	 * <p>
	 * These checks prevent trivial mistakes. However, due to the small
	 * uncertainties if p and q are not prime, advanced attacks are not prevented.
	...
	/**
	 * Internal package-private constructor used by the pre-approved groups in
	 * {@link JPAKEPrimeOrderGroups}. These pre-approved groups can avoid the
	 * expensive checks.
	 */
	JPAKEPrimeOrderGroup(BigInteger p, BigInteger q, BigInteger g, boolean skipChecks) {
		...
		if (!skipChecks) {
			...
			/*
			 * Note that these checks do not guarantee that p and q are prime. We just have
			 * reasonable certainty that they are prime.
			 */
